...
CpuBackend::GgmlQuantizedMatMulPath CpuBackend::MatMulGgmlQuantizedTransBWithPath(const CpuExecutionOptions& options, const Tensor& A,
    ...
    if (weight_type == GGML_TYPE_Q4_K && A.shape[0] >= 4 && (W.cols % kernels::kQ4KSuperBlock) == 0) {
        if (internal::RunQ4KRawBatchedDenseGemm(options, this, W.data, A.DataAs<float>(), C->DataAs<float>(), A.shape[0], W.rows,
