...
std::unique_ptr<BackendRegistry> BackendRegistry::CreateForEngine() {
    auto registry = std::unique_ptr<BackendRegistry>(new BackendRegistry());
    registry->RegisterCpuBackend();
    registry->non_cpu_fallback_ = &Instance();
    return registry;
}
...
void BackendRegistry::RegisterCpuBackend() {
    ...
    // ===========================================================================
    // Step 2: Register the experimental QNN scaffold only when explicitly opted in
    // ===========================================================================
    if (QnnBackend::ExperimentalRegistrationEnabled() && QnnBackend::CheckAvailability()) {
        try {
            auto qnn_backend = std::make_unique<QnnBackend>();
            std::cout << "[BackendRegistry] Registered experimental QNN backend: " << qnn_backend->Name() << std::endl;
...
ComputeBackend* BackendRegistry::Get(DeviceType device) {
    ...
    return device != DeviceType::CPU && non_cpu_fallback_ ? non_cpu_fallback_->Get(device) : nullptr;
}
...
ComputeBackend* BackendRegistry::GetDefault() {
    ...
    {
        std::lock_guard<std::mutex> lock(mutex_);
        selected = default_device_;
    }
    if (auto* backend = Get(selected)) return backend;
...
bool BackendRegistry::SetDefault(DeviceType device) {
    const bool registered = IsRegistered(device);
    std::lock_guard<std::mutex> lock(mutex_);
    if (registered) {
        default_device_ = device;
...
bool BackendRegistry::SupportsProfile(DeviceType device, InferenceProfile profile, bool include_fallback) {
    ...
    return device != DeviceType::CPU && non_cpu_fallback_
               ? non_cpu_fallback_->SupportsProfile(device, profile, include_fallback)
               : false;
}
