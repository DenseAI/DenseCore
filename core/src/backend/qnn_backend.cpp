 ...
 *
 * Implements an experimental QNN scaffold using the Qualcomm AI Stack
 * (QNN/HTP). Native execution coverage is still partial; most ops continue
 * through maintained CPU kernels.
 */
...
constexpr const char* kQnnExperimentalEnv = "DENSECORE_QNN_EXPERIMENTAL";
...
bool QnnBackend::ExperimentalRegistrationEnabled() {
    return EnvFlagEnabled(kQnnExperimentalEnv);
}
