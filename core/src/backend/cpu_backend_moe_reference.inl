...
bool IsMoEDebugTimingEnabled() {
    static const bool enabled = []() {
        const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_MOE_DEBUG_TIMING");
...
bool IsMoEFFNDebugTimingEnabled() {
    static const bool enabled = []() {
        const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_MOE_FFN_TIMING");
        return env && env[0] != '\0' && std::strcmp(env, "0") != 0;
    }();
    return enabled;
}
...
bool IsMoEMatmulPathDebugEnabled() {
    const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_MOE_MATMUL_PATHS");
    return env && env[0] != '\0' && std::strcmp(env, "0") != 0;
}
...
bool IsMoECachePolicyDebugEnabled() {
    const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_MOE_CACHE_POLICY");
    return env && env[0] != '\0' && std::strcmp(env, "0") != 0;
}

bool IsGemma4PackedChecksumDebugEnabled() {
    const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_GEMMA4_PACKED_CHECKSUM");
    return env && env[0] != '\0' && std::strcmp(env, "0") != 0;
}

bool IsGemma4ParityTraceEnabled() {
    // Cached deliberately, matching the attention-side twin in
    // llm/attention/exec.cpp. This runs per projection on MoE worker threads, and
    // getenv() is not safe against a concurrent setenv(): a test that flips an env
    // var while a MoE batch is in flight can realloc environ under these readers
    // and segfault inside getenv. Reading once at first use also keeps a diagnostic
    // switch off the hot path.
    static const bool enabled = []() {
        const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_GEMMA4_PARITY_TRACE");
        return env && env[0] != '\0' && std::strcmp(env, "0") != 0;
    }();
    return enabled;
}
...
int Gemma4ParityTraceLayerFilter() {
    // Same reasoning as IsGemma4ParityTraceEnabled(): read once, never from a
    // worker thread racing a setenv.
    static const int layer_filter = []() {
        const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_GEMMA4_PARITY_TRACE_LAYER");
        if (!env || env[0] == '\0') {
            return -1;
        }
        char* end = nullptr;
...
bool IsMoEReferenceCheckEnabled() {
    static const bool enabled = []() {
        const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_MOE_REFERENCE");
        return env && env[0] != '\0' && std::strcmp(env, "0") != 0;
    }();
    return enabled;
}
...
float MoEReferenceTolerance() {
    static const float tol = []() {
        const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_MOE_REFERENCE_TOL");
        if (!env || env[0] == '\0') {
            return 1e-2f;
        }
        char* end = nullptr;
...
bool ShouldRunMoEReferenceCheck() {
    ...
    static std::atomic<int> remaining_budget{[]() {
        const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_MOE_REFERENCE_MAX_CALLS");
        if (!env || env[0] == '\0') {
            return 1;
        }
        char* end = nullptr;
...
void RecordMoEReferencePathTrace(CpuBackend* backend, int layer_idx, const CpuBatchTraceView* batch,
