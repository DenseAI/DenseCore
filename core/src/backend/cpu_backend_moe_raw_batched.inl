...
bool ShouldUseLFM2PrefillQ4KRawBatchedHwy(const CpuExecutionOptions& options) {
    return options.IsLfm2() &&
           options.EffectivePhase() == CpuExecutionPhase::Prefill;
}
...
bool ComputeMoEQ4KQ8KBatchedRow(const CpuExecutionOptions& options, const void* weight_row, const uint8_t* quant_input_base, size_t quant_row_stride, int M,
    ...
    if (ShouldUseLFM2PrefillQ4KRawBatchedHwy(options) &&
...
bool ComputeMoEQ4KQ8KBatchedRowPair(const CpuExecutionOptions& options, const void* gate_weight_row, const void* up_weight_row,
                                    const uint8_t* quant_input_base, size_t quant_row_stride, int M, int K,
                                    float* gate_out_sums, float* up_out_sums,
...
#endif
    return ComputeMoEQ4KQ8KBatchedRow(options, gate_weight_row, quant_input_base, quant_row_stride, M, K, gate_out_sums) &&
           ComputeMoEQ4KQ8KBatchedRow(options, up_weight_row, quant_input_base, quant_row_stride, M, K, up_out_sums);
}
...
bool RunMoEQ4KRawBatchedProjectionImpl(const CpuExecutionOptions& options, CpuBackend* backend, const void* weight_ptr, const uint8_t* qinput_data,
    ...
    const auto compute_rows = [&](int n_start, int n_end) {
        ...
        for (int n = n_start; n < n_end; ++n) {
            ...
            for (int64_t m0 = 0; m0 < M; m0 += kMoEQ4KRawBatchedTileM) {
                ...
                if (!ComputeMoEQ4KQ8KBatchedRow(options, weight_row, q_tile, qinput_row_bytes, tile_m, static_cast<int>(K),
                                                sums)) {
    ...
    if (success && record_metrics) {
        ...
        RecordMoEKQuantRawBatchedUse(
            options.telemetry, GGML_TYPE_Q4_K,
...
bool RunMoEQ4KRawBatchedWeightedScatterProjectionImpl(const CpuExecutionOptions& options,
...
bool RunMoEKQuantRawBatchedProjectionImpl(const CpuExecutionOptions& options, CpuBackend* backend, ggml_type weight_type, const void* weight_ptr,
...
bool RunMoEQ4KRawBatchedFusedSwiGLUImpl(const CpuExecutionOptions& options, CpuBackend* backend, const void* gate_weight_ptr, const void* up_weight_ptr,
    ...
    const auto compute_rows = [&](int n_start, int n_end) {
        ...
        for (int64_t m0 = 0; m0 < M; m0 += kMoEQ4KRawBatchedTileM) {
            ...
            for (int n = n_start; n < n_end; ++n) {
                ...
                if (!ComputeMoEQ4KQ8KBatchedRowPair(options, gate_row, up_row, q_tile, qinput_row_bytes, tile_m,
    ...
    if (success) {
        ...
        RecordMoEKQuantRawBatchedUse(
            options.telemetry, GGML_TYPE_Q4_K, elapsed_ns, /*qwen_native_w2_q5k=*/false);
#if (defined(__aarch64__) || defined(_M_ARM64)) && defined(__ARM_FEATURE_DOTPROD)
        if (arm_rowpair_used.load(std::memory_order_relaxed)) {
            RecordQwenNativeMoEQ4GateUpRowPairUse(options.telemetry, elapsed_ns);
        }
#endif
    }
    return success;
...
bool RunMoEQ4KRawBatchedFusedSwiGLUToQ8Impl(const CpuExecutionOptions& options, CpuBackend* backend, const void* gate_weight_ptr, const void* up_weight_ptr,
...
bool RunMoEKQuantRawBatchedFusedSwiGLUImpl(const CpuExecutionOptions& options, CpuBackend* backend, ggml_type weight_type, const void* gate_weight_ptr,
...
bool RunMoEQ4KRawBatchedFusedGEGLU(const CpuExecutionOptions& options, CpuBackend* backend, const void* gate_weight_ptr, const void* up_weight_ptr,
                                   const uint8_t* qinput_data, size_t qinput_row_bytes, float* out_data, int64_t M,
                                   int64_t N, int64_t K, int numa_node, bool allow_parallel) {
    if (!backend || !gate_weight_ptr || !up_weight_ptr || !qinput_data || !out_data || M <= 0 ||
        M > kMoEQuantizedProjectionMaxBatch || N <= 0 || K <= 0 || (K % QK_K) != 0) {
