...
#include "densecore/backend/apple/metal_backend.h"  // For RoPE/FlashAttention fallback
...
void ANEBackend::GemmInt4(const Tensor& A, const Tensor& W, const Tensor& scales,
    ...
    if (cachedWeights) {
        ...
        // Prefer GPU dequantization when Metal supports this tensor layout.
...
bool ANEBackend::CompileTransformerLayer(const std::string& name,
    ...
    @autoreleasepool {
        ...
        // Instead, we look for a pre-compiled .mlmodelc artifact supplied by
        // the deployment.
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:modelPath]) {
            ...
            std::cout << "  Provide a compatible pre-compiled CoreML artifact in the cache directory."
                      << std::endl;
