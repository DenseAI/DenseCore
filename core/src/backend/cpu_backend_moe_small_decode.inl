...
bool TryExecuteMoESafeReferenceFastPath(CpuBackend* backend, const TransformerModel* model, int layer_idx,
                                        const CpuBatchTraceView* batch, const float* input_data, int batch_size,
...
bool IsMoEStickyDecodeDispatchEnabled() {
    static const bool enabled = []() {
        const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_DISABLE_MOE_NUMA_STICKY");
...
inline void MoESmallDecodeGateUpUnit(const CpuExecutionOptions& options, const MoESmallDecodeTileParallelRequest& req, int assignment, int split,
    ...
    const bool gate_up_ok =
        exp.use_gelu_activation
            ? TryRunGgmlQuantizedFusedGEGLUProjection(options, req.backend, gate_ptr, exp.w1_type, up_ptr, exp.w3_type,
                                                      expert_input, &hidden_tensor, tile_rows, req.hidden_dim,
                                                      expert_numa_node, /*allow_parallel=*/false,
                                                      unit_input_projection_cache)
            : TryRunGgmlQuantizedFusedSwiGLUProjection(options, req.backend, gate_ptr, exp.w1_type, up_ptr, exp.w3_type,
                                                       expert_input, &hidden_tensor, tile_rows, req.hidden_dim,
                                                       expert_numa_node, /*allow_parallel=*/false,
                                                       unit_input_projection_cache);
    if (!gate_up_ok) {
...
void RunMoESmallDecodeTileGateUp(const CpuExecutionOptions& options, const MoESmallDecodeTileParallelRequest& req, int gate_splits,
        ...
        req.backend->RunConcurrentNodeTasks(partition->num_nodes, [&](int node) {
            ...
            req.backend->GetThreadPool(node).ParallelFor(group_units, [&](int unit_start, int unit_end,
                ...
                for (int unit = unit_start; unit < unit_end && tile_ok->load(std::memory_order_relaxed); ++unit) {
                    MoESmallDecodeGateUpUnit(options, req, group[static_cast<size_t>(unit / gate_splits)], unit % gate_splits,
    ...
    req.pool->ParallelFor(req.total_assignments * gate_splits, [&](int unit_start, int unit_end, int /*thread_id*/) {
        for (int unit = unit_start; unit < unit_end && tile_ok->load(std::memory_order_relaxed); ++unit) {
            ...
            MoESmallDecodeGateUpUnit(options, req, assignment, split, gate_splits, assignment_hiddens, shared_input_cache,
                                     tile_ok);
        }
    });
}
...
inline void MoESmallDecodeDownUnit(const CpuExecutionOptions& options, const MoESmallDecodeTileParallelRequest& req, int assignment, int split,
    ...
    const int expert_numa_node = req.profiler ? req.profiler->GetExpertNumaNode(expert_id) : -1;
    if (!TryRunGgmlQuantizedProjection(options, req.backend, down_ptr, exp.w2_type, hidden_tensor, &output_tensor,
...
void RunMoESmallDecodeTileDown(const CpuExecutionOptions& options, const MoESmallDecodeTileParallelRequest& req, int down_splits,
    ...
    if (partition && partition->active) {
        req.backend->RunConcurrentNodeTasks(partition->num_nodes, [&](int node) {
            ...
            req.backend->GetThreadPool(node).ParallelFor(group_units, [&](int unit_start, int unit_end,
                                                                          int /*thread_id*/) {
                for (int unit = unit_start; unit < unit_end && tile_ok->load(std::memory_order_relaxed); ++unit) {
                    MoESmallDecodeDownUnit(options, req, group[static_cast<size_t>(unit / down_splits)], unit % down_splits,
    ...
    req.pool->ParallelFor(req.total_assignments * down_splits, [&](int unit_start, int unit_end, int /*thread_id*/) {
        for (int unit = unit_start; unit < unit_end && tile_ok->load(std::memory_order_relaxed); ++unit) {
            ...
            MoESmallDecodeDownUnit(options, req, assignment, split, down_splits, assignment_hiddens, assignment_outputs,
...
bool TryExecuteMoESmallDecodeQuantizedTileParallel(const CpuExecutionOptions& options, const MoESmallDecodeTileParallelRequest& req) {
    ...
    RunMoESmallDecodeTileGateUp(options, req, gate_splits, assignment_hiddens, shared_decode_input_projection_cache_ptr,
    ...
    if (tile_ok.load(std::memory_order_relaxed)) {
        RunMoESmallDecodeTileDown(options, req, down_splits, assignment_hiddens, assignment_outputs, down_input_projection_caches_ptr,
                                  &tile_ok, &node_partition);
    }
    if (!tile_ok.load(std::memory_order_relaxed)) {
        return false;
