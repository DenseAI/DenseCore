...
bool TryRunGgmlQuantizedProjection(const CpuExecutionOptions& options, CpuBackend* backend, const void* weight_ptr, int ggml_type_id, const Tensor& input,
    ...
    const bool dispatch_census_enabled = []() {
        const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_MATMUL_DISPATCH_CENSUS");
    ...
    const auto record_dispatch = [&](const char* path) {
        ...
        const uint64_t wall_ns = static_cast<uint64_t>(
            std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now() - dispatch_begin)
                .count());
        RecordMatmulDispatchCensus(census_ctx, options.EffectivePhase(), path, wtype, input.shape[0], N, K, wall_ns);
    };
    ...
    if (enable_q4k_repacked_projection_prefill && q4k_prefill_repacked_eligible && prefer_q4k_repacked_prefill) {
        ...
        if (packed && RunQ4KRepackedMoEGemv(options, backend, packed, qinput_data, iq_row_bytes, out_data, M, N, numa_node,
                                            allow_parallel)) {
    ...
    if (CanUseMoEQ4KRawBatchedScalar() && wtype == GGML_TYPE_Q4_K && iq_type == GGML_TYPE_Q8_K && M > 1 &&
        K % ggml_blck_size(GGML_TYPE_Q4_K) == 0) {
        if (RunMoEQ4KRawBatchedProjectionImpl(options, backend, weight_ptr, qinput_data, iq_row_bytes, out_data, M, N, K,
...
bool TryRunGgmlQuantizedFusedGEGLUProjection(const CpuExecutionOptions& options, CpuBackend* backend, const void* gate_weight_ptr, int gate_ggml_type_id,
    ...
    if (CanUseMoEQ4KRawBatchedScalar() && gate_type == GGML_TYPE_Q4_K && up_type == GGML_TYPE_Q4_K &&
        ...
        if (RunMoEQ4KRawBatchedFusedGEGLU(options, backend, gate_weight_ptr, up_weight_ptr, qinput_data, iq_row_bytes, out_data,
                                          M, N, K, numa_node, allow_parallel)) {
...
bool TryRunGgmlQuantizedFusedSwiGLUProjection(const CpuExecutionOptions& options, CpuBackend* backend, const void* gate_weight_ptr, int gate_ggml_type_id,
                                              const void* up_weight_ptr, int up_ggml_type_id, const Tensor& input,
                                              Tensor* output, int64_t N, int64_t K, int numa_node,
                                              bool allow_parallel = true,
                                              QuantizedProjectionInputCache* input_cache = nullptr,
    ...
    if (CanUseMoEQ4KRawBatchedScalar() && gate_type == GGML_TYPE_Q4_K && up_type == GGML_TYPE_Q4_K &&
        iq_type == GGML_TYPE_Q8_K && M > 1 && K % ggml_blck_size(GGML_TYPE_Q4_K) == 0) {
        if (RunMoEQ4KRawBatchedFusedSwiGLUImpl(options, backend, gate_weight_ptr, up_weight_ptr, qinput_data, iq_row_bytes,
                                               out_data, M, N, K, numa_node, allow_parallel)) {
