...
bool EmitMoEProjectionFromPlan(const CpuExecutionOptions& options, const MoEProjectionRequest& req, const MoEProjectionPlan& plan) {
    ...
    if (plan.try_ggml_quantized &&
        TryRunGgmlQuantizedProjection(options,
...
bool RunMoEProjectionFromContext(const CpuExecutionOptions& options, const MoEProjectionRuntimeContext& ctx, char projection_slot, const Tensor& src,
    ...
    return EmitMoEProjectionFromPlan(options, request, plan);
}
