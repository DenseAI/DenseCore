...
bool IsInt4PathDebugEnabled() {
    static const bool enabled = densecore::env::ParseDiagnosticEnv("DENSECORE_DEBUG_INT4_PATHS") ||
                                densecore::env::ParseDiagnosticEnv("DENSECORE_DEBUG_MOE_MATMUL_PATHS");
    return enabled;
}
...
int ResolveEnterpriseThreadPoolNode(int requested_node, int pool_count) {
    ...
    request.abi_version = DENSECORE_ENT_ABI_VERSION;
    request.struct_size = sizeof(DenseCoreEntNumaRoutingRequest);
    ...
    decision.abi_version = DENSECORE_ENT_ABI_VERSION;
    decision.struct_size = sizeof(DenseCoreEntNumaRoutingDecision);
 ...
 */
void CpuBackend::ConfigureThreads(int n_threads) {
    if (n_threads <= 0) {
    ...
    const int numa_count = GetNumaNodeCount();
    for (int i = 0; i < numa_count; ++i) {
        ...
        GetThreadPool(i).Configure(local_threads);
    }
}
...
int GetBackendThreadCount() {
    ...
    int total_threads = 0;
    for (int node = 0; node < numa_count; ++node) {
        total_threads += backend.GetThreadPool(node).GetNumThreads();
    }
...
void CpuBackend::MatMul(const Tensor& A, const Tensor& B, Tensor* C, int numa_node_id) {
    ...
    if (auto* op = GetOpRegistry().GetBest(OpType::MatMul, DeviceType::CPU)) {
        ...
        if (matmul_op) {
            matmul_op->MatMul(A, B, C, GetThreadPool(numa_node_id >= 0 ? numa_node_id : 0));
            return;
        }
    }
...
void CpuBackend::MatMulTransB(const Tensor& A, const Tensor& B, Tensor* C, int numa_node_id) {
    ...
    if (auto* op = GetOpRegistry().GetBest(OpType::MatMulTransB, DeviceType::CPU)) {
        auto* matmul_op = dynamic_cast<MatMulOps*>(op);
        if (matmul_op) {
            matmul_op->MatMulTransB(A, B, C, GetThreadPool(0));
            return;
        }
    }
