...
bool CanUseLFM2QuantizedMoEPrefillBatch(const CpuExecutionOptions& options, const TransformerModel* model, const CpuBackend::ExpertWeights& exp,
    ...
    const CpuExecutionPhase phase = options.EffectivePhase();
    if (phase != CpuExecutionPhase::Prefill && options.HasWorkContext()) {
        return false;
    }
...
struct MoEQuantPrefillAdmissionPlan {
    const CpuExecutionTelemetry* work_ctx = nullptr;
...
MoEQuantPrefillAdmissionPlan ResolveMoEQuantPrefillAdmissionPlan(const CpuExecutionOptions& options, const CpuBackend::ExpertWeights& expert, int64_t batch,
                                                                 int64_t hidden_dim, int64_t intermediate_dim,
                                                                 bool safe_reference_mode, bool enabled) {
    MoEQuantPrefillAdmissionPlan plan;
    plan.work_ctx = options.telemetry;
...
MoEQuantPrefillAdmissionPlan ResolveGemma4MoEPrefillAdmissionPlan(const CpuExecutionOptions& options, const TransformerModel* model,
                                                                  const CpuBackend::ExpertWeights& expert,
                                                                  int64_t batch, int64_t hidden_dim,
                                                                  int64_t intermediate_dim, bool safe_reference_mode) {
    const bool prefill_phase =
        options.EffectivePhase() == CpuExecutionPhase::Prefill || !options.HasWorkContext();
    const bool enabled =
        prefill_phase && model && model->arch_flags.is_gemma4 && IsGemma4MoEPrefillQuantBatchEnabled(model);
    return ResolveMoEQuantPrefillAdmissionPlan(options, expert, batch, hidden_dim, intermediate_dim, safe_reference_mode,
                                               enabled);
}
...
void DispatchExpertFFNImpl(const CpuExecutionOptions& options, CpuBackend* backend, int numa_node, const Tensor& input,
                           ...
                           bool record_gemma4_quant_prefill_batch = false, bool prefer_q4k_repacked_prefill = false,
    ...
    const MoEQuantPrefillAdmissionPlan quant_prefill_plan = ResolveMoEQuantPrefillAdmissionPlan(options,
    ...
    const bool lfm2_q4k_fused_q8_down_candidate =
        ...
        output->ndim == 2 && input.shape[0] == batch && input.shape[1] == hidden_dim && output->shape[0] == batch &&
        output->shape[1] == hidden_dim && !expert.use_gelu_activation && expert.w1.ptr && expert.w2.ptr &&
        expert.w3.ptr && expert.w1_type == GGML_TYPE_Q4_K && expert.w2_type == GGML_TYPE_Q4_K &&
        expert.w3_type == GGML_TYPE_Q4_K && (!expert.w2_scale_tensor || IsScalarScaleSidecar(expert.w2_scale_tensor)) &&
    ...
    if (lfm2_q4k_fused_q8_down_candidate) {
        ...
        if (q8_traits && q8_traits->from_float && qinput_row_bytes > 0 && qhidden_row_bytes > 0) {
            ...
            const bool metadata_hit =
                input_projection_cache->source == input_data && input_projection_cache->rows == batch &&
                input_projection_cache->cols == hidden_dim && input_projection_cache->type == GGML_TYPE_Q8_K &&
                input_projection_cache->row_bytes == qinput_row_bytes;
            const bool external_hit = metadata_hit && input_projection_cache->external_bytes &&
            ...
            if (!cache_hit) {
                ...
                for (int64_t m = 0; m < batch; ++m) {
                    q8_traits->from_float(
                        input_data + static_cast<size_t>(m) * static_cast<size_t>(hidden_dim),
                        input_projection_cache->bytes.data() + static_cast<size_t>(m) * qinput_row_bytes, hidden_dim);
                }
            }
            ...
            const bool w1w3_ok = RunMoEQ4KRawBatchedFusedSwiGLUToQ8Impl(options,
            ...
            const auto w2_begin =
                profile_enabled ? std::chrono::steady_clock::now() : std::chrono::steady_clock::time_point{};
            const bool w2_ok =
                w1w3_ok && RunMoEQ4KRawBatchedProjectionImpl(options, backend, expert.w2.ptr, qhidden_scratch.data(),
                                                             qhidden_row_bytes, output_data, batch, hidden_dim,
                                                             intermediate_dim, numa_node, enable_inner_parallel);
            if (profile_enabled) {
    ...
    fused_gate_up_request.gemma4_quant_prefill_ctx = quant_prefill_plan.work_ctx;
    ...
    const bool used_fused_gate_up = EmitMoEFusedGateUpFromPlan(options, fused_gate_up_request, fused_gate_up_plan);
    ...
    projection_context.gemma4_quant_prefill_ctx = quant_prefill_plan.work_ctx;
    ...
    if (!used_fused_gate_up) {
        const auto w1_begin =
            debug_ffn_timing ? std::chrono::steady_clock::now() : std::chrono::steady_clock::time_point{};
        RunMoEProjectionFromContext(options, projection_context, '1', input, w1, expert.w1_int4, expert.w1, expert.w1_type,
    ...
    if (!used_fused_gate_up && (w3.IsValid() || expert.w3_int4.IsValid() || expert.w3.ptr)) {
        ...
        const auto gate_begin =
            debug_ffn_timing ? std::chrono::steady_clock::now() : std::chrono::steady_clock::time_point{};
        RunMoEProjectionFromContext(options, projection_context, '3', input, w3, expert.w3_int4, expert.w3, expert.w3_type,
    ...
    const auto w2_profile_begin =
        profile_enabled ? std::chrono::steady_clock::now() : std::chrono::steady_clock::time_point{};
    RunMoEProjectionFromContext(options, projection_context, '2', hidden, w2, expert.w2_int4, expert.w2, expert.w2_type,
...
bool RunQ5KRepackedMoEFusedSwiGLURawProjection(const CpuExecutionOptions& options, CpuBackend* backend, const void* gate_weight_ptr,
                                               ...
                                               bool allow_parallel) {
    return RunQ5KRepackedMoEFusedSwiGLURawProjectionImpl(options, backend, gate_weight_ptr, up_weight_ptr, input_data,
                                                         qinput_data, qinput_row_bytes, output_data, rows, cols,
                                                         input_cols, numa_node, allow_parallel);
}

bool RunQ4KRepackedMoEProjection(const CpuExecutionOptions& options, CpuBackend* backend, const void* weight_ptr, const uint8_t* qinput_data,
                                 size_t qinput_row_bytes, float* output_data, int64_t rows, int64_t cols,
                                 int64_t input_cols, int numa_node, bool allow_parallel) {
    const CpuExecutionTelemetry* census_ctx = options.telemetry;
    ...
    if (!RunQ4KRepackedMoEGemv(options, backend, packed, qinput_data, qinput_row_bytes, output_data, rows, cols, numa_node,
                               allow_parallel)) {
...
bool RunQ6KRepackedMoEProjectionCached(CpuBackend* backend, const void* weight_ptr, const uint8_t* qinput_data,
    ...
    std::shared_ptr<Q6KRepackedMoEWeight> packed;
    if (packed_cache && *packed_cache) {
        packed = std::static_pointer_cast<Q6KRepackedMoEWeight>(*packed_cache);
        if (!packed || packed->weight_ptr != weight_ptr || packed->rows != cols || packed->cols != input_cols) {
            packed.reset();
            packed_cache->reset();
        }
    }
    if (!packed) {
        packed = GetOrCreateQ6KRepackedMoEWeight(weight_ptr, cols, input_cols);
        if (!packed) {
            return false;
        }
        if (packed_cache) {
...
bool RunQ4KRepackedMoEFusedSwiGLUProjection(const CpuExecutionOptions& options, CpuBackend* backend, const void* gate_weight_ptr, const void* up_weight_ptr,
...
bool RunQ4KPrepackedMoEFusedSwiGLUTileRange(const void* fused_weight_ptr, const uint8_t* qinput_data,
                                            ...
                                            int tile_end) {
    if (!fused_weight_ptr || !qinput_data || !output_data || cols <= 0 || input_cols <= 0 || (cols % 8) != 0 ||
        (input_cols % QK_K) != 0) {
        return false;
    }
    ...
    for (int tile = tile_start; tile < tile_end; ++tile) {
        ...
        const void* gate_vx =
            gate_blocks + static_cast<size_t>(tile) * static_cast<size_t>(blocks_per_row) * sizeof(MoEQ4Kx8Block);
        const void* up_vx =
            up_blocks + static_cast<size_t>(tile) * static_cast<size_t>(blocks_per_row) * sizeof(MoEQ4Kx8Block);
...
bool RunMoEQ4KRawBatchedProjection(const CpuExecutionOptions& options, CpuBackend* backend, const void* weight_ptr, const uint8_t* qinput_data,
                                   size_t qinput_row_bytes, float* out_data, int64_t M, int64_t N, int64_t K,
                                   int numa_node, bool allow_parallel) {
    return RunMoEQ4KRawBatchedProjectionImpl(options, backend, weight_ptr, qinput_data, qinput_row_bytes, out_data, M, N, K,
                                             numa_node, allow_parallel);
}

bool internal::RunQ4KRawBatchedQuantizedProjection(const CpuExecutionOptions& options, CpuBackend* backend, const void* weight_data,
                                                   const uint8_t* quantized_input, size_t quantized_input_row_bytes,
                                                   float* output_data, int64_t rows, int64_t output_cols,
                                                   int64_t input_cols, int numa_node, bool allow_parallel) {
    return RunMoEQ4KRawBatchedProjectionImpl(options, backend, weight_data, quantized_input, quantized_input_row_bytes,
                                             output_data, rows, output_cols, input_cols, numa_node, allow_parallel,
                                             /*record_metrics=*/false);
}

bool RunMoEQ4KRawBatchedWeightedScatterProjection(const CpuExecutionOptions& options, CpuBackend* backend, const void* weight_ptr,
    ...
    return RunMoEQ4KRawBatchedWeightedScatterProjectionImpl(options, backend, weight_ptr, qinput_data, qinput_row_bytes,
                                                            token_indices, token_weights, output_data,
                                                            output_row_stride, M, N, K, numa_node, allow_parallel);
}

bool RunMoEKQuantRawBatchedProjection(const CpuExecutionOptions& options, CpuBackend* backend, int ggml_type_id, const void* weight_ptr,
    ...
    return RunMoEKQuantRawBatchedProjectionImpl(options, backend, static_cast<ggml_type>(ggml_type_id), weight_ptr, qinput_data,
                                                qinput_row_bytes, out_data, M, N, K, numa_node, allow_parallel);
}

bool RunMoEKQuantRawBatchedFusedSwiGLU(const CpuExecutionOptions& options, CpuBackend* backend, int ggml_type_id, const void* gate_weight_ptr,
    ...
    if (static_cast<ggml_type>(ggml_type_id) == GGML_TYPE_Q4_K) {
        return RunMoEQ4KRawBatchedFusedSwiGLUImpl(options, backend, gate_weight_ptr, up_weight_ptr, qinput_data,
                                                  qinput_row_bytes, out_data, M, N, K, numa_node, allow_parallel);
    }
    return RunMoEKQuantRawBatchedFusedSwiGLUImpl(options, backend, static_cast<ggml_type>(ggml_type_id), gate_weight_ptr,
                                                 up_weight_ptr, qinput_data, qinput_row_bytes, out_data, M, N, K,
                                                 numa_node, allow_parallel);
}

bool RunMoEQ4KRawBatchedFusedSwiGLUToQ8(const CpuExecutionOptions& options, CpuBackend* backend, const void* gate_weight_ptr, const void* up_weight_ptr,
                                        ...
                                        bool allow_parallel) {
    return RunMoEQ4KRawBatchedFusedSwiGLUToQ8Impl(options, backend, gate_weight_ptr, up_weight_ptr, qinput_data,
                                                  qinput_row_bytes, qoutput_data, qoutput_row_bytes, M, N, K, numa_node,
                                                  allow_parallel);
}

namespace testing {
bool RunMoEQ4KQ8KBatchedRowPairParityForTest(const CpuExecutionOptions& options, const void* gate_weight_row, const void* up_weight_row,
    ...
    const bool pair_ok = ComputeMoEQ4KQ8KBatchedRowPair(options, gate_weight_row, up_weight_row, qinput_data, qinput_row_bytes,
                                                        M, K, gate_pair.data(), up_pair.data(), &specialized);
    const bool reference_ok =
        ComputeMoEQ4KQ8KBatchedRow(options, gate_weight_row, qinput_data, qinput_row_bytes, M, K, gate_reference.data()) &&
        ComputeMoEQ4KQ8KBatchedRow(options, up_weight_row, qinput_data, qinput_row_bytes, M, K, up_reference.data());
    if (specialized_pair_used) *specialized_pair_used = specialized;
    if (!pair_ok || !reference_ok) return false;
    for (int m = 0; m < M; ++m) {
        if (std::fabs(gate_pair[m] - gate_reference[m]) > 1e-4f || std::fabs(up_pair[m] - up_reference[m]) > 1e-4f) {
            return false;
        }
    }
    return true;
...
void CpuBackend::DispatchExpertFFN(const CpuExecutionOptions& options, const TransformerLayer* layer_key, int expert_id, const Tensor& input,
    ...
    DispatchExpertFFNImpl(options, this, numa_node, input, expert, w1, w2, w3, output);
}
...
void CpuBackend::ForwardMoE(const CpuExecutionOptions& options, const TransformerModel* model, const TransformerLayer* layer_key, int layer_idx,
                            const CpuBatchTraceView* batch, const Tensor& input, const moe::MoERouteResult& routing,
    ...
    if (!small_decode_ready) {
        const char* reject_reason = !small_decode_candidate              ? "not_small_decode"
                                    ...
                                    : small_step_max_expert_batch > 1    ? "expert_batch_gt_1"
                                                                         : "no_active_experts";
        RecordMoESmallDecodeParallelDecision(options.telemetry, small_decode_candidate, false, reject_reason,
    ...
    if (small_decode_ready) {
        ...
        RecordMoESmallDecodeParallelDecision(
            options.telemetry, true, use_small_decode_expert_parallel,
        ...
        if (use_small_decode_quantized_tile_parallel) {
            ...
            if (TryExecuteMoESmallDecodeQuantizedTileParallel(options, tile_request)) {
                return;
            }
        }
        if (use_small_decode_expert_parallel) {
            ...
            // One pre-quantized hidden row per token in the batch. The expert loop
            // below fans out `batch_size * top_k` assignments, and every assignment
            // for the same token quantizes the identical hidden state; at
            // batch_size == 1 that redundancy was already hoisted out, but a
            // concurrent decode batch used to re-quantize per assignment, so the
            // fixed per-assignment cost scaled with batch width instead of staying
            // flat. Caches are filled here and only read inside ParallelFor (the
            // projection helper mutates a cache only on a metadata miss, and these
            // are pre-populated to hit), so sharing them across workers is safe.
            std::vector<QuantizedProjectionInputCache> shared_decode_input_projection_caches;
            const bool shared_decode_input_projection_cache_ready = [&]() {
                const auto* q8_traits = ggml_get_type_traits_cpu(GGML_TYPE_Q8_K);
                const size_t q8_row_bytes = ggml_row_size(GGML_TYPE_Q8_K, hidden_dim);
                if (!q8_traits || !q8_traits->from_float || q8_row_bytes == 0 || batch_size <= 0) {
                    return false;
                }
                shared_decode_input_projection_caches.resize(static_cast<size_t>(batch_size));
                for (int token = 0; token < batch_size; ++token) {
                    const float* token_input = input_data + static_cast<size_t>(token) * hidden_dim;
                    auto& cache = shared_decode_input_projection_caches[static_cast<size_t>(token)];
                    cache.source = token_input;
                    cache.rows = 1;
                    cache.cols = hidden_dim;
                    cache.type = GGML_TYPE_Q8_K;
                    cache.row_bytes = q8_row_bytes;
                    cache.bytes.resize(q8_row_bytes);
                    q8_traits->from_float(token_input, cache.bytes.data(), hidden_dim);
                }
                return true;
            }();
            small_decode_pool.ParallelFor(effective_small_decode_workers, [&](int worker_start, int worker_end,
                ...
                for (int worker = worker_start; worker < worker_end; ++worker) {
                    ...
                    for (int i = assignment_start; i < assignment_end; ++i) {
                        ...
                        QuantizedProjectionInputCache* shared_token_input_projection_cache =
                            shared_decode_input_projection_cache_ready
                                ? &shared_decode_input_projection_caches[static_cast<size_t>(token_idx)]
                                : nullptr;
                        QuantizedProjectionInputCache* decode_input_projection_cache =
                            (shared_token_input_projection_cache &&
                             CanUseSharedDecodeInputCacheForExpert(exp, shared_token_input_projection_cache->type))
                                ? shared_token_input_projection_cache
                        ...
                        DispatchExpertFFNImpl(options, this, expert_numa_node, expert_input, exp, Tensor(), Tensor(), Tensor(),
        ...
        for (int i = 0; i < total_assignments; ++i) {
            ...
            DispatchExpertFFNImpl(options, this, expert_numa_node, expert_input, exp, w1, w2, w3, &expert_out, true, &trace_ctx,
    ...
    auto run_active_work_range = [&](int start_idx, int end_idx, bool allow_inner_parallel) {
        ...
        for (int raw_idx = start_idx; raw_idx < end_idx; ++raw_idx) {
            ...
            const MoEQuantPrefillAdmissionPlan quant_prefill_plan =
                ResolveGemma4MoEPrefillAdmissionPlan(options, model, exp, work.count, static_cast<int64_t>(hidden_dim),
                                                     static_cast<int64_t>(exp.intermediate_dim), safe_reference_mode);
            const bool lfm2_quant_prefill_batch_safe =
                CanUseLFM2QuantizedMoEPrefillBatch(options, model, exp, work.count, static_cast<int64_t>(hidden_dim),
                                                   static_cast<int64_t>(exp.intermediate_dim), safe_reference_mode);
            ...
            if (quant_prefill_plan.enabled && work.count > 1) {
                RecordGemma4MoEPrefillQuantBatchDecision(
                    ...
                    quant_prefill_plan.batch_safe ? nullptr : quant_prefill_plan.reject_reason, false, false);
            }
            const bool should_try_cache =
            ...
            DispatchExpertFFNImpl(options, this, work.numa_node, expert_input, exp, w1, w2, w3, &expert_out,
                                  allow_inner_parallel, &trace_ctx, nullptr, profile, quantized_prefill_batch_safe,
                                  quant_prefill_plan.enabled, quant_prefill_plan.batch_safe,
...
bool RunGgmlQuantizedProjectionForTest(const CpuExecutionOptions& options, CpuBackend* backend, const void* weight_ptr, int ggml_type_id,
                                       const Tensor& input, Tensor* output, int64_t N, int64_t K) {
    return TryRunGgmlQuantizedProjection(options, backend, weight_ptr, ggml_type_id, input, output, N, K, /*numa_node=*/0,
                                         /*allow_parallel=*/false, nullptr);
}

bool RunGgmlQuantizedFusedSwiGLUProjectionForTest(const CpuExecutionOptions& options, CpuBackend* backend, const void* gate_weight_ptr,
    ...
    return TryRunGgmlQuantizedFusedSwiGLUProjection(options, backend, gate_weight_ptr, gate_ggml_type_id, up_weight_ptr,
                                                    up_ggml_type_id, input, output, N, K, /*numa_node=*/0,
                                                    /*allow_parallel=*/false, input_cache);
}

bool RunGgmlQuantizedFusedGEGLUProjectionForTest(const CpuExecutionOptions& options, CpuBackend* backend, const void* gate_weight_ptr,
                                                 int gate_ggml_type_id, const void* up_weight_ptr, int up_ggml_type_id,
                                                 const Tensor& input, Tensor* output, int64_t N, int64_t K) {
    return TryRunGgmlQuantizedFusedGEGLUProjection(options, backend, gate_weight_ptr, gate_ggml_type_id, up_weight_ptr,
                                                   up_ggml_type_id, input, output, N, K, /*numa_node=*/0,
                                                   /*allow_parallel=*/false, nullptr);
}
