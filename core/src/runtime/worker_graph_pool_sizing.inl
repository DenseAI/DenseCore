...
bool GraphPoolReplacementFits(size_t target_bytes, size_t safety_margin_bytes, size_t available_bytes) {
    if (available_bytes == 0) {
        return true;
    }
    return target_bytes <= available_bytes && safety_margin_bytes <= available_bytes - target_bytes;
}

size_t ClampGraphPoolToExplicitMax(size_t requested_bytes, size_t required_floor_bytes) {
    const char* value = std::getenv("DENSECORE_GRAPH_CTX_MAX_MB");
    if (!value || value[0] == '\0') {
        return requested_bytes;
    }
    errno = 0;
    char* end = nullptr;
    const unsigned long long parsed_mb = std::strtoull(value, &end, 10);
    if (errno != 0 || end == value || *end != '\0' || parsed_mb < 128) {
        return requested_bytes;
    }
    constexpr size_t MB = 1024ULL * 1024ULL;
    const size_t max_mb = static_cast<size_t>(std::min<unsigned long long>(
        parsed_mb, std::numeric_limits<size_t>::max() / MB));
    const size_t explicit_max_bytes = max_mb * MB;
    return std::min(requested_bytes, std::max(required_floor_bytes, explicit_max_bytes));
}
...
size_t ResolveGraphPoolReservationBytes(size_t required_payload_bytes, size_t current_pool_bytes,
                                        bool allow_growth_reserve) {
    // Current capacity already includes GGML metadata. Never feed it back as
    // payload: that adds metadata and speculative slack on every reuse.
    const size_t required_total_bytes = IncludeGgmlGraphObjectMetadata(required_payload_bytes);
    if (!allow_growth_reserve || current_pool_bytes == 0 || required_total_bytes <= current_pool_bytes) {
        return required_total_bytes;
    }
    constexpr size_t MB = 1024ULL * 1024ULL;
    const size_t grow_ahead = std::max<size_t>(current_pool_bytes / 2, 2048ULL * MB);
    return std::max(required_total_bytes, AlignUpBytes(current_pool_bytes + grow_ahead, 512ULL * MB));
}
...
FlexibleGraphPoolSizing MeasureFlexibleGraphPoolSize(TransformerModel* model, PagedKVCache* cache,
    ...
    const size_t dry_mb = std::clamp<size_t>(default_dry_mb, 512, auto_dry_cap_mb);
    ...
    try {
        ...
        ScopedBatchWorkContext dry_scope(dry_work_ctx.get(), &batch, dry_phase,
                                         /*reset_context=*/true, model ? model->variant : ModelVariant::UNKNOWN,
