...
Scheduler::Scheduler(BlockManager* block_manager, const SchedulerConfig& config)
    ...
    decode_homogeneous_batch_n_past_ = config_.decode_homogeneous_batch_n_past;
    ...
    config_.max_parallel_prefill_chunk_tokens =
        std::min(config_.max_parallel_prefill_chunk_tokens, config_.max_prefill_tokens);
}
...
SchedulerOutput Scheduler::Schedule() {
    ...
    if (config_.isolate_prefill_decode) {
        ...
        if (has_waiting) {
            ...
            } else if (config_.fill_slots_before_first_decode &&
                       running_seqs_.size() < static_cast<size_t>(config_.max_num_seqs) &&
                       std::all_of(waiting_queue_.begin(), waiting_queue_.end(), [&](const SequenceGroup& group) {
                           return group.num_tokens_to_process <= config_.max_prefill_tokens &&
                                  (group.max_prefill_chunk_tokens <= 0 ||
                                   group.num_tokens_to_process <= group.max_prefill_chunk_tokens);
                       }) &&
                       std::all_of(running_seqs_.begin(), running_seqs_.end(), [&](int seq_id) {
                           const auto it = seq_generated_tokens_.find(seq_id);
...
void Scheduler::PreemptSequences(SchedulerOutput& output) {
    if (GetMemoryUsage() <= config_.watermark_low || running_seqs_.empty()) {
        return;
    }

    // Scheduler preemption only marks victims; the worker frees the physical
    // blocks later in the same iteration. Do not loop on the unchanged
    // BlockManager usage here or a single pressure event can drain the entire
    // running set before any blocks are actually released.
    const int victim = SelectPreemptionVictim();
    if (victim < 0) {
        return;
    }
