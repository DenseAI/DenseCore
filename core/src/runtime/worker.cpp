...
namespace {
using densecore::llm::graph::ScopedGgmlGraphArena;
...
LFM2GreedyLMHeadArgmaxRejectReason
GetGreedyLMHeadArgmaxRejectReason(const TransformerModel* model, const std::vector<Request*>& batch_requests,
                                  InferenceExecutionPhase phase, bool is_embedding_batch, float final_logit_softcap) {
    ...
    if (densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_SAMPLE") != nullptr ||
        densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_SAMPLE_TOP") != nullptr ||
        densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_SAMPLER_TRACE") != nullptr) {
...
public:
    ScopedBatchWorkContext(InferenceWorkContext* ctx, const BatchSpec* batch, InferenceExecutionPhase phase,
                           bool reset_context, ModelVariant model_variant,
...
bool IsHybridSSMPrefillGraphCacheDebugProbeEnabled() {
    static const char* const kEnvNames[] = {
        "DENSECORE_DEBUG_SSM_QKV_REFERENCE",
        "DENSECORE_DEBUG_SSM_PROJECTION_REFERENCE",
        "DENSECORE_DEBUG_HIDDEN_SNAPSHOT",
        "DENSECORE_DEBUG_ATTN_CORE_REFERENCE",
        "DENSECORE_DEBUG_ATTN_POST_REFERENCE",
        "DENSECORE_DEBUG_FFN_PROJECTION_REFERENCE",
        "DENSECORE_DEBUG_RMSNORM_REFERENCE",
        "DENSECORE_DEBUG_ADD_RMSNORM_REFERENCE",
        "DENSECORE_DEBUG_SHARED_SCALAR_GATE_REFERENCE",
    };
...
bool IsPrefillGraphCacheSafeForCurrentBatch(const TransformerModel* model, const BatchSpec& batch) {
    if (!model) {
        return false;
    }
    ...
    return !batch.skip_output_logits && batch.num_seqs == 1 && batch.hybrid_ssm_runtime_states.size() == 1 &&
           batch.hybrid_ssm_runtime_states[0] != nullptr && !batch.seq_id.empty();
}
...
// Watchdog on a scheduler that keeps returning empty batches while work is
// queued: warn, then fail the request rather than hang.
...
#include "runtime/worker_qwen36_prefill_alias_plan.inl"
...
bool ResolveSamplingLogitsColumnForRequestImpl(int token_offset, int processed_count, int output_columns,
                                               ...
                                               std::string* error, int sequence_index, int num_sequences) {
    ...
    // The graph gathers one final-token column per sequence for mixed batches.
    // Decode requests share that layout with prefill requests; token offsets no
    // longer address the output once earlier prompt columns have been removed.
    if (num_sequences > 1 && output_columns == num_sequences) {
        if (sequence_index < 0 || sequence_index >= num_sequences) {
            return fail("sampling sequence index is out of bounds for compact output tensor");
        }
        *out_last_token_idx = sequence_index;
        return true;
    }
...
#include "runtime/worker_diagnostics.inl"
...
bool ResolveSamplingLogitsColumnForRequest(int token_offset, int processed_count, int output_columns,
                                           ...
                                           int sequence_index, int num_sequences) {
    return ResolveSamplingLogitsColumnForRequestImpl(token_offset, processed_count, output_columns,
                                                     ...
                                                     n_past_after, out_last_token_idx, error, sequence_index, num_sequences);
}

int GetDebugSamplingLogitsOffset() {
    static const int value = []() {
        const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_SAMPLING_LOGITS_OFFSET");
        if (!env || env[0] == '\0') {
            return 0;
        }
        char* end = nullptr;
...
int GetDebugFirstTokenSamplingLogitsOffset() {
    static const int value = []() {
        const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_FIRST_TOKEN_SAMPLING_LOGITS_OFFSET");
        if (!env || env[0] == '\0') {
            return 0;
        }
        char* end = nullptr;
...
bool IsPrefixCacheAllowedForModelForTest(const TransformerModel* model) {
    return IsPrefixCacheAllowedForModel(model);
}

const char* PrefixCacheSkipReasonForModelForTest(const TransformerModel* model) {
    return PrefixCacheSkipReasonForModel(model);
}
...
size_t ResolveGraphPoolReservationBytesForTest(size_t required_payload_bytes, size_t current_pool_bytes,
    ...
    return ResolveGraphPoolReservationBytes(required_payload_bytes, current_pool_bytes, allow_growth_reserve);
}

bool GraphPoolReplacementFitsForTest(size_t target_bytes, size_t safety_margin_bytes, size_t available_bytes) {
    return GraphPoolReplacementFits(target_bytes, safety_margin_bytes, available_bytes);
}

size_t ClampGraphPoolToExplicitMaxForTest(size_t requested_bytes, size_t required_floor_bytes) {
    return ClampGraphPoolToExplicitMax(requested_bytes, required_floor_bytes);
}

#ifdef DENSECORE_TEST_BUILD
namespace {
struct PendingDequeuePauseHookState {
    std::mutex mu;
...
struct PendingPublishPauseHookState {
    std::mutex mu;
    std::condition_variable cv;
    bool pause_enabled = false;
    bool submitter_waiting = false;
    bool release_requested = false;
};

struct SchedulerAdmissionPauseHookState {
    std::mutex mu;
    std::condition_variable cv;
    bool pause_enabled = false;
    bool worker_waiting = false;
...
PendingDequeuePauseHookState& PendingDequeuePauseHook() {
    static PendingDequeuePauseHookState hook;
    return hook;
}

PendingPublishPauseHookState& PendingPublishPauseHook() {
    static PendingPublishPauseHookState hook;
    return hook;
}

SchedulerAdmissionPauseHookState& SchedulerAdmissionPauseHook() {
    static SchedulerAdmissionPauseHookState hook;
    return hook;
}

void MaybePauseBeforePendingDequeueForTest() {
    auto& hook = PendingDequeuePauseHook();
    std::unique_lock<std::mutex> lock(hook.mu);
    if (!hook.pause_enabled) {
        return;
    }
    hook.worker_waiting = true;
    hook.cv.notify_all();
    hook.cv.wait(lock, [&hook]() { return !hook.pause_enabled || hook.release_requested; });
    hook.worker_waiting = false;
    hook.release_requested = false;
    hook.cv.notify_all();
}
}  // namespace
...
void MaybePauseAfterPendingPublishForTest() {
    auto& hook = PendingPublishPauseHook();
    std::unique_lock<std::mutex> lock(hook.mu);
    if (!hook.pause_enabled) {
        return;
    }
    hook.submitter_waiting = true;
    hook.cv.notify_all();
    hook.cv.wait(lock, [&hook]() { return !hook.pause_enabled || hook.release_requested; });
    hook.submitter_waiting = false;
    hook.release_requested = false;
    hook.cv.notify_all();
}
...
void EnablePauseAfterPendingPublishForTest(bool enabled) {
    ...
    hook.pause_enabled = enabled;
    hook.submitter_waiting = false;
    hook.release_requested = !enabled;
    hook.cv.notify_all();
}
...
bool WaitForPauseAfterPendingPublishForTest(int timeout_ms) {
    auto& hook = PendingPublishPauseHook();
    std::unique_lock<std::mutex> lock(hook.mu);
    return hook.cv.wait_for(lock, std::chrono::milliseconds(timeout_ms), [&hook]() { return hook.submitter_waiting; });
}

void ReleasePauseAfterPendingPublishForTest() {
    auto& hook = PendingPublishPauseHook();
    std::lock_guard<std::mutex> lock(hook.mu);
    hook.pause_enabled = false;
    hook.release_requested = true;
    hook.cv.notify_all();
}

void MaybePauseAfterSchedulerAdmissionForTest() {
    auto& hook = SchedulerAdmissionPauseHook();
    std::unique_lock<std::mutex> lock(hook.mu);
    if (!hook.pause_enabled) {
        return;
    }
...
bool WaitForPauseAfterSchedulerAdmissionForTest(int timeout_ms) {
    auto& hook = SchedulerAdmissionPauseHook();
    std::unique_lock<std::mutex> lock(hook.mu);
    return hook.cv.wait_for(lock, std::chrono::milliseconds(timeout_ms), [&hook]() { return hook.worker_waiting; });
}
...
void EngineLoop(EngineState* state) {
    ...
    try {
        ...
        if (!state->backend_registry) {
            state->InitializeExecutionDependencies(state->n_threads);
        }
        ...
        // Match the effective policy reported to language adapters. Safety is
        // enforced here even for callers that submit directly through the C API.
        DenseCoreRuntimeOptimizationState submission_policy{};
        const bool exclusive_execution =
            DenseCoreGetRuntimeOptimizationState(state, &submission_policy) == DENSECORE_STATUS_OK &&
            submission_policy.prefix_cache_reuse_enabled && submission_policy.hybrid_ssm_snapshot_restore_enabled;
        ...
        auto free_request_blocks = [&](Request* req) {
            if (req && current_kv_cache && !req->block_table.empty()) {
                current_kv_cache->block_manager->Free(req->block_table);
                req->block_table.clear();
            }
        };
        auto remove_request_from_scheduler = [&](Request* req, bool completed) {
            if (req && req->seq_id >= 0) {
                state->scheduler->RemoveRequest(req->seq_id, completed);
                seq_to_request.erase(req->seq_id);
            }
        };
        auto finalize_generation_error = [&](Request* req, DecodeFinishCause finish_cause,
                                             const std::string& error_text, bool use_direct_callback,
                                             const densecore::TransformerGraphExecutionPlan* graph_plan = nullptr) {
            if (!req || req->is_graph_execution) {
                return false;
            }
            if (req->is_embedding) {
                const int status = finish_cause == DecodeFinishCause::RequestCanceled
                                       ? DENSECORE_STATUS_ENGINE_STOPPED
                                       : DENSECORE_STATUS_INTERNAL_ERROR;
                if (!FinalizeEmbeddingRequestOnce(state, req, {}, status, use_direct_callback)) {
                    return false;
                }
                state->metrics.failed_requests++;
                return true;
            }
            req->decode_finish_cause = finish_cause;
            FinalizeDecodeSilentFinishReason(req);
            LogRequestDecodeSummary(req, current_model, graph_plan);
            if (!FinalizeGenerationRequestOnce(state, req, error_text, -1, true, use_direct_callback)) {
                return false;
            }
            state->metrics.failed_requests++;
            return true;
        };
        auto finalize_generation_success = [&](Request* req, bool use_direct_callback,
                                               const densecore::TransformerGraphExecutionPlan* /*graph_plan*/ = nullptr) {
            if (!req || req->is_graph_execution || req->is_embedding) {
                return false;
            }
            FinalizeDecodeSilentFinishReason(req);
            if (!FinalizeGenerationRequestOnce(state, req, "", -1, false, use_direct_callback)) {
                return false;
            }
        ...
        while (state->status != EngineStatus::STOPPED) {
...

            {
                ...
                while (state->status == EngineStatus::RUNNING) {
                    Request* req = state->PopReadyPendingRequest(exclusive_execution);
                    if (!req) break;  // Queue empty
#ifdef DENSECORE_TEST_BUILD
                    MaybePauseBeforePendingDequeueForTest();
#endif

                    state->RemovePendingRequest(req->id);
                    LogRequestLifecyclePhase("engine_queue_pop", req, current_model);

                    if (state->status.load(std::memory_order_acquire) != EngineStatus::RUNNING) {
                        ...
                        if (req->is_graph_execution) {
                            ...
                            finalize_generation_error(req, DecodeFinishCause::RequestCanceled,
                                                      "Error: Engine shutdown", global_direct_callback);
                        }
                        state->request_pool.Release(req);
                        continue;
                    }
                    ...
                    if (req->cancelled.load(std::memory_order_relaxed)) {
                        ...
                        if (req->is_graph_execution) {
                            ...
                            state->metrics.failed_requests++;
                            CompleteGraphRequest(req, nullptr, DENSECORE_STATUS_ENGINE_STOPPED);
                        } else if (finalize_generation_error(req, DecodeFinishCause::RequestCanceled,
                                                             "Error: request canceled", global_direct_callback)) {
                            // Request finalized before scheduler admission.
                        }
                        state->request_pool.Release(req);
                        continue;
                    }
                    ...
                    if (req->tokens.empty()) {
                        ...
                        finalize_generation_error(req, DecodeFinishCause::MissingTokens, "Error: Missing tokens",
                    ...
                    if (seq_id < 0) {
                        ...
                        finalize_generation_error(
                            req, DecodeFinishCause::SchedulerRejected,
...
#ifdef DENSECORE_TEST_BUILD
                    densecore::testing::MaybePauseAfterSchedulerAdmissionForTest();
#endif
                    req->empty_schedule_stall_count = 0;
            ...
            if (state->pending_requests.Empty()) {
                ...
                if (scheduler_single_tenant && single_req && !single_req->finished && !single_req->is_embedding &&
                    ...
                    if (prefix_probe.cached_tokens > 0 && !prefix_probe.cached_block_ids.empty()) {
                        ...
                        const int requested_tokens =
                            single_req->is_prefill ? static_cast<int>(single_req->tokens.size()) : 1;
                        const int tokens_to_process = single_req->is_prefill
                                                          ? ResolveRecurrentPrefixSnapshotPrefillTokens(
                                                                current_model, single_req->prefix_cache_allowed,
                                                                single_req->n_past, requested_tokens)
                                                          : requested_tokens;
            ...
            if (!used_single_request_fast_path) {
                for (int seq_id : sched_output.swap_in_seq_ids) {
                    ...
                    if ((int)req->block_table.size() != req->swap_state.num_blocks) {
                        ...
                        finalize_generation_error(req, DecodeFinishCause::OutOfMemory, "Error: Swap-in failed",
                                                  global_direct_callback);
                        if (req->seq_id >= 0) {
            ...
            deps.config = &state->inference_config;
            deps.hardware_topology = &densecore::HardwareTopology::GetInstance();
            deps.backend_registry = state->backend_registry.get();
            deps.cpu_backend = static_cast<densecore::CpuBackend*>(
                ...
                if (batch_build_stats.scheduled_total() > 0) {
                    ...
                    if (should_fail) {
                        for (Request* req : batch_build_stats.stalled_requests) {
                            ...
                            finalize_generation_error(
                                req, DecodeFinishCause::BatchBuildStall,
                                "Error: Scheduler scheduled work but batch construction stalled",
                                global_direct_callback);
                            free_request_blocks(req);
                            remove_request_from_scheduler(req, /*completed=*/false);
                        }
            ...
            const auto fail_batch_requests = [&](const std::string& fail_reason, const std::string& error_message,
                                                 DecodeFinishCause finish_cause, bool count_as_oom) {
                for (Request* req : batch_requests) {
                    if (!req || req->finished) {
                        continue;
                    }
                    ...
                    finalize_generation_error(req, finish_cause, error_message, global_direct_callback);
                    free_request_blocks(req);
                    remove_request_from_scheduler(req, /*completed=*/false);
                }
            };
            ...
            auto bind_greedy_lm_head_argmax_sampling = [&](InferenceWorkContext* target_ctx,
                ...
                const float final_logit_softcap = current_model && current_model->arch_flags.is_gemma4 &&
                                                      ...
                                                      : 0.0f;
                const auto lfm2_argmax_reject_reason = GetGreedyLMHeadArgmaxRejectReason(
                    current_model, batch_requests, phase, is_embedding_batch, final_logit_softcap);
                ...
                SetInferenceWorkContextLFM2GreedyLMHeadArgmaxSampling(
                    ...
                    lfm2_argmax_req ? lfm2_argmax_req->sampling_params.repetition_penalty : 1.0f, final_logit_softcap,
                    lfm2_argmax_req ? &lfm2_argmax_req->token_history : nullptr, lfm2_argmax_disallowed_tokens);
            };
            ...
            const bool prefill_intermediate_chunk =
                is_prefill_batch && !is_embedding_batch && !batch_requests.empty() &&
                batch_token_counts.size() == batch_requests.size() &&
                [&]() {
                    for (size_t i = 0; i < batch_requests.size(); ++i) {
                        const Request* req = batch_requests[i];
                        if (!req || !req->is_prefill ||
                            batch_token_counts[i] >= static_cast<int>(req->tokens.size())) {
                            return false;
                        }
                    }
                    return true;
            ...
            InferenceConfig& infer_cfg = state->inference_config;
            ...
            if (is_decode_batch) {
                NoteDecodeBuiltPhysicalBatch(decode_batch_size);
            }
            PrepareWorkerWeightsForExecution(current_model, is_decode_batch, is_prefill_batch,
                                             prefill_prompt_token_count, state->fast_path_config, batch_requests);
            ...
            const int final_thread_cap = ResolveFinalWorkerThreadCap(
                current_model, physical_core_count, base_threads, bench_respect_threads);
            ...
            const auto build_transformer_graph_or_fail =
                [&](struct ggml_context* graph_ctx, struct ggml_cgraph* graph, struct ggml_tensor** graph_output,
                    struct ggml_tensor** graph_embd_inp, struct ggml_tensor** graph_pos) {
                    try {
                        *graph_output = BuildTransformerGraph(current_model, current_kv_cache, graph_ctx, batch,
                                                              is_embedding_batch, graph, graph_embd_inp, graph_pos);
                        return true;
                    } catch (const densecore::DenseCoreException& e) {
                        if (is_decode_batch) {
                            NoteDecodeRejectedPhysicalBatch(decode_batch_size);
                        }
                        const std::string error_message = std::string("Error: graph build failed: ") + e.what();
                        LOG_ERROR("Transformer graph build failed (code={}): {}", e.CodeInt(), e.what());
                        fail_batch_requests("graph_build_failed", error_message, DecodeFinishCause::GraphBuildFailed,
                                            /*count_as_oom=*/false);
                    } catch (const std::exception& e) {
                        if (is_decode_batch) {
                            NoteDecodeRejectedPhysicalBatch(decode_batch_size);
                        ...
                        const std::string error_message = std::string("Error: graph build failed: ") + e.what();
                        LOG_ERROR("Transformer graph build failed: {}", e.what());
                        fail_batch_requests("graph_build_failed", error_message, DecodeFinishCause::GraphBuildFailed,
                                            /*count_as_oom=*/false);
                    }
                    reap_finished_requests();
            ...
            size_t prefill_graph_ctx_bytes = prefill_graph_cache_active_effective ? graph_ctx_estimate.total_bytes : 0;
            if (prefill_graph_ctx_bytes > 0 && is_prefill_batch && current_model &&
            ...
            if (decode_reuse_candidate) {
                ...
                if (DoesDecodeGraphCacheRequireRuntimeRebind(current_model)) {
                    // Reuse the graph across decode steps of the same requests,
                    // but never carry stateful custom-op userdata across request
                    // lifetimes. Runtime pointer rebind alone is insufficient
                    // for Qwen3.6 token parity across consecutive requests.
                    uint64_t identity = 1469598103934665603ull;
                    for (const Request* req : batch_requests) {
                        identity ^= static_cast<uint64_t>(req ? req->id : -1);
                        identity *= 1099511628211ull;
                    }
                    decode_graph_key.stateful_request_identity = identity;
                }
            }
            const bool decode_key_marked_uncacheable =
                decode_reuse_candidate &&
                graph_cache.IsUncacheable(decode_graph_key);
            ...
            if (decode_reuse_attempt_allowed) {
                ...
                auto* cached = graph_cache.FindDecode(decode_graph_key);
                if (cached != nullptr && cached->graph && cached->output && cached->embd_inp &&
                    cached->pos && cached->work_ctx) {
                    bool rebind_ok = true;
                    ResetCachedDecodeGraphWorkContext(cached->work_ctx.get());
                    bind_greedy_lm_head_argmax_sampling(cached->work_ctx.get(), InferenceExecutionPhase::Decode);
                    if (DoesDecodeGraphCacheRequireRuntimeRebind(current_model)) {
                        const auto rebind_begin = std::chrono::steady_clock::now();
                        rebind_ok = RebindDecodeGraphRuntimeStateForModel(current_model, cached->graph, batch);
                        ...
                        if (!rebind_ok) {
                            ...
                            graph_cache.EraseDecode(decode_graph_key);
                        }
                    }
                    if (!rebind_ok) {
                        // Fall back to rebuilding below.
                    } else {
                        graph_cache.Touch(cached);
                        gf = cached->graph;
                        output = cached->output;
                        embd_inp = cached->embd_inp;
                        pos = cached->pos;
                        graph_execution_work_ctx = cached->work_ctx.get();
                        cached_graph_verified_paged_decode_op = cached->verified_paged_decode_op;
            ...
            if (!reused_decode_graph && prefill_reuse_candidate) {
                auto* cached = graph_cache.FindPrefill(prefill_graph_key);
                if (cached != nullptr && cached->graph && cached->output && cached->embd_inp &&
                    cached->pos && cached->work_ctx) {
                    bool rebind_ok = true;
                    ResetCachedPrefillGraphWorkContext(cached->work_ctx.get());
                    SetCurrentWorkContext(cached->work_ctx.get());
                    SetCurrentExecutionPhase(InferenceExecutionPhase::Prefill);
                    SetCurrentBatch(&batch);
                    if (current_model && current_model->arch_flags.is_hybrid_ssm) {
                        const auto rebind_begin = std::chrono::steady_clock::now();
                        rebind_ok = RebindPrefillGraphRuntimeStateForModel(current_model, cached->graph, batch);
                        graph_runtime_rebind_ns +=
                            static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(
                                                      std::chrono::steady_clock::now() - rebind_begin)
                                                      .count());
                        if (!rebind_ok && IsDebugGraphLoggingEnabled()) {
                            std::cerr << "[PrefillGraphCache] evicting cached graph after runtime rebind failure"
                                      << " (tokens=" << prefill_graph_key.tokens
                                      << ", n_past=" << prefill_graph_key.n_past
                                      << ", threads=" << prefill_graph_key.threads << ")" << std::endl;
                        }
                    }
                    if (!rebind_ok) {
                        graph_cache.ErasePrefill(prefill_graph_key);
                    } else {
                        graph_cache.Touch(cached);
                        gf = cached->graph;
                        output = cached->output;
            ...
            if (!reused_decode_graph && !reused_prefill_graph) {
                if (decode_reuse_attempt_allowed) {
                    graph_cache.MakeDecodeRoom(static_cast<size_t>(decode_graph_cache_lru_size));
                        ...
                        if (candidate.graph) {
                            graph_build_begin = std::chrono::steady_clock::now();
                            {
                                ScopedBatchWorkContext cache_build_ctx(candidate.work_ctx.get(), &batch,
                                                                       InferenceExecutionPhase::Decode, true,
                                                                       current_model ? current_model->variant : ModelVariant::UNKNOWN);
                                bind_greedy_lm_head_argmax_sampling(candidate.work_ctx.get(),
                                                                    InferenceExecutionPhase::Decode);
                                if (!build_transformer_graph_or_fail(candidate.ctx, candidate.graph, &candidate.output,
                            ...
                            if (candidate.output && candidate.embd_inp && candidate.pos) {
                                ...
                                if (requires_paged_decode_verify && !candidate.verified_paged_decode_op) {
                                    ...
                                    const bool inserted_uncacheable =
                                        graph_cache.MarkUncacheable(decode_graph_key, decode_graph_uncacheable_limit);
                                    if (inserted_uncacheable) {
                                        if (is_decode_batch && decode_batch_size >= 1 &&
                                            decode_batch_size < static_cast<int>(kDecodeGraphCacheTrackedBatches)) {
                                ...
                                if (cache_entry_admissible) {
                                    DecodeGraphCacheEntry& entry = graph_cache.InsertDecode(
                                        decode_graph_key, std::move(candidate),
                                        static_cast<size_t>(decode_graph_cache_lru_size));
                                    ...
                                    if (lfm2_first_use_needs_runtime_reset) {
                                        ResetCachedDecodeGraphWorkContext(entry.work_ctx.get());
                                        ...
                                        graph_runtime_rebind_ns += static_cast<uint64_t>(
                                            std::chrono::duration_cast<std::chrono::nanoseconds>(
                                                std::chrono::steady_clock::now() - first_use_rebind_begin)
                                                .count());
                ...
                if (!built_decode_graph_cache_entry && prefill_reuse_candidate) {
                    graph_cache.MakePrefillRoom(static_cast<size_t>(prefill_graph_cache_lru_size));

                    PrefillGraphCacheEntry candidate;
                    candidate.work_ctx = PrefillGraphCacheEntry::WorkContextPtr(CreateInferenceWorkContext(),
                                                                                DestroyInferenceWorkContext);
                    ...
                    candidate.ctx = candidate.work_ctx ? ggml_init(prefill_params) : nullptr;
                    if (candidate.ctx) {
                        candidate.graph = ggml_new_graph_custom(candidate.ctx, 32768, false);
                        if (candidate.graph) {
                            graph_build_begin = std::chrono::steady_clock::now();
                            ...
                            if (candidate.output && candidate.embd_inp && candidate.pos) {
                                bool cache_entry_admissible = true;
                                if (current_model && current_model->arch_flags.is_hybrid_ssm) {
                                    ResetCachedPrefillGraphWorkContext(candidate.work_ctx.get());
                                    SetCurrentWorkContext(candidate.work_ctx.get());
                                    SetCurrentExecutionPhase(InferenceExecutionPhase::Prefill);
                                    SetCurrentBatch(&batch);
                                    const auto rebind_begin = std::chrono::steady_clock::now();
                                    cache_entry_admissible =
                                        RebindPrefillGraphRuntimeStateForModel(current_model, candidate.graph, batch);
                                    graph_runtime_rebind_ns +=
                                        static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(
                                                                  std::chrono::steady_clock::now() - rebind_begin)
                                                                  .count());
                                    if (!cache_entry_admissible && IsDebugGraphLoggingEnabled()) {
                                        std::cerr
                                            << "[PrefillGraphCache] skip cache insert: runtime rebind probe failed"
                                            << " (tokens=" << prefill_graph_key.tokens
                                            << ", n_past=" << prefill_graph_key.n_past
                                            << ", threads=" << prefill_graph_key.threads << ")" << std::endl;
                                    }
                                ...
                                if (cache_entry_admissible) {
                                    PrefillGraphCacheEntry& entry = graph_cache.InsertPrefill(
                                        prefill_graph_key, std::move(candidate),
                                        static_cast<size_t>(prefill_graph_cache_lru_size), prefill_graph_cache_max_bytes);
                                    gf = entry.graph;
                                    output = entry.output;
                                    embd_inp = entry.embd_inp;
                                    pos = entry.pos;
                ...
                if (!built_decode_graph_cache_entry && !built_prefill_graph_cache_entry) {
                    ...
                    if (use_transient_prefill_arena) {
                        ...
                        auto* arena_entry = &graph_cache.AcquireArena(
                            arena_key, metadata_bytes, static_cast<size_t>(prefill_graph_cache_lru_size), &arena_reused);
                        if (arena_reused) {
                        ...
                        graph_build_begin = std::chrono::steady_clock::now();
                        if (!build_transformer_graph_or_fail(ctx_nodes, gf, &output, &embd_inp, &pos)) {
                            continue;
                        }
                    ...
                    if (!using_transient_graph_arena) {
                        ...
                        bool allow_growth_reserve = false;
                        if (IsFlexibleGraphPoolSizingEnabled(current_model)) {
                            ...
                            const bool x86_qwen_hybrid_moe_chunked_prefill_graph =
                                content_dependent_prefill_graph &&
                            ...
                            if (cached_sizing == flexible_graph_pool_sizing_cache.end()) {
                                flexible_sizing =
                                    content_dependent_prefill_graph
                                        ? EstimateFlexibleGraphPoolSizeWithoutDryRun(current_model, graph_ctx_estimate)
                                        : MeasureFlexibleGraphPoolSize(current_model, current_kv_cache, batch,
                            ...
                            if (used_flexible_sizing) {
                                ...
                                allow_growth_reserve = !x86_qwen_hybrid_moe_chunked_prefill_graph;
                                ...
                                if (x86_qwen_hybrid_moe_chunked_prefill_graph && graph_ctx_estimate.total_bytes > 0) {
                                    constexpr size_t MB = 1024ULL * 1024ULL;
                                    const size_t reclaimable_pool_bytes = state->inference_ctx.IsInitialized()
                                                                              ? state->inference_ctx.compute_buffer_size
                                                                              : 0;
                                    const size_t sizing_available_bytes =
                                        flexible_sizing.available_bytes + reclaimable_pool_bytes;
                                    const size_t object_guard = DynamicGraphObjectPoolGuardBytes(
                                        graph_ctx_estimate, graph_ctx_estimate.total_bytes, sizing_available_bytes);
                                    ctx_size =
                                        std::max(ctx_size, AlignUpBytes(graph_ctx_estimate.total_bytes + object_guard,
                                                                        64ULL * MB));
                                    const size_t sequence_object_pressure =
                                        GraphSequenceObjectPressureBytes(graph_ctx_estimate);
                                    if (sequence_object_pressure > 0) {
                                        ctx_size = std::max(
                                            ctx_size,
                                            AlignUpBytes(graph_ctx_estimate.total_bytes +
                                                             AlignUpBytes(sequence_object_pressure, 64ULL * MB),
                                                         64ULL * MB));
                                    }
                                }
                            }
                        }
                        ctx_size = ResolveGraphPoolReservationBytes(
                            ctx_size, state->inference_ctx.IsInitialized() ? state->inference_ctx.compute_buffer_size : 0,
                            allow_growth_reserve);
                        ctx_size = ClampGraphPoolToExplicitMax(
                            ctx_size, used_flexible_sizing ? flexible_sizing.required_bytes : 0);
                        ...
                        if (!state->inference_ctx.IsInitialized() ||
                            ...
                            if (is_prefill_batch && graph_ctx_estimate.effective_query_len > 1 &&
                                ...
                                size_t safety_for_prefill = GraphContextPrefillAdmissionMarginBytes(
                                    current_model, graph_ctx_estimate.effective_query_len, available_for_prefill);
                                size_t q4k_cache_limit = q4k_cache_before.resident_bytes;
                                ...
                                const bool lfm2_moe_prefill = current_model &&
                                                              prefill_descriptor.variant == ModelVariant::LFM2MOE &&
                                                              current_model->arch_flags.is_lfm2_shortconv &&
                                                              ModelHasMoEGraphLayers(current_model);
                                ...
                                const bool c4a_qwen_hybrid_moe_prefill =
                                    ...
                                    current_model->arch_flags.is_hybrid_ssm && ModelHasMoEGraphLayers(current_model);
#else
                                    false;
#endif
                                ...
                                const size_t qwen_prefill_runtime_headroom =
                                    (preserve_moe_q4k_repacked_cache || c4a_qwen_hybrid_moe_prefill)
                                        ? std::clamp<size_t>(available_for_prefill > 0 ? available_for_prefill / 64
                                                                                       : 128ULL * MB,
                                                             64ULL * MB, 256ULL * MB)
                                        : std::max<size_t>(512ULL * MB, ctx_size / 8);
                                const size_t reserve_floor =
                                    ctx_size + safety_for_prefill + qwen_prefill_runtime_headroom;
                                ...
                                if (preserve_moe_q4k_repacked_cache && graph_ctx_estimate.effective_query_len >= 128) {
                                    ...
                                    if (moe_fast_path_cache_floor >
                                        ...
                                        std::cerr
                                            << "[DenseCore] Q4KRepackedCacheFloor before_moe_prefill" << " variant="
                                            ...
                                            << " floor_mb=" << (moe_fast_path_cache_floor / (1024ULL * 1024ULL))
                                            << " cache_limit_mb=" << (q4k_cache_limit / (1024ULL * 1024ULL))
                                            << " available_mb=" << (available_for_prefill / (1024ULL * 1024ULL))
                                            << " query_tokens=" << graph_ctx_estimate.effective_query_len << std::endl;
                                    }
                                }
                                if (!q4k_cache_fits_prefill) {
                                    if (preserve_moe_q4k_repacked_cache && available_for_prefill > 0 &&
                                        ...
                                        const size_t moe_cache_residency_floor = std::min<size_t>(
                                            q4k_cache_before.resident_bytes,
                                            std::clamp<size_t>(available_for_prefill > 0 ? available_for_prefill / 8
                                                                                         : 512ULL * MB,
                                                               512ULL * MB, 1024ULL * MB));
                                        ...
                                        if (trim_can_preserve_qwen_residency) {
                                            ...
                                            if (target_cache_bytes >= moe_cache_residency_floor) {
                                                ...
                                                const auto q4k_cache_after =
                                                    densecore::kernels::Q4KRepackedGemvTrimCacheToBytes(
                                                        target_cache_bytes);
                                                if (q4k_cache_after.resident_bytes < q4k_cache_before.resident_bytes) {
                                                    std::cerr
                                                        << "[DenseCore] Q4KRepackedCachePressureTrim"
                                                        << " before_mb=" << (q4k_cache_before.resident_bytes / MB)
                                                        << " after_mb=" << (q4k_cache_after.resident_bytes / MB)
                                                        << " ctx_mb=" << (ctx_size / MB)
                                                        << " safety_margin_mb=" << (safety_for_prefill / MB)
                                                        << " required_available_mb=" << (required_available / MB)
                                                        << " available_before_mb=" << (available_for_prefill / MB)
                                                        << " cache_limit_mb=" << (q4k_cache_limit / MB)
                                                        << " moe_cache_floor_mb=" << (moe_cache_residency_floor / MB)
                                                        << std::endl;
                                                    q4k_cache_trimmed_for_prefill = true;
                                                }
                                            }
                                        } else {
                                            reserved_for_prefill = std::min(
                                                available_for_prefill > 0 ? available_for_prefill : reserve_floor,
                                                reserve_floor);
                                            q4k_cache_limit =
                                        ...
                                        if (q4k_cache_trimmed_for_prefill &&
                                            ...
                                            if (q4k_cache_after.resident_bytes < q4k_cache_before.resident_bytes) {
                                                std::cerr
                                                    << "[DenseCore] Q4KRepackedCacheTrim before_qwen_prefill"
                                                    << " before_mb="
                                                    << (q4k_cache_before.resident_bytes / (1024 * 1024))
                                    ...
                                    } else if (IsGraphContextAutoDowngradeEnabled()) {
                                        constexpr int kCachePreservingPressureCandidates[] = {
                                            320, 288, 256, 224, 192, 160, 128, 96, 64, 48, 32, 24, 16, 12, 8, 4, 2, 1};
                                        for (Request* req : batch_requests) {
                                            ...
                                            for (int candidate : kCachePreservingPressureCandidates) {
                                                if (candidate >= current_chunk ||
                                                    static_cast<size_t>(candidate) > req->tokens.size()) {
                                                    continue;
                                                }
                                                const auto candidate_estimate = EngineState::EstimateGraphContextSize(
                                                    current_model,
                                                    static_cast<size_t>(std::max(1, req->n_past + candidate)),
                                                    /*num_seqs_hint=*/1, static_cast<size_t>(candidate));
                                                size_t candidate_ctx_bytes = candidate_estimate.total_bytes;
                                                if (IsFlexibleGraphPoolSizingEnabled(current_model)) {
                                                    const FlexibleGraphPoolSizing candidate_sizing =
                                                        EstimateFlexibleGraphPoolSizeWithoutDryRun(current_model,
                                                                                                   candidate_estimate);
                                                    if (candidate_sizing.ok && candidate_sizing.reserved_bytes > 0) {
                                                        candidate_ctx_bytes = candidate_sizing.reserved_bytes;
                                                    }
                                                }
                                                const size_t candidate_safety_margin =
                                                    GraphContextPrefillAdmissionMarginBytes(
                                                        current_model, static_cast<size_t>(candidate),
                                                        available_for_prefill);
                                                if (candidate_ctx_bytes + candidate_safety_margin <=
                                                    available_for_prefill) {
                                                    selected_chunk = candidate;
                                                    selected_ctx_bytes = candidate_ctx_bytes;
                                                    safety_for_prefill = candidate_safety_margin;
                                                    break;
                                                }
                                            }
                                            if (selected_chunk > 0 && selected_chunk < current_chunk) {
                                                req->prefill_chunk_tokens_effective = selected_chunk;
                                                req->graph_ctx_downgraded_chunk_tokens = selected_chunk;
                                                req->graph_ctx_requested_mb = selected_ctx_bytes / (1024ULL * 1024ULL);
                                                req->graph_ctx_available_mb =
                                                    available_for_prefill / (1024ULL * 1024ULL);
                                                req->graph_ctx_safety_margin_mb =
                                                    safety_for_prefill / (1024ULL * 1024ULL);
                                                req->graph_ctx_fail_reason.clear();
                                                cache_preserving_chunk_downgraded = true;
                                                std::cerr
                                                    << "[DenseCore] GraphCtxCachePreservingChunkDowngrade"
                                                    << " req=" << req->id << " original_chunk_tokens=" << current_chunk
                                                    << " downgraded_chunk_tokens=" << selected_chunk
                                                    << " requested_mb=" << (ctx_size / (1024ULL * 1024ULL))
                                                    << " downgraded_mb=" << (selected_ctx_bytes / (1024ULL * 1024ULL))
                                                    << " available_mb=" << (available_for_prefill / (1024ULL * 1024ULL))
                                                    << " safety_margin_mb="
                                                    << (safety_for_prefill / (1024ULL * 1024ULL)) << " q4k_resident_mb="
                                                    << (q4k_cache_before.resident_bytes / (1024ULL * 1024ULL))
                                                    << std::endl;
                                            }
                                        }
                                    }
                                    if (cache_preserving_chunk_downgraded) {
                                        continue;
                                    }
                                    reserved_for_prefill =
                                        std::min(available_for_prefill > 0 ? available_for_prefill : reserve_floor,
                                                 reserve_floor);
                                    q4k_cache_limit = densecore::kernels::Q4KRepackedGemvRefreshRuntimeCacheBudget(
                                        reserved_for_prefill);
                                }
                                if (!q4k_cache_fits_prefill && !q4k_cache_trimmed_for_prefill &&
                                    q4k_cache_before.resident_bytes > q4k_cache_limit) {
                                    const auto q4k_cache_after =
                                        densecore::kernels::Q4KRepackedGemvTrimCacheToBytes(q4k_cache_limit);
                                    if (q4k_cache_after.resident_bytes < q4k_cache_before.resident_bytes) {
                            ...
                            size_t graph_ctx_safety_margin_bytes =
                                is_prefill_batch
                                    ? GraphContextPrefillAdmissionMarginBytes(current_model,
                                                                              graph_ctx_estimate.effective_query_len,
                                                                              graph_ctx_available_bytes)
                                    : GraphContextSafetyMarginBytes();
                            for (Request* req : batch_requests) {
                                if (!req) {
                                    continue;
                                }
                            ...
                            if (graph_ctx_pressure) {
                                ...
                                if (is_prefill_batch && IsGraphContextAutoDowngradeEnabled()) {
                                    ...
                                    const bool runtime_gemma4_moe =
                                        ...
                                        ModelHasMoEGraphLayers(current_model);
                                    constexpr int kRuntimePressureCandidates[] = {
                                        320, 288, 256, 224, 192, 160, 128, 96, 64, 48, 32, 24, 16, 12, 8, 4, 2, 1};
                                    constexpr int kGemma4RuntimePressureCandidates[] = {
                                        448, 384, 320, 288, 256, 224, 192, 160, 128, 96,
                                        64,  48,  32,  24,  16,  12,  8,   4,   2,   1};
                                    const int* runtime_pressure_candidates = runtime_gemma4_moe
                                                                                 ? kGemma4RuntimePressureCandidates
                                                                                 : kRuntimePressureCandidates;
                                    const size_t runtime_pressure_candidate_count =
                                        runtime_gemma4_moe ? (sizeof(kGemma4RuntimePressureCandidates) /
                                    ...
                                    for (Request* req : batch_requests) {
                                        ...
                                        for (size_t i = 0; i < runtime_pressure_candidate_count; ++i) {
                                            ...
                                            if (candidate_ctx_bytes + candidate_safety_margin <=
                                                ...
                                                selected_chunk = candidate;
                                                selected_ctx_bytes = candidate_ctx_bytes;
                                                graph_ctx_safety_margin_bytes = candidate_safety_margin;
                                                break;
                                            }
                                        }
                                        if (selected_chunk > 0 && selected_chunk < current_chunk) {
                                            ...
                                            req->graph_ctx_requested_mb = selected_ctx_bytes / (1024ULL * 1024ULL);
                                            req->graph_ctx_available_mb =
                                                graph_ctx_available_bytes / (1024ULL * 1024ULL);
                                            req->graph_ctx_safety_margin_mb =
                                                graph_ctx_safety_margin_bytes / (1024ULL * 1024ULL);
                                            req->graph_ctx_fail_reason.clear();
                                            runtime_chunk_downgraded = true;
                                            std::cerr
                                                << "[DenseCore] GraphCtxRuntimeChunkDowngrade" << " req=" << req->id
                                                << " original_chunk_tokens=" << current_chunk
                                                << " downgraded_chunk_tokens=" << selected_chunk
                                                << " requested_mb=" << (ctx_size / (1024ULL * 1024ULL))
                                                << " downgraded_mb=" << (selected_ctx_bytes / (1024ULL * 1024ULL))
                                                << " available_mb=" << (graph_ctx_available_bytes / (1024ULL * 1024ULL))
                                                << " safety_margin_mb="
                                                << (graph_ctx_safety_margin_bytes / (1024ULL * 1024ULL)) << std::endl;
                                        }
                                    }
                                }
                                if (runtime_chunk_downgraded) {
                                    continue;
                                }
                                if (IsGraphContextFailClosedEnabled()) {
                                    ...
                                    fail_batch_requests(
                                        ...
                                        DecodeFinishCause::OutOfMemory, /*count_as_oom=*/true);
                                    continue;
                                }
                            }
                            try {
                                ...
                                if (!is_prefill_batch) {
                                    const size_t refreshed_q4k_cache_limit =
                                        densecore::kernels::Q4KRepackedGemvRefreshRuntimeCacheBudget(0);
                                    const auto decode_descriptor = densecore::models::DescribeModel(current_model);
                                    const bool qwen_hybrid_moe_decode =
                                        current_model &&
                                        (decode_descriptor.variant == ModelVariant::QWEN35 ||
                                         decode_descriptor.variant == ModelVariant::QWEN36) &&
                                        current_model->arch_flags.is_hybrid_ssm &&
                                        ModelHasMoEGraphLayers(current_model);
                                    if (qwen_hybrid_moe_decode) {
                                        const auto q4k_cache_after_init =
                                            densecore::kernels::Q4KRepackedGemvCacheStatsSnapshot();
                                        const size_t qwen_fast_path_cache_floor =
                                            q4k_cache_after_init.resident_bytes +
                                            EstimateQwenMoEQ4KPrefillFutureReserveBytes(
                                                current_model, graph_ctx_estimate.effective_query_len,
                                                graph_ctx_available_bytes);
                                        if (qwen_fast_path_cache_floor > refreshed_q4k_cache_limit) {
                                            densecore::kernels::Q4KRepackedGemvRaiseRuntimeCacheBudgetFloor(
                                ...
                                fail_batch_requests("graph_ctx_allocation_failed",
                        ...
                        size_t steady_available_bytes = ReadAvailableMemoryBytesForRuntimePools();
                        size_t steady_safety_margin_bytes =
                            is_prefill_batch
                                ? GraphContextPrefillAdmissionMarginBytes(
                                      current_model, graph_ctx_estimate.effective_query_len, steady_available_bytes)
                                : GraphContextSafetyMarginBytes();
                        if (is_prefill_batch && steady_available_bytes > 0 &&
                            steady_available_bytes < steady_safety_margin_bytes && current_model &&
                            IsX86QwenHybridMoEChunkedPrefillEstimate(current_model, graph_ctx_estimate)) {
                            const auto q4k_cache_before = densecore::kernels::Q4KRepackedGemvCacheStatsSnapshot();
                            constexpr size_t MB = 1024ULL * 1024ULL;
                            const size_t free_needed = steady_safety_margin_bytes - steady_available_bytes;
                            if (q4k_cache_before.resident_bytes > free_needed) {
                                const size_t target_cache_bytes = q4k_cache_before.resident_bytes - free_needed;
                                densecore::kernels::Q4KRepackedGemvSetRuntimeCacheBudgetFloor(target_cache_bytes);
                                const auto q4k_cache_after =
                                    densecore::kernels::Q4KRepackedGemvTrimCacheToBytes(target_cache_bytes);
                                steady_available_bytes = ReadAvailableMemoryBytesForRuntimePools();
                                steady_safety_margin_bytes = GraphContextPrefillAdmissionMarginBytes(
                                    current_model, graph_ctx_estimate.effective_query_len, steady_available_bytes);
                                std::cerr << "[DenseCore] Q4KRepackedCacheSteadyTrim"
                                          << " before_mb=" << (q4k_cache_before.resident_bytes / MB)
                                          << " after_mb=" << (q4k_cache_after.resident_bytes / MB)
                                          << " steady_available_mb=" << (steady_available_bytes / MB)
                                          << " safety_margin_mb=" << (steady_safety_margin_bytes / MB) << std::endl;
                            }
                        }
                        if (is_prefill_batch && steady_available_bytes > 0 &&
                            steady_available_bytes < steady_safety_margin_bytes && IsGraphContextFailClosedEnabled()) {
                            bool steady_chunk_downgraded = false;
                            if (IsGraphContextAutoDowngradeEnabled() && state->inference_ctx.IsInitialized()) {
                                ...
                                const bool steady_gemma4_moe = steady_descriptor.variant == ModelVariant::GEMMA4 &&
                                                               ModelHasMoEGraphLayers(current_model);
                                constexpr int kSteadyPressureCandidates[] = {320, 288, 256, 224, 192, 160, 128, 96, 64,
                                                                             48,  32,  24,  16,  12,  8,   4,   2,  1};
                                constexpr int kGemma4SteadyPressureCandidates[] = {448, 384, 320, 288, 256, 224, 192,
                                                                                   160, 128, 96,  64,  48,  32,  24,
                                                                                   16,  12,  8,   4,   2,   1};
                                ...
                                for (Request* req : batch_requests) {
                                    ...
                                    for (size_t i = 0; i < steady_pressure_candidate_count; ++i) {
                                        ...
                                        const auto candidate_estimate = EngineState::EstimateGraphContextSize(
                                            current_model, static_cast<size_t>(std::max(1, req->n_past + candidate)),
                                            /*num_seqs_hint=*/1, static_cast<size_t>(candidate));
                                        size_t candidate_ctx_bytes = candidate_estimate.total_bytes;
                                        if (IsFlexibleGraphPoolSizingEnabled(current_model)) {
                                            const FlexibleGraphPoolSizing candidate_sizing =
                                        ...
                                        const size_t candidate_safety_margin = GraphContextPrefillAdmissionMarginBytes(
                                            current_model, static_cast<size_t>(candidate),
                                            available_after_pool_release);
                                        if (candidate_ctx_bytes + candidate_safety_margin <=
                                            available_after_pool_release) {
                                            selected_chunk = candidate;
                                            selected_ctx_bytes = candidate_ctx_bytes;
                                            steady_safety_margin_bytes = candidate_safety_margin;
                                            break;
                                        }
                                    }
                                    if (selected_chunk > 0 && selected_chunk < current_chunk) {
                                        ...
                                        req->graph_ctx_requested_mb = selected_ctx_bytes / (1024ULL * 1024ULL);
                                        req->graph_ctx_available_mb =
                                            available_after_pool_release / (1024ULL * 1024ULL);
                                        req->graph_ctx_safety_margin_mb =
                                            steady_safety_margin_bytes / (1024ULL * 1024ULL);
                                        req->graph_ctx_fail_reason.clear();
                                        steady_chunk_downgraded = true;
                                        std::cerr
                                            << "[DenseCore] GraphCtxSteadyChunkDowngrade" << " req=" << req->id
                                            << " original_chunk_tokens=" << current_chunk
                                            << " downgraded_chunk_tokens=" << selected_chunk
                                            << " current_pool_mb=" << (current_pool_bytes / (1024ULL * 1024ULL))
                                            << " steady_available_mb=" << (steady_available_bytes / (1024ULL * 1024ULL))
                                            << " available_after_pool_release_mb="
                                            << (available_after_pool_release / (1024ULL * 1024ULL))
                                            << " downgraded_mb=" << (selected_ctx_bytes / (1024ULL * 1024ULL))
                                            << " safety_margin_mb="
                                            << (steady_safety_margin_bytes / (1024ULL * 1024ULL)) << std::endl;
                                    }
                                }
                                if (steady_chunk_downgraded) {
                                    state->inference_ctx.Free();
                                    continue;
                                }
                            }
                            ...
                            fail_batch_requests("prefill_graph_ctx_headroom_exhausted",
                                                "Error: graph context headroom exhausted",
                                                DecodeFinishCause::OutOfMemory, /*count_as_oom=*/true);
                            continue;
                        }

                        const auto prefill_phase_trace_start = std::chrono::steady_clock::now();
                        auto emit_prefill_phase_marker = [&](const char* phase) {
                        ...
                        emit_prefill_phase_marker("reset_begin");
                        state->inference_ctx.Reset();
                        emit_prefill_phase_marker("reset_end");
                        ...
                        emit_prefill_phase_marker("graph_alloc_begin");
                        gf = ggml_new_graph_custom(ctx_nodes, 32768, false);
                        emit_prefill_phase_marker("graph_alloc_end");
                        bind_main_work_context_for_graph(batch, batch_execution_phase);
                        emit_prefill_phase_marker("build_begin");
                        graph_build_begin = std::chrono::steady_clock::now();
                        if (!build_transformer_graph_or_fail(ctx_nodes, gf, &output, &embd_inp, &pos)) {
                            continue;
                        }
                        graph_build_end = std::chrono::steady_clock::now();
                        emit_prefill_phase_marker("build_end");
                        const auto graph_build_ns = static_cast<uint64_t>(
                            std::chrono::duration_cast<std::chrono::nanoseconds>(graph_build_end - graph_build_begin)
                                .count());
                        for (Request* req : batch_requests) {
            ...
            LogRequestLifecyclePhase(
                "post_compute_profile_begin", !batch_requests.empty() ? batch_requests.front() : nullptr, current_model,
                !batch_requests.empty() && batch_requests.front() ? batch_requests.front()->seq_id : -1, batch.num_seqs,
                static_cast<int>(batch.tokens.size()), active_threads, is_decode_batch ? "decode" : "prefill");
            if (is_prefill_batch && batch.tokens.size() >= 32) {
            ...
            LogRequestLifecyclePhase(
                "post_compute_profile_accumulated", !batch_requests.empty() ? batch_requests.front() : nullptr,
                current_model, !batch_requests.empty() && batch_requests.front() ? batch_requests.front()->seq_id : -1,
                batch.num_seqs, static_cast<int>(batch.tokens.size()), active_threads,
                is_decode_batch ? "decode" : "prefill");
            const bool collect_prefill_graph_diagnostics = is_prefill_batch && IsLLMNodeTimingDumpEnabled();
            const Qwen36PrefillBreakdown qwen36_prefill_breakdown =
                collect_prefill_graph_diagnostics ? SummarizeQwen36PrefillNodeTimes(current_model, gf)
                                                  : Qwen36PrefillBreakdown{};
            const Gemma4PrefillAttentionTimingBreakdown gemma4_prefill_attention_breakdown =
                collect_prefill_graph_diagnostics
                    ? SummarizeGemma4PrefillAttentionNodeTimes(current_model, gf, IsLLMNodeTimingDumpEnabled())
            ...
            LogRequestLifecyclePhase(
                "post_compute_profile_snapshot", !batch_requests.empty() ? batch_requests.front() : nullptr,
                current_model, !batch_requests.empty() && batch_requests.front() ? batch_requests.front()->seq_id : -1,
                batch.num_seqs, static_cast<int>(batch.tokens.size()), active_threads,
                is_decode_batch ? "decode" : "prefill");
            ...
            const uint64_t qwen36_prefill_graph_build_ns =
                (is_prefill_batch && graph_build_begin != std::chrono::steady_clock::time_point() &&
                 graph_build_end != std::chrono::steady_clock::time_point() && graph_build_end >= graph_build_begin)
                    ...
                    : 0;
            LogRequestLifecyclePhase(
                "post_compute_request_merge_begin", !batch_requests.empty() ? batch_requests.front() : nullptr,
                current_model, !batch_requests.empty() && batch_requests.front() ? batch_requests.front()->seq_id : -1,
                batch.num_seqs, static_cast<int>(batch.tokens.size()), active_threads,
                is_decode_batch ? "decode" : "prefill");
            for (Request* req : batch_requests) {
                if (req) {
                    ...
                    if (collect_decode_graph_node_timing) {
                        ...
                        req->decode_semantic_attention_o_count += decode_graph_node_timing.semantic_attention_o_count;
                        ...
                        req->decode_semantic_unattributed_count += decode_graph_node_timing.semantic_unattributed_count;
                    ...
                    if (native_moe_graph_timing.native_node_count > 0) {
                        ...
                        if (!native_moe_graph_timing.top_slow_nodes.empty()) {
                    ...
                    req->q8_batched_weight_cache_ns += qwen36_profile.q8_batched_weight_cache_ns;
                    req->q8_batched_activation_quant_ns += qwen36_profile.q8_batched_activation_quant_ns;
                    req->q8_batched_activation_wait_ns += qwen36_profile.q8_batched_activation_wait_ns;
                    req->q8_batched_activation_pack_ns += qwen36_profile.q8_batched_activation_pack_ns;
                    req->q8_batched_compute_ns += qwen36_profile.q8_batched_compute_ns;
                    req->q8_batched_used_ops += qwen36_profile.q8_batched_used_ops;
                    req->q8_batched_true_gemm_ops += qwen36_profile.q8_batched_true_gemm_ops;
                    req->q8_batched_gemv_ops += qwen36_profile.q8_batched_gemv_ops;
                    req->moe_kquant_raw_batched_q4k_used_ops += qwen36_profile.moe_kquant_raw_batched_q4k_used_ops;
                    req->moe_kquant_raw_batched_q4k_ns += qwen36_profile.moe_kquant_raw_batched_q4k_ns;
                    req->moe_kquant_raw_batched_q5k_used_ops += qwen36_profile.moe_kquant_raw_batched_q5k_used_ops;
                    req->moe_kquant_raw_batched_q5k_ns += qwen36_profile.moe_kquant_raw_batched_q5k_ns;
                    ...
                    if (IsQwenHybridSSMModel(current_model) &&
                        Qwen36HybridSSMProjectionWeightsAreNotQ4K(current_model)) {
                    ...
                    req->qwen_native_moe_w2_q5k_raw_batched_ns += qwen36_profile.qwen_native_moe_w2_q5k_raw_batched_ns;
                    req->qwen_native_moe_fused_router_used_ops += qwen36_profile.qwen_native_moe_fused_router_used_ops;
                    ...
                    req->qwen_native_moe_q4_gateup_rowpair_ns += qwen36_profile.qwen_native_moe_q4_gateup_rowpair_ns;
                    ...
                    req->ssm_delta_fast_default_used_ops += qwen36_profile.ssm_delta_fast_default_used_ops;
                    req->ssm_delta_fast_default_wall_ns += qwen36_profile.ssm_delta_fast_default_wall_ns;
            ...
            LogRequestLifecyclePhase(
                "post_compute_request_merge_end", !batch_requests.empty() ? batch_requests.front() : nullptr,
                current_model, !batch_requests.empty() && batch_requests.front() ? batch_requests.front()->seq_id : -1,
                batch.num_seqs, static_cast<int>(batch.tokens.size()), active_threads,
                is_decode_batch ? "decode" : "prefill");
            ...
            if (ConsumeMoEStrictFailure(&moe_strict_failure)) {
                ...
                for (Request* req : batch_requests) {
                    ...
                    finalize_generation_error(req, DecodeFinishCause::Unknown, "Error: " + moe_strict_failure,
                                              global_direct_callback);
                    free_request_blocks(req);
                    remove_request_from_scheduler(req, /*completed=*/false);
                }
            ...
            if (is_decode_batch && decode_batch_size >= 1 &&
                ...
                NoteDecodeExecutedPhysicalBatch(decode_batch_size);
            ...
            for (int i = 0; i < batch.num_seqs; ++i) {
                ...
                if (is_embedding_batch) {
                    ...
                    if (FinalizeEmbeddingRequestOnce(state, req, std::move(embedding), DENSECORE_STATUS_OK,
                                                     global_direct_callback)) {
                        state->metrics.completed_requests++;
                    }
                    ...
                    free_request_blocks(req);
                    remove_request_from_scheduler(req, /*completed=*/true);
                } else {
                    // Generation
                    ...
                    auto register_completed_prefix_blocks = [&]() {
                        ...
                        // Only prompt blocks can be reused by a later request.
                        // Registering generated continuation blocks retained a
                        // full hybrid-SSM snapshot (about 63 MiB for Qwen3.6)
                        // at every aligned decode boundary and made RSS grow by
                        // roughly 1 GiB per 256-token response.
                        const int prompt_blocks =
                            static_cast<int>(req->original_prompt_tokens_for_cache.size()) / BLOCK_SIZE;
                        const int completed_blocks = std::min(
                            {req->n_past / BLOCK_SIZE, static_cast<int>(req->block_table.size()), prompt_blocks});
                        ...
                        for (int blk_idx = req->registered_prefix_blocks; blk_idx < completed_blocks; ++blk_idx) {
                            ...
                            const uint64_t hash = BlockManager::ComputeTokenHash(
                                req->sequence_tokens_for_prefix_cache.data(), start_token + BLOCK_SIZE);
                    ...
                    if (!ResolveSamplingLogitsColumnForRequest(
                            ...
                            &sampling_guard_error, i, batch.num_seqs)) {
                    ...
                    LogRequestLifecyclePhase("sample_begin", req, current_model, req->seq_id, batch.num_seqs,
                                             static_cast<int>(batch.tokens.size()), active_threads);
                    int best_token = -1;
                    ...
                    LogRequestLifecyclePhase("sample_end", req, current_model, req->seq_id, batch.num_seqs,
                                             static_cast<int>(batch.tokens.size()), active_threads);
                    ...
                    {
                        std::string token_piece = req->token_result_callback
                                                      ? Tokenizer::DetokenizeStructured(current_model, best_token)
                                                      : Tokenizer::Detokenize(current_model, best_token);
                        if (const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_FIRST_TOKEN_RAW");
                    ...
                    if (!req->json_mode && !token_str.empty() && !req->token_result_callback &&
                        req->suppress_reasoning_tags &&
                    ...
                    if (!req->json_mode && !token_str.empty()) {
                        SuppressQwenVisibleControlMarkers(&token_str);
                    }
                    ...
                    if (req->callback || req->callback_ex || req->token_result_callback) {
                        if (!token_str.empty() || req->token_result_callback) {
                            ...
                            if (densecore::env::GetDiagnosticEnv("DENSECORE_PARITY_DEBUG") != nullptr &&
                    ...
                    while ((int)req->block_table.size() < blocks_needed) {
                        ...
                        if (new_blocks.empty()) {
                            ...
                            state->metrics.oom_errors++;
                            terminal_error_emitted = finalize_generation_error(
                                req, DecodeFinishCause::OutOfMemory, "Error: Out of memory", global_direct_callback);
                            break;
                        }
                    ...
                    if (req->finished || IsStopTokenId(current_model, best_token) ||
                        ...
                        LogRequestLifecyclePhase("finish_begin", req, current_model, req->seq_id, batch.num_seqs,
                                                 static_cast<int>(batch.tokens.size()), active_threads);
                        if (!req->finished) {
                        ...
                        if (IsMoEPathTraceDumpEnabled() && !IsMoETracePlumbingDisabled()) {
                            const auto forward_calls =
                                static_cast<densecore::CpuBackend*>(deps.backend_registry->Get(densecore::DeviceType::CPU))->GetMoEForwardInvocationCount();
                            const auto graph_wiring = GetMoEGraphWiringDebugCounter();
                            const auto traces = static_cast<densecore::CpuBackend*>(deps.backend_registry->Get(densecore::DeviceType::CPU))->GetMoEPathTraceSnapshot();
                        ...
                        if (!terminal_error_emitted && !req->utf8_pending.empty() &&
                            ...
                            if (emit_len > 0) {
                                ...
                                if (!req->json_mode && !req->token_result_callback && req->suppress_reasoning_tags &&
                                    IsReasoningTagSuppressionEnabled()) {
                                    const bool may_contain_tag =
                                        req->in_think_block || req->in_tool_call_block || req->in_tool_response_block ||
                                        !req->think_tag_pending.empty() || !req->tool_call_tag_pending.empty() ||
                                ...
                                if (!req->json_mode && !tail.empty()) {
                                    SuppressQwenVisibleControlMarkers(&tail);
                                }
                            ...
                            if (!req->finished && HasDecodeVisibleProgressStalled(req, decode_sample_time)) {
                                ...
                                terminal_error_emitted = finalize_generation_error(
                                    req, DecodeFinishCause::DecodeVisibleProgressTimeout,
                                    "Error: decode made no externally visible progress", global_direct_callback);
                            }
                        }
                        LogRequestLifecyclePhase("finish_before_summary", req, current_model, req->seq_id,
                                                 batch.num_seqs, static_cast<int>(batch.tokens.size()), active_threads);
                        if (!terminal_error_emitted) {
                            LogRequestDecodeSummary(req, current_model, deps.transformer_execution_plan);
                        }
                        LogRequestLifecyclePhase("finish_after_summary", req, current_model, req->seq_id,
                                                 batch.num_seqs, static_cast<int>(batch.tokens.size()), active_threads);
                        req->utf8_pending.clear();
                        // Decode graph cache is keyed by batch shape/threading, not request ID.
                        if (!terminal_error_emitted) {
                            finalize_generation_success(req, global_direct_callback, deps.transformer_execution_plan);
                        }
        ...
        graph_cache.ClearDecode();
        graph_cache.ClearPrefill();
        ClearQwen36SSMQ8PrefillAMXAliases(current_model);
        graph_cache.ClearArenas();
    } catch (const densecore::DenseCoreException& e) {
