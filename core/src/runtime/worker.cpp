...
void EngineLoop(EngineState* state) {
    ...
    try {
        ...
        struct PrefillGraphCacheEntry {
            using WorkContextPtr = DecodeGraphCacheEntry::WorkContextPtr;
        ...
        auto reset_cached_prefill_graph_runtime_context = [](InferenceWorkContext* ctx) {
            ResetCachedPrefillGraphWorkContext(ctx);
        };
        auto free_prefill_graph_entry = [](PrefillGraphCacheEntry* entry) {
            if (!entry) return;
            if (entry->ctx) {
                ggml_free(entry->ctx);
                entry->ctx = nullptr;
        ...
        auto clear_prefill_graph_cache = [&]() {
            for (auto& kv : prefill_graph_cache) {
                free_prefill_graph_entry(&kv.second);
            }
        ...
        while (state->status != EngineStatus::STOPPED) {
            ...
            const WorkerQwen36SSMQ8PrefillAliasPlan qwen36_q8_prefill_alias_plan =
                ResolveWorkerQwen36SSMQ8PrefillAliasPlan(current_model, is_decode_batch, is_prefill_batch,
                                                         prefill_prompt_token_count, state->fast_path_config);
            ApplyWorkerQwen36SSMQ8PrefillAliasPlan(current_model, qwen36_q8_prefill_alias_plan, batch_requests);
            ...
            if (!reused_decode_graph && prefill_reuse_candidate) {
                ...
                if (it != prefill_graph_cache.end() && it->second.graph && it->second.output && it->second.embd_inp &&
                    it->second.pos && it->second.work_ctx) {
                    bool rebind_ok = true;
                    reset_cached_prefill_graph_runtime_context(it->second.work_ctx.get());
                    SetCurrentWorkContext(it->second.work_ctx.get());
                    SetCurrentExecutionPhase(InferenceExecutionPhase::Prefill);
                    SetCurrentBatch(&batch);
                    if (current_model && current_model->arch_flags.is_hybrid_ssm) {
                        const auto rebind_begin = std::chrono::steady_clock::now();
                        rebind_ok = RebindPrefillGraphRuntimeStateForModel(current_model, it->second.graph, batch);
                        graph_runtime_rebind_ns +=
                            static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(
                                                      std::chrono::steady_clock::now() - rebind_begin)
                                                      .count());
                    ...
                    if (!rebind_ok) {
                        prefill_graph_cache_bytes = (prefill_graph_cache_bytes > it->second.ctx_bytes)
                                                        ? (prefill_graph_cache_bytes - it->second.ctx_bytes)
                                                        : 0;
                        prefill_graph_lru.erase(it->second.lru_it);
                        free_prefill_graph_entry(&it->second);
                        prefill_graph_cache.erase(it);
                    } else {
            ...
            if (!reused_decode_graph && !reused_prefill_graph) {
                ...
                if (!built_decode_graph_cache_entry && prefill_reuse_candidate) {
                    while (prefill_graph_cache.size() >= static_cast<size_t>(prefill_graph_cache_lru_size) &&
                        ...
                        if (evict_it != prefill_graph_cache.end()) {
                            prefill_graph_cache_bytes = (prefill_graph_cache_bytes > evict_it->second.ctx_bytes)
                                                            ...
                                                            : 0;
                            free_prefill_graph_entry(&evict_it->second);
                    ...
                    if (candidate.ctx) {
                        ...
                        if (candidate.graph) {
                            ...
                            if (candidate.output && candidate.embd_inp && candidate.pos) {
                                bool cache_entry_admissible = true;
                                if (current_model && current_model->arch_flags.is_hybrid_ssm) {
                                    reset_cached_prefill_graph_runtime_context(candidate.work_ctx.get());
                                ...
                                if (cache_entry_admissible) {
                                    while (!prefill_graph_lru.empty() &&
                                        ...
                                        prefill_graph_cache_bytes =
                                            (prefill_graph_cache_bytes > budget_evict_it->second.ctx_bytes)
                                                ? (prefill_graph_cache_bytes - budget_evict_it->second.ctx_bytes)
                                                : 0;
                                        free_prefill_graph_entry(&budget_evict_it->second);
                                    ...
                                    auto inserted =
                                        prefill_graph_cache.emplace(prefill_graph_key, std::move(candidate));
                    ...
                    if (!built_prefill_graph_cache_entry && candidate.ctx) {
                        free_prefill_graph_entry(&candidate);
                    }
                }
                    ...
                    if (!using_transient_graph_arena) {
                        ...
                        if (IsFlexibleGraphPoolSizingEnabled(current_model)) {
                            ...
                            if (used_flexible_sizing) {
                                ...
                                if (x86_qwen_hybrid_moe_chunked_prefill_graph && graph_ctx_estimate.total_bytes > 0) {
                                    constexpr size_t MB = 1024ULL * 1024ULL;
                                    const size_t reclaimable_pool_bytes =
                                        state->inference_ctx.IsInitialized() ? state->inference_ctx.compute_buffer_size : 0;
                                    ...
                                    const size_t object_guard = DynamicGraphObjectPoolGuardBytes(
                                        graph_ctx_estimate, graph_ctx_estimate.total_bytes,
                                        sizing_available_bytes);
                                    ctx_size = std::max(
                                        ctx_size,
                                        AlignUpBytes(graph_ctx_estimate.total_bytes + object_guard,
                                                     64ULL * MB));
                                    const size_t sequence_object_pressure =
                                        GraphSequenceObjectPressureBytes(graph_ctx_estimate);
                                    if (sequence_object_pressure > 0 && reclaimable_pool_bytes > 0) {
                                        ctx_size = std::max(
                                            ctx_size,
                                            AlignUpBytes(reclaimable_pool_bytes +
                                                             AlignUpBytes(sequence_object_pressure, 64ULL * MB),
                                                         64ULL * MB));
                                    }
                                }
                                if (content_dependent_prefill_graph && !x86_qwen_hybrid_moe_chunked_prefill_graph &&
                                ...
                                if (state->inference_ctx.IsInitialized() &&
                                    state->inference_ctx.compute_buffer_size < ctx_size &&
                                    !content_dependent_grow_applied && !x86_qwen_hybrid_moe_chunked_prefill_graph) {
                        ...
                        if (!state->inference_ctx.IsInitialized() ||
                                ...
                                if (preserve_moe_q4k_repacked_cache &&
                                    ...
                                    if (moe_fast_path_cache_floor >
                                        ...
                                        std::cerr << "[DenseCore] Q4KRepackedCacheFloor before_moe_prefill"
                                                  ...
                                                  << " query_tokens="
                                                  << graph_ctx_estimate.effective_query_len << std::endl;
                                    }
                                }
                                    ...
                                    if (preserve_moe_q4k_repacked_cache && available_for_prefill > 0 &&
                                        ...
                                        if (trim_can_preserve_qwen_residency) {
                                            ...
                                            if (target_cache_bytes >= moe_cache_residency_floor) {
                                                ...
                                                if (q4k_cache_after.resident_bytes < q4k_cache_before.resident_bytes) {
                                                    std::cerr << "[DenseCore] Q4KRepackedCachePressureTrim"
                                                              << " before_mb="
                                                              << (q4k_cache_before.resident_bytes / MB)
                                                              << " after_mb="
                                                              << (q4k_cache_after.resident_bytes / MB)
                                                              << " ctx_mb=" << (ctx_size / MB)
                                                              << " safety_margin_mb="
                                                              << (safety_for_prefill / MB)
                                                              << " required_available_mb="
                                                              << (required_available / MB)
                                                              << " available_before_mb="
                                            ...
                                            reserved_for_prefill =
                                                std::min(available_for_prefill > 0 ? available_for_prefill
                                                                                  : reserve_floor,
                                                         reserve_floor);
                                            q4k_cache_limit =
                                        ...
                                        if (moe_fast_path_cache_floor > q4k_cache_limit &&
                                            moe_fast_path_cache_floor + ctx_size + safety_for_prefill <=
	                                            ...
	                                            if (q4k_cache_after.resident_bytes < q4k_cache_before.resident_bytes) {
                                                std::cerr << "[DenseCore] Q4KRepackedCacheTrim before_qwen_prefill"
                                                          << " before_mb="
                                                          ...
                                                          << " reserve_mb="
                                                          << (reserved_for_prefill / (1024 * 1024))
                                                          << " cache_limit_mb="
                                                          << (q4k_cache_limit / (1024 * 1024))
                                                          << std::endl;
                                            }
                                        }
                                        ...
                                        for (Request* req : batch_requests) {
                                            ...
                                            if (selected_chunk > 0 && selected_chunk < current_chunk) {
                                                ...
                                                req->graph_ctx_requested_mb =
                                                    selected_ctx_bytes / (1024ULL * 1024ULL);
                                                ...
                                                std::cerr << "[DenseCore] GraphCtxCachePreservingChunkDowngrade"
                                                          << " req=" << req->id
                                                          << " original_chunk_tokens=" << current_chunk
                                                          << " downgraded_chunk_tokens=" << selected_chunk
                                                          << " requested_mb="
                                                          << (ctx_size / (1024ULL * 1024ULL))
                                                          << " downgraded_mb="
                                                          << (selected_ctx_bytes / (1024ULL * 1024ULL))
                                                          << " available_mb="
                                                          << (available_for_prefill / (1024ULL * 1024ULL))
                            ...
                            size_t graph_ctx_safety_margin_bytes =
                                is_prefill_batch ? GraphContextPrefillAdmissionMarginBytes(
                                                       current_model, graph_ctx_estimate.effective_query_len,
                                ...
                                if (is_prefill_batch && IsGraphContextAutoDowngradeEnabled()) {
                                    ...
                                    for (Request* req : batch_requests) {
                                        ...
                                        if (selected_chunk > 0 && selected_chunk < current_chunk) {
                                            ...
                                            std::cerr << "[DenseCore] GraphCtxRuntimeChunkDowngrade"
                                                      << " req=" << req->id
                                                      << " original_chunk_tokens=" << current_chunk
                                                      << " downgraded_chunk_tokens=" << selected_chunk
                                                      << " requested_mb="
                        ...
                        if (is_prefill_batch && steady_available_bytes > 0 &&
                            ...
                            if (q4k_cache_before.resident_bytes > free_needed) {
                                ...
                                std::cerr << "[DenseCore] Q4KRepackedCacheSteadyTrim"
                                          ...
                                          << " safety_margin_mb=" << (steady_safety_margin_bytes / MB)
                                          << std::endl;
                            }
                        }
                        if (is_prefill_batch && steady_available_bytes > 0 &&
                            ...
                            if (IsGraphContextAutoDowngradeEnabled() && state->inference_ctx.IsInitialized()) {
                                ...
                                for (Request* req : batch_requests) {
                                    ...
                                    if (selected_chunk > 0 && selected_chunk < current_chunk) {
                                        ...
                                        std::cerr << "[DenseCore] GraphCtxSteadyChunkDowngrade"
                                                  << " req=" << req->id
                                                  << " original_chunk_tokens=" << current_chunk
                                                  << " downgraded_chunk_tokens=" << selected_chunk
                                                  << " current_pool_mb="
                                                  << (current_pool_bytes / (1024ULL * 1024ULL))
                                                  << " steady_available_mb="
                                                  << (steady_available_bytes / (1024ULL * 1024ULL))
            ...
            LogRequestLifecyclePhase("post_compute_profile_begin",
                                     !batch_requests.empty() ? batch_requests.front() : nullptr, current_model,
                                     !batch_requests.empty() && batch_requests.front() ? batch_requests.front()->seq_id
                                                                                       : -1,
                                     batch.num_seqs, static_cast<int>(batch.tokens.size()), active_threads,
            ...
            LogRequestLifecyclePhase("post_compute_profile_accumulated",
                                     !batch_requests.empty() ? batch_requests.front() : nullptr, current_model,
                                     !batch_requests.empty() && batch_requests.front() ? batch_requests.front()->seq_id
                                                                                       : -1,
                                     batch.num_seqs, static_cast<int>(batch.tokens.size()), active_threads,
            ...
            LogRequestLifecyclePhase("post_compute_profile_snapshot",
                                     !batch_requests.empty() ? batch_requests.front() : nullptr, current_model,
                                     !batch_requests.empty() && batch_requests.front() ? batch_requests.front()->seq_id
                                                                                       : -1,
                                     batch.num_seqs, static_cast<int>(batch.tokens.size()), active_threads,
                 ...
                 graph_build_end != std::chrono::steady_clock::time_point() && graph_build_end >= graph_build_begin)
                    ? static_cast<uint64_t>(
                          std::chrono::duration_cast<std::chrono::nanoseconds>(graph_build_end - graph_build_begin)
                              .count())
                    : 0;
            LogRequestLifecyclePhase("post_compute_request_merge_begin",
                                     !batch_requests.empty() ? batch_requests.front() : nullptr, current_model,
                                     !batch_requests.empty() && batch_requests.front() ? batch_requests.front()->seq_id
                                                                                       : -1,
                                     batch.num_seqs, static_cast<int>(batch.tokens.size()), active_threads,
            ...
            for (Request* req : batch_requests) {
                if (req) {
                    ...
                    req->moe_kquant_raw_batched_q4k_used_ops +=
                        qwen36_profile.moe_kquant_raw_batched_q4k_used_ops;
                    req->moe_kquant_raw_batched_q4k_ns += qwen36_profile.moe_kquant_raw_batched_q4k_ns;
                    req->moe_kquant_raw_batched_q5k_used_ops +=
                        qwen36_profile.moe_kquant_raw_batched_q5k_used_ops;
            ...
            LogRequestLifecyclePhase("post_compute_request_merge_end",
                                     !batch_requests.empty() ? batch_requests.front() : nullptr, current_model,
                                     !batch_requests.empty() && batch_requests.front() ? batch_requests.front()->seq_id
                                                                                       : -1,
                                     batch.num_seqs, static_cast<int>(batch.tokens.size()), active_threads,
