...
template <typename T>
bool ValidABIHeader(const T* value, size_t required_size) {
    return value != nullptr && DENSECORE_ENT_ABI_MAJOR(value->abi_version) == DENSECORE_ENT_ABI_VERSION_MAJOR &&
           value->struct_size >= required_size;
}

bool RequiredCapabilityVTablesPresent(uint64_t capabilities) {
    if ((capabilities & DENSECORE_ENT_CAP_TELEMETRY) != 0 && !g_telemetry_vtable.load(std::memory_order_acquire)) return false;
    if ((capabilities & DENSECORE_ENT_CAP_LICENSE) != 0 && !g_license_vtable.load(std::memory_order_acquire)) return false;
    if ((capabilities & DENSECORE_ENT_CAP_NUMA_ROUTING) != 0 && !g_numa_routing_vtable.load(std::memory_order_acquire)) return false;
    if ((capabilities & DENSECORE_ENT_CAP_CONFIDENTIAL) != 0 && !g_confidential_vtable.load(std::memory_order_acquire)) return false;
    if ((capabilities & DENSECORE_ENT_CAP_QUOTA) != 0 && !g_quota_vtable.load(std::memory_order_acquire)) return false;
    return true;
}
...
int DenseCoreEntLoadPlugin(const char* plugin_path, void* engine) {
    ...
    constexpr size_t kRequiredPluginInfoSize = offsetof(DenseCoreEntPluginInfo, capabilities) + sizeof(uint64_t);
    if (!ValidABIHeader(plugin_info, kRequiredPluginInfoSize)) {
        const uint32_t major = DENSECORE_ENT_ABI_MAJOR(plugin_info->abi_version);
        ENT_LOG_ERROR("Plugin API version mismatch: host=%u.x, plugin=%u.%u", DENSECORE_ENT_API_VERSION_MAJOR, major,
                      plugin_info->abi_version & 0xFFFF);
        RollbackFailedLoad();
        return -3;
    }
    if (!plugin_info->name || !plugin_info->version || !RequiredCapabilityVTablesPresent(plugin_info->capabilities)) {
        ENT_LOG_ERROR("Plugin ABI v2 is missing required metadata or capability callbacks");
        RollbackFailedLoad();
        return -3;
    }
...
void DenseCoreEntRegisterTelemetry(const DenseCoreEntTelemetryVTable* vtable) {
    constexpr size_t required = offsetof(DenseCoreEntTelemetryVTable, record_cross_numa) + sizeof(DenseCoreEntRecordCrossNumaFn);
    if (!ValidABIHeader(vtable, required) || !vtable->get_snapshot || !vtable->record_tenant_tokens ||
        !vtable->record_kv_event || !vtable->record_expert_route || !vtable->record_cross_numa) {
        ENT_LOG_ERROR("Rejected incompatible telemetry VTable");
        g_telemetry_vtable.store(nullptr, std::memory_order_release);
        return;
    }
...
void DenseCoreEntRegisterLicense(const DenseCoreEntLicenseVTable* vtable) {
    constexpr size_t required = offsetof(DenseCoreEntLicenseVTable, is_valid) + sizeof(DenseCoreEntIsLicenseValidFn);
    if (!ValidABIHeader(vtable, required) || !vtable->validate || !vtable->set_state || !vtable->get_state || !vtable->is_valid) {
        ENT_LOG_ERROR("Rejected incompatible license VTable");
        g_license_vtable.store(nullptr, std::memory_order_release);
        return;
    }
...
void DenseCoreEntRegisterNumaRouting(const DenseCoreEntNumaRoutingVTable* vtable) {
    constexpr size_t required = offsetof(DenseCoreEntNumaRoutingVTable, get_policy) + sizeof(DenseCoreEntGetNumaPolicyFn);
    if (!ValidABIHeader(vtable, required) || !vtable->resolve_node || !vtable->get_policy) {
        ENT_LOG_ERROR("Rejected incompatible NUMA routing VTable");
        g_numa_routing_vtable.store(nullptr, std::memory_order_release);
        return;
    }
...
void DenseCoreEntRegisterConfidential(const DenseCoreEntConfidentialVTable* vtable) {
    constexpr size_t required = offsetof(DenseCoreEntConfidentialVTable, is_ready) + sizeof(DenseCoreEntIsConfidentialReadyFn);
    if (!ValidABIHeader(vtable, required) || !vtable->set_state || !vtable->get_state || !vtable->is_ready) {
        ENT_LOG_ERROR("Rejected incompatible confidential VTable");
        g_confidential_vtable.store(nullptr, std::memory_order_release);
        return;
    }
...
void DenseCoreEntRegisterQuota(const DenseCoreEntQuotaVTable* vtable) {
    constexpr size_t required = offsetof(DenseCoreEntQuotaVTable, get_active_requests) + sizeof(DenseCoreEntQuotaGetActiveRequestsFn);
    if (!ValidABIHeader(vtable, required) || !vtable->try_begin_request || !vtable->end_request ||
        !vtable->record_tokens || !vtable->get_active_requests) {
        ENT_LOG_ERROR("Rejected incompatible quota VTable");
        g_quota_vtable.store(nullptr, std::memory_order_release);
        return;
    }
