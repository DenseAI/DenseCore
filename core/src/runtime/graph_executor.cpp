...
inline bool DeclaresShape(const Tensor& t) {
    return t.ndim > 0;
}
...
void ValidateTensorIndex(size_t idx, size_t tensor_count, const GraphNode& node, const char* role) {
    if (idx < tensor_count) {
        return;
    }
    std::ostringstream oss;
    oss << "Graph node '" << node.name << "' has out-of-range " << role << " tensor index " << idx
        << " (tensor_count=" << tensor_count << ")";
    throw InvalidArgumentException(oss.str());
}

void ValidateReferencedTensorShape(const Tensor& tensor, size_t idx, const GraphNode& node, const char* role) {
    if (!DeclaresShape(tensor)) {
        return;
    }
    if (tensor.HasValidShape()) {
        return;
    }
    std::ostringstream oss;
    oss << "Graph node '" << node.name << "' references " << role << " tensor index " << idx
        << " with non-positive or invalid shape";
    throw InvalidArgumentException(oss.str());
}
...
void GraphExecutor::ValidateGraph(const OperationGraph& graph) const {
    const size_t tensor_count = graph.TensorCount();
    for (size_t i = 0; i < graph.NodeCount(); ++i) {
        const GraphNode& node = graph.GetNode(i);
        for (size_t idx : node.inputs) {
            ValidateTensorIndex(idx, tensor_count, node, "input");
            ValidateReferencedTensorShape(graph.GetTensor(idx), idx, node, "input");
        }
        for (size_t idx : node.outputs) {
            ValidateTensorIndex(idx, tensor_count, node, "output");
            ValidateReferencedTensorShape(graph.GetTensor(idx), idx, node, "output");
        }
    }
}
