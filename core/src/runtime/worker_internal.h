...
int ResolveFinalWorkerThreadCap(const TransformerModel* model, int physical_core_count, int base_threads,
                                bool respect_configured_threads = false);
...
static constexpr std::size_t kDecodePhysicalBatchHistCount = 6;
...
struct DecodeWorkerStats {
    ...
    std::array<std::atomic<uint64_t>, kDecodePhysicalBatchHistCount> built_batch_width_hist{};
    std::array<std::atomic<uint64_t>, kDecodePhysicalBatchHistCount> executed_batch_width_hist{};
    std::array<std::atomic<uint64_t>, kDecodePhysicalBatchHistCount> rejected_batch_width_hist{};
...
bool ResolveSamplingLogitsColumnForRequest(int token_offset, int processed_count, int output_columns,
                                           ...
                                           int sequence_index = -1, int num_sequences = 0);
