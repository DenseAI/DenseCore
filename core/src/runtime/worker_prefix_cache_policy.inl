...
bool IsQwen35HybridSSMModel(const TransformerModel* model) {
    if (!model || !model->arch_flags.is_hybrid_ssm) {
        return false;
    }
    return densecore::models::DescribeModel(model).variant == ModelVariant::QWEN35;
}

bool IsQwen38HybridSSMModel(const TransformerModel* model) {
    if (!model || !model->arch_flags.is_hybrid_ssm) {
        return false;
    }
    return densecore::models::DescribeModel(model).variant == ModelVariant::QWEN38;
}
...
bool IsPrefixCacheAllowedForModel(const TransformerModel* model) {
    ...
    if (IsQwen35HybridSSMModel(model)) {
        return false;
    }
    if (IsQwen38HybridSSMModel(model)) {
        return false;
    }
    return true;
}
...
const char* PrefixCacheSkipReasonForModel(const TransformerModel* model) {
    ...
    if (IsQwen35HybridSSMModel(model)) {
        return "qwen35_prefix_cache_unqualified";
    }
    if (IsQwen38HybridSSMModel(model)) {
        return "qwen38_prefix_cache_unqualified";
    }
    return "none";
}
