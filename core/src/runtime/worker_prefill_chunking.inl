...
int ResolveQwen36PrefillChunkTokensImpl(const TransformerModel* model, const Request* req) {
    ...
    const bool qwen38 = descriptor.variant == ModelVariant::QWEN38;
    ...
    const int hybrid_ssm_chunk_tokens =
        ...
        2048;
#endif
    const int base_chunk_tokens = qwen38 ? 128 : qwen35_dense ? 768 : ((qwen35_moe || qwen_hybrid_ssm) ? hybrid_ssm_chunk_tokens : 192);
    const int base_auto_min_tokens = qwen38 ? 128 : qwen35_dense ? 1024 : ((qwen35_moe || qwen_hybrid_ssm) ? 1280 : 1536);
    const int base_fail_closed_min_tokens = qwen38 ? 128 : qwen35_dense ? 1024 : ((qwen35_moe || qwen_hybrid_ssm) ? 1280 : 1536);
    const char* chunk_env = qwen38 ? "DENSECORE_QWEN38_PREFILL_CHUNK_TOKENS" : "DENSECORE_QWEN36_PREFILL_CHUNK_TOKENS";
    const char* default_env = qwen38 ? "DENSECORE_QWEN38_PREFILL_CHUNK_DEFAULT_TOKENS" : "DENSECORE_QWEN36_PREFILL_CHUNK_DEFAULT_TOKENS";
    const char* auto_min_env = qwen38 ? "DENSECORE_QWEN38_PREFILL_CHUNK_AUTO_MIN_TOKENS" : "DENSECORE_QWEN36_PREFILL_CHUNK_AUTO_MIN_TOKENS";
...
size_t GraphContextSafetyMarginBytes() {
    ...
    constexpr size_t kSafetyMarginMb = 256;
    return kSafetyMarginMb * 1024ULL * 1024ULL;
}
...
int SelectLargestModelPrefillChunkThatFits(const TransformerModel* model, size_t prompt_tokens, size_t available_bytes,
    ...
    constexpr int kQwenCandidates[] = {
        ...
        192, 160,
#else
        2048, 1536, 1024, 768, 512, 384, 320, 256, 192, 160,
#endif
        128, 96, 64, 48, 32};
