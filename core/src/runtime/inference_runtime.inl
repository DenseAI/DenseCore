...
void WriteInferenceWorkContextLFM2GreedyLMHeadSparseLogits(InferenceWorkContext* ctx, float* output, int vocab_size,
    ...
    const bool can_reuse_sparse_buffer =
        ctx && ctx->lfm2_greedy_lm_head_sparse_logits_ptr == output &&
        ctx->lfm2_greedy_lm_head_sparse_logits_vocab_size == vocab_size &&
        ctx->lfm2_greedy_lm_head_sparse_logits_token >= 0 &&
        ctx->lfm2_greedy_lm_head_sparse_logits_token < vocab_size;
    if (can_reuse_sparse_buffer) {
        output[ctx->lfm2_greedy_lm_head_sparse_logits_token] = -std::numeric_limits<float>::infinity();
    } else {
    ...
    if (ctx) {
        ctx->lfm2_greedy_lm_head_sparse_logits_ptr = output;
        ctx->lfm2_greedy_lm_head_sparse_logits_vocab_size = vocab_size;
        ctx->lfm2_greedy_lm_head_sparse_logits_token = token;
    }
}
...
void RecordInferenceWorkContextLFM2GreedyLMHeadArgmaxToken(InferenceWorkContext* ctx, uint64_t generation,
    ...
    ctx->lfm2_greedy_lm_head_argmax_generation = generation;
    ctx->lfm2_greedy_lm_head_argmax_token = token;
    ctx->lfm2_greedy_lm_head_argmax_value = value;
    ctx->lfm2_greedy_lm_head_argmax_vocab_size = vocab_size;
}

bool TryGetInferenceWorkContextLFM2GreedyLMHeadArgmaxToken(const InferenceWorkContext* ctx, int vocab_size,
                                                           int* token, float* value) {
    if (!ctx || !token || !value || vocab_size <= 0 || !ctx->lfm2_greedy_lm_head_argmax_allowed ||
        ctx->lfm2_greedy_lm_head_argmax_generation != ctx->execution_generation ||
        ctx->lfm2_greedy_lm_head_argmax_vocab_size != vocab_size ||
        ctx->lfm2_greedy_lm_head_argmax_token < 0 ||
        ctx->lfm2_greedy_lm_head_argmax_token >= vocab_size ||
        !std::isfinite(ctx->lfm2_greedy_lm_head_argmax_value)) {
        return false;
    }
