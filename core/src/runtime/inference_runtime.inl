...
void GatherTokenHeadContiguous(const struct ggml_tensor* src, int token_idx, int head_dim, int n_head_kv,
                                             float* out) {
...
void WriteCurrentBatchKvToCache(const BatchSpec* batch, const struct ggml_tensor* src,
...
void cb_kv_update_and_gather_custom(struct ggml_tensor* dst, int ith, int nth, void* userdata) {
    ...
    const char* debug_shared_env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_GEMMA4_SHARED_KV");
