...
ResolvedModelDescriptor ResolveModelDescriptorFromArchName(std::string_view arch_name) {
    ...
    if (MatchesAny(lowered,
                   std::array<std::string_view, 10>{"qwen38", "qwen3.8", "qwen3_8", "qwen3-8", "qwen3.8-27b",
                                                    "qwen3_8_27b", "qwen3-8-27b", "qwen38_27b",
                                                    "qwen38-27b", "qwen3.8-27b-instruct"})) {
        return make_result(DescribeModelVariant(ModelVariant::QWEN38));
    }
    if (MatchesAny(lowered,
...
TokenizerFamily ResolveTokenizerFamilyFromMetadata(std::string_view tokenizer_type) {
    ...
    if (lowered.find("qwen38") != std::string::npos || lowered.find("qwen3.8") != std::string::npos ||
        lowered.find("qwen36") != std::string::npos || lowered.find("qwen3.6") != std::string::npos) {
...
bool IsKnownTokenizerModel(std::string_view tokenizer_name) {
    ...
    static constexpr std::array<std::string_view, 28> kKnown = {
        "llama",
        "gpt2",
        "qwen2",
        "qwen2.5",
...
const char* TokenizerFamilyName(TokenizerFamily family) {
    ...
    case TokenizerFamily::BERT_BPE: return "bert_bpe";
    case TokenizerFamily::UNKNOWN:
    default: return "unknown";
    }
}
