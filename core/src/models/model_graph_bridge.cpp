...
public:
    ...
    std::unique_ptr<OperationGraph> Build(const std::vector<Tensor>& inputs,
        ...
        model_->ForEachWeightContext([&](ggml_context* weight_ctx) {
            struct ggml_tensor* t = ggml_get_first_tensor(weight_ctx);
            while (t) {
...
public:
    ...
    std::unique_ptr<OperationGraph> Build(const std::vector<Tensor>& inputs,
                                          const std::string& /*variant_name*/) override {
        if (inputs.size() < 2) {
            std::cerr << "[OpenVlaContractGraphBuilder][experimental] Expected image + text inputs for scaffold path"
                      << std::endl;
            return nullptr;
        }
...
bool ModelGraphBridge::RegisterLlmBuilder(const TransformerModel* model) {
    ...
    GraphRegistry::Instance().Register("openvla_contract", [model]() -> std::unique_ptr<GraphBuilder> {
        return std::make_unique<OpenVlaContractGraphBuilder>(model);
    });
    std::cout << "[ModelGraphBridge] Registered GenericLlmBuilder for llm_generic/llm_universal and "
                 "experimental OpenVlaContractGraphBuilder scaffold for openvla_contract"
              << std::endl;
    return true;
}
