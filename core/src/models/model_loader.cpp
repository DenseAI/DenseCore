...
bool AlignUpSize(size_t value, size_t alignment, size_t* result) {
    if (!result || alignment == 0) {
        return false;
    }
    const size_t remainder = value % alignment;
    return remainder == 0 ? (*result = value, true) : CheckedAddSize(value, alignment - remainder, result);
}

bool GetGgufNonNegativeInteger(const gguf_context* ctx, const char* key, uint64_t* value) {
    if (!ctx || !key || !value) {
        return false;
    }
    ...
    case GGUF_TYPE_UINT8: *value = gguf_get_val_u8(ctx, index); return true;
    case GGUF_TYPE_UINT16: *value = gguf_get_val_u16(ctx, index); return true;
    case GGUF_TYPE_UINT32: *value = gguf_get_val_u32(ctx, index); return true;
    case GGUF_TYPE_UINT64: *value = gguf_get_val_u64(ctx, index); return true;
    case GGUF_TYPE_INT8: {
        const int8_t v = gguf_get_val_i8(ctx, index);
        if (v < 0) return false;
        *value = static_cast<uint64_t>(v);
        return true;
    }
    case GGUF_TYPE_INT16: {
        const int16_t v = gguf_get_val_i16(ctx, index);
        if (v < 0) return false;
        *value = static_cast<uint64_t>(v);
        return true;
    }
    case GGUF_TYPE_INT32: {
        const int32_t v = gguf_get_val_i32(ctx, index);
        if (v < 0) return false;
        *value = static_cast<uint64_t>(v);
        return true;
    }
    case GGUF_TYPE_INT64: {
        const int64_t v = gguf_get_val_i64(ctx, index);
        if (v < 0) return false;
        *value = static_cast<uint64_t>(v);
        return true;
    }
...
bool ResolveGgufShardPaths(const std::string& input_path, const gguf_context* ctx, std::vector<std::string>* paths,
                           uint64_t* input_shard_index, std::string* error) {
    if (!paths || !input_shard_index || !error) {
        return false;
    }
    paths->clear();
    *input_shard_index = 0;
    ...
    if (!GetGgufNonNegativeInteger(ctx, "split.count", &shard_count) || shard_count <= 1) {
        paths->push_back(input_path);
        return true;
    }
    uint64_t shard_index = 0;
    if (!GetGgufNonNegativeInteger(ctx, "split.no", &shard_index) || shard_index >= shard_count) {
        *error = "split GGUF has invalid or missing split.no metadata";
        return false;
    }

    const std::regex split_name(R"(^(.*)-([0-9]+)-of-([0-9]+)(\.gguf)$)", std::regex::icase);
    std::smatch match;
    if (!std::regex_match(input_path, match, split_name)) {
        *error = "split GGUF filename must end in -00001-of-000NN.gguf";
        return false;
    }

    uint64_t filename_index = 0;
    uint64_t filename_count = 0;
    try {
        filename_index = std::stoull(match[2].str());
        filename_count = std::stoull(match[3].str());
    } catch (const std::exception&) {
        *error = "split GGUF filename contains an invalid shard number";
        return false;
    }
    if (filename_index != shard_index + 1 || filename_count != shard_count) {
        *error = "split GGUF filename disagrees with split.no/split.count metadata";
        return false;
    }

    const std::string prefix = match[1].str();
    const std::string extension = match[4].str();
    const int index_width = static_cast<int>(match[2].length());
    const int count_width = static_cast<int>(match[3].length());
    paths->reserve(static_cast<size_t>(shard_count));
    for (uint64_t i = 0; i < shard_count; ++i) {
        std::ostringstream path;
        path << prefix << '-' << std::setfill('0') << std::setw(index_width) << (i + 1) << "-of-"
             << std::setw(count_width) << shard_count << extension;
...
TransformerModel::CpuRepackAliasLayout InferCpuRepackAliasLayout(const ggml_tensor* alias) {
    if (!alias) {
        return TransformerModel::CpuRepackAliasLayout::Unknown;
    }
    if (alias->type == GGML_TYPE_Q4_K) {
...
void RegisterCpuRepackAlias(
    ...
    auto existing = model->prepared_weights.cpu_repack_aliases.find(source);
    if (existing != model->prepared_weights.cpu_repack_aliases.end() && existing->second) {
        model->prepared_weights.cpu_repack_alias_sources.erase(existing->second);
        model->prepared_weights.cpu_repack_alias_layouts.erase(existing->second);
    }
    model->prepared_weights.cpu_repack_aliases[source] = alias;
    model->prepared_weights.cpu_repack_alias_sources[alias] = source;
    model->prepared_weights.cpu_repack_alias_layouts[alias] =
        layout == TransformerModel::CpuRepackAliasLayout::Unknown ? InferCpuRepackAliasLayout(alias) : layout;
}
...
void EraseCpuRepackAlias(TransformerModel* model, const ggml_tensor* source) {
    ...
    auto it = model->prepared_weights.cpu_repack_aliases.find(source);
    if (it != model->prepared_weights.cpu_repack_aliases.end()) {
        if (it->second) {
            model->prepared_weights.cpu_repack_alias_sources.erase(it->second);
            model->prepared_weights.cpu_repack_alias_layouts.erase(it->second);
        }
        model->prepared_weights.cpu_repack_aliases.erase(it);
    }
}
...
void ClearCpuRepackAliases(TransformerModel* model) {
    ...
    model->prepared_weights.cpu_repack_aliases.clear();
    model->prepared_weights.cpu_repack_alias_sources.clear();
    model->prepared_weights.cpu_repack_alias_layouts.clear();
}
...
void PrepareGenericCpuFastMatmulAliases(TransformerModel* model) {
    ...
    auto repack_q5k_8x8_in_place = [&](ggml_tensor* tensor, size_t* out_bytes) -> bool {
        ...
        if (experts <= 0 || row_bytes == 0 || !CheckedMulSize(static_cast<size_t>(rows), row_bytes, &expert_bytes) ||
            expert_bytes == 0 || !CheckedMulSize(expert_bytes, static_cast<size_t>(experts), &packed_bytes) ||
            row_bytes > static_cast<size_t>(std::numeric_limits<int64_t>::max()) ||
            expert_bytes > static_cast<size_t>(std::numeric_limits<int64_t>::max()) ||
            tensor->nb[1] != static_cast<int64_t>(row_bytes) || tensor->nb[2] < static_cast<int64_t>(expert_bytes)) {
            return false;
        }
        std::vector<uint8_t> expert_scratch;
        try {
            expert_scratch.resize(expert_bytes);
        ...
        for (int64_t expert = 0; expert < experts; ++expert) {
            ...
            if (ggml_repack_q5_K_8x8(src, expert_bytes, rows, cols, expert_scratch.data(), expert_bytes) != 0) {
                return false;
            }
            std::memcpy(dst_base + static_cast<size_t>(expert) * static_cast<size_t>(tensor->nb[2]),
                        expert_scratch.data(), expert_bytes);
        }
        model->prepared_weights.q5k_8x8_repacked_tensors.insert(tensor);
        if (out_bytes) {
    ...
    auto prepare_native_moe_gateup_q5k_8x8 = [&]() {
        const bool qwen_hybrid_moe =
            (model->variant == ModelVariant::QWEN35 || model->variant == ModelVariant::QWEN36) &&
            model->arch_flags.is_hybrid_ssm;
        const bool lfm2_shortconv_moe = model->variant == ModelVariant::LFM2MOE && model->arch_flags.is_lfm2_shortconv;
        if (!qwen_hybrid_moe && !lfm2_shortconv_moe) {
            return;
        }
        size_t candidates = 0;
        size_t prepared = 0;
        ...
        for (auto& layer : model->layers) {
            ...
            for (ggml_tensor* tensor : {gate, up}) {
                ...
                ++candidates;
                if (model->prepared_weights.q5k_8x8_repacked_tensors.find(tensor) != model->prepared_weights.q5k_8x8_repacked_tensors.end()) {
                    continue;
                }
        ...
        if (candidates > 0) {
            std::cout << "[DenseCore] native MoE gate/up Q5_K single-copy 8x8 repack: candidates=" << candidates
                      << ", prepared=" << prepared << ", bytes=" << (bytes / 1024 / 1024) << " MiB" << std::endl;
        }
    };

    prepare_native_moe_gateup_q5k_8x8();
    ...
    auto make_alias = [&](ggml_tensor* source, const char* suffix) {
        ...
        if (model->prepared_weights.cpu_repack_aliases.find(source) != model->prepared_weights.cpu_repack_aliases.end()) {
            return;
        }
        ...
        ggml_context** ctx = use_amx ? &model->prepared_weights.ctx_cpu_amx : &model->prepared_weights.ctx_cpu_repack;
    ...
    auto make_decode_alias = [&](ggml_tensor* source, const char* suffix) {
        ...
        if (model->prepared_weights.cpu_decode_repack_aliases.find(source) != model->prepared_weights.cpu_decode_repack_aliases.end()) {
            return;
        }
        if (!init_alias_context(&model->prepared_weights.ctx_cpu_repack, "CPU_REPACK")) {
            return;
        }
        ggml_tensor* alias = ggml_new_tensor_2d(model->prepared_weights.ctx_cpu_repack, source->type, source->ne[0], source->ne[1]);
        if (!alias) {
            return;
        }
        const std::string name = std::string(source->name[0] ? source->name : "weight") + suffix;
        ggml_set_name(alias, name.c_str());
        model->prepared_weights.cpu_decode_repack_aliases[source] = alias;
    ...
    auto make_qwen35_ssm_qkv_gate_amx_fused_alias = [&](TransformerLayer& layer, uint32_t layer_idx) {
        ...
        if (!init_alias_context(&model->prepared_weights.ctx_cpu_amx, "AMX")) {
            return;
        }
        const int64_t fused_rows = qkv->ne[1] + gate->ne[1];
        ggml_tensor* alias = ggml_new_tensor_2d(model->prepared_weights.ctx_cpu_amx, qkv->type, qkv->ne[0], fused_rows);
        if (!alias) {
            return;
        }
        char name[128];
    ...
    auto make_qwen_hybrid_ssm_qkv_gate_cpu_fused_alias = [&](TransformerLayer& layer, uint32_t layer_idx) {
        ...
        const int64_t fused_rows = qkv->ne[1] + gate->ne[1];
        ggml_tensor* alias = ggml_new_tensor_2d(model->prepared_weights.ctx_cpu_repack, qkv->type, qkv->ne[0], fused_rows);
        if (!alias) {
            return;
        }
        char name[128];
    ...
    auto commit_plain_cpu_fused_alias = [&](PendingFusedAlias& item) -> bool {
        ...
        ggml_tensor* alias = ggml_new_tensor(model->prepared_weights.ctx_cpu_repack, item.alias->type, GGML_MAX_DIMS, item.alias->ne);
        if (!alias) {
            return false;
        }
        ...
        model->prepared_weights.host_buffers.push_back({data, item.bytes.size(), densecore::AllocationType::Aligned});
        model->prepared_weights.cpu_repack_buffers.push_back(buffer);
        item.layer->Set(item.key, alias);
        MarkLayerTensorRole(model, item.layer, item.key.c_str(),
    ...
    auto allocate_and_commit_aliases = [&](std::vector<PendingAlias>& pending, ggml_context* ctx,
        ...
        if (!buffer) {
            ...
            for (const auto& item : pending) {
                if (item.source) {
                    if (item.decode_only) {
                        model->prepared_weights.cpu_decode_repack_aliases.erase(item.source);
                    } else {
    ...
    allocate_and_commit_aliases(pending_cpu_amx, model->prepared_weights.ctx_cpu_amx, pending_fused_cpu_amx, repack_bufts.cpu_amx,
                                "AMX");
    allocate_and_commit_aliases(pending_cpu_repack, model->prepared_weights.ctx_cpu_repack, pending_fused_cpu_repack,
...
void PrepareGemma4CpuRepackAliases(TransformerModel* model) {
    ...
    if (!primary_repack_buft && !repack_bufts.cpu_kleidiai) {
        if (densecore::env::ParseDiagnosticEnv("DENSECORE_DEBUG_GEMMA4_CPU_REPACK")) {
    ...
    if (!init_repack_context(&model->prepared_weights.ctx_cpu_repack, "CPU_REPACK")) {
        return;
    }
    const bool allow_gemma4_kleidiai_aliases = false;
    if (allow_gemma4_kleidiai_aliases && repack_bufts.cpu_kleidiai &&
        !init_repack_context(&model->prepared_weights.ctx_cpu_kleidiai, "CPU_KLEIDIAI")) {
        return;
    }
    ...
    auto use_kleidiai_for_type = [&](ggml_type type) -> bool {
        ...
        return repack_bufts.cpu_kleidiai && model->prepared_weights.ctx_cpu_kleidiai && CanUseCpuKleidiaiRepack(type);
    };

    auto context_for_type = [&](ggml_type type) -> ggml_context* {
        return use_kleidiai_for_type(type) ? model->prepared_weights.ctx_cpu_kleidiai : model->prepared_weights.ctx_cpu_repack;
    };
    ...
    if (cpu_repack_aliases > 0) {
        ggml_backend_buffer_t buffer =
            ggml_backend_alloc_ctx_tensors_from_buft(model->prepared_weights.ctx_cpu_repack, primary_repack_buft);
        if (!buffer) {
    ...
    if (cpu_kleidiai_aliases > 0) {
        ggml_backend_buffer_t buffer =
            ggml_backend_alloc_ctx_tensors_from_buft(model->prepared_weights.ctx_cpu_kleidiai, repack_bufts.cpu_kleidiai);
        if (!buffer) {
...
bool ResolveModelLoadMemoryPlan(size_t weight_tensor_count, size_t layer_count, size_t moe_layer_count,
                                size_t expert_count, ModelLoadMemoryPlan* plan, std::string* error) {
    if (!plan) {
        if (error) *error = "memory plan output is null";
        return false;
    }

    constexpr size_t kBaseViewSlots = 128;
    constexpr size_t kAuxiliaryViewsPerWeight = 2;
    // Current packed INT4/Gemma4 materialization uses at most 11 metadata tensors
    // per expert. Keep one slot of topology-derived headroom without a model-size cap.
    constexpr size_t kViewsPerExpertLayer = 12;
    constexpr size_t kContextSlackBytes = size_t{1} * 1024 * 1024;
    constexpr size_t kContextAlignment = size_t{64} * 1024;

    const size_t effective_moe_layers = expert_count > 0 && moe_layer_count == 0 ? layer_count : moe_layer_count;
    size_t auxiliary_slots = 0;
    size_t expert_layer_count = 0;
    size_t expert_slots = 0;
    size_t view_slots = 0;
    size_t metadata_bytes = 0;
    size_t context_bytes = 0;
    if (!CheckedMulSize(weight_tensor_count, kAuxiliaryViewsPerWeight, &auxiliary_slots) ||
        !CheckedMulSize(effective_moe_layers, expert_count, &expert_layer_count) ||
        !CheckedMulSize(expert_layer_count, kViewsPerExpertLayer, &expert_slots) ||
        !CheckedAddSize(kBaseViewSlots, auxiliary_slots, &view_slots) ||
        !CheckedAddSize(view_slots, expert_slots, &view_slots) ||
        !CheckedMulSize(view_slots, ggml_tensor_overhead(), &metadata_bytes) ||
        !CheckedAddSize(metadata_bytes, kContextSlackBytes, &context_bytes) ||
        !AlignUpSize(context_bytes, kContextAlignment, &context_bytes)) {
        if (error) *error = "model topology overflows loader view-memory accounting";
        return false;
    }

    *plan = ModelLoadMemoryPlan{
        weight_tensor_count, layer_count, effective_moe_layers, expert_count, view_slots, context_bytes,
    };
...
TransformerModel* LoadGGUFModel(const char* path) {
    ...
    uint64_t input_shard_index = 0;
    std::string shard_error;
    if (!ResolveGgufShardPaths(path, ctx_gguf, &shard_paths, &input_shard_index, &shard_error)) {
        return fail_load(shard_error);
    }
    if (shard_paths.size() > 1 && input_shard_index != 0) {
        const std::string first_shard = shard_paths.front();
        std::cout << "[DenseCore] Redirecting split GGUF load to first shard '" << first_shard << "'" << std::endl;
        delete model;
        return LoadGGUFModel(first_shard.c_str());
    }
    if (shard_paths.size() > 1) {
        uint64_t expected_tensor_count = 0;
        const bool has_expected_tensor_count =
            GetGgufNonNegativeInteger(ctx_gguf, "split.tensors.count", &expected_tensor_count);
        uint64_t loaded_tensor_count = static_cast<uint64_t>(gguf_get_n_tensors(ctx_gguf));
        std::unordered_set<std::string> tensor_names;
        for (int64_t i = 0; i < gguf_get_n_tensors(ctx_gguf); ++i) {
            tensor_names.emplace(gguf_get_tensor_name(ctx_gguf, i));
        }

        for (size_t shard = 1; shard < shard_paths.size(); ++shard) {
            ggml_context* shard_ctx_w = nullptr;
            gguf_init_params shard_params{
                /*.no_alloc=*/false,
                /*.ctx=*/&shard_ctx_w,
            };
            gguf_context* shard_ctx_gguf = gguf_init_from_file(shard_paths[shard].c_str(), shard_params);
            if (!shard_ctx_gguf || !shard_ctx_w) {
                if (shard_ctx_w) ggml_free(shard_ctx_w);
                if (shard_ctx_gguf) gguf_free(shard_ctx_gguf);
                return fail_load("failed to load split GGUF shard '" + shard_paths[shard] + "'");
            }

            uint64_t shard_number = 0;
            uint64_t shard_count = 0;
            if (!GetGgufNonNegativeInteger(shard_ctx_gguf, "split.no", &shard_number) || shard_number != shard ||
                !GetGgufNonNegativeInteger(shard_ctx_gguf, "split.count", &shard_count) ||
                shard_count != shard_paths.size()) {
                ggml_free(shard_ctx_w);
                gguf_free(shard_ctx_gguf);
                return fail_load("split GGUF shard metadata mismatch in '" + shard_paths[shard] + "'");
            }

            const int64_t shard_tensor_count = gguf_get_n_tensors(shard_ctx_gguf);
            for (int64_t i = 0; i < shard_tensor_count; ++i) {
                const char* name = gguf_get_tensor_name(shard_ctx_gguf, i);
                if (!name || !tensor_names.emplace(name).second) {
                    ggml_free(shard_ctx_w);
                    gguf_free(shard_ctx_gguf);
                    return fail_load(std::string("duplicate tensor across split GGUF shards: ") +
                                     (name ? name : "<unnamed>"));
                }
            }
            loaded_tensor_count += static_cast<uint64_t>(shard_tensor_count);
            model->ctx_w_shards.push_back(shard_ctx_w);
            model->ctx_gguf_shards.push_back(shard_ctx_gguf);
        }

        if (has_expected_tensor_count && loaded_tensor_count != expected_tensor_count) {
            return fail_load("split GGUF tensor count mismatch: expected " + std::to_string(expected_tensor_count) +
                             ", loaded " + std::to_string(loaded_tensor_count));
        }
        std::cout << "[DenseCore] Loaded split GGUF: " << shard_paths.size() << " shards, " << loaded_tensor_count
                  << " tensors" << std::endl;
    }
    ...
    detection_hints.has_gemma4_tensor_signatures = model->FindWeightTensor("per_layer_model_proj.weight") != nullptr ||
                                                   model->FindWeightTensor("per_layer_proj_norm.weight") != nullptr ||
                                                   model->FindWeightTensor("per_layer_token_embd.weight") != nullptr ||
                                                   model->FindWeightTensor("blk.0.inp_gate.weight") != nullptr;
    ...
    const bool qwen38_named_variant = contains_lower(general_name, "qwen3.8") ||
                                      contains_lower(general_basename, "qwen3.8") ||
                                      contains_lower(general_repo, "qwen3.8") ||
                                      contains_lower(general_hf_repo, "qwen3.8") || contains_lower(hf_repo, "qwen3.8");
    const bool qwen35_family_aux =
        qwen35_architecture || qwen35_hf_model_type || qwen35_hf_architecture || qwen36_named_variant ||
        qwen38_named_variant;
    ...
    } else if (contains_lower(general_name, "qwen3.8")) {
        qwen35_resolution_signal = "general.name contains Qwen3.8";
    } else if (contains_lower(general_basename, "qwen3.8")) {
        qwen35_resolution_signal = "general.basename contains Qwen3.8";
    } else if (contains_lower(general_repo, "qwen3.8")) {
        qwen35_resolution_signal = "general.repo contains Qwen3.8";
    } else if (contains_lower(general_hf_repo, "qwen3.8")) {
        qwen35_resolution_signal = "general.hf_repo_id contains Qwen3.8";
    } else if (contains_lower(hf_repo, "qwen3.8")) {
        qwen35_resolution_signal = "hf.repo_id contains Qwen3.8";
    }
    if (!resolved_arch.known && qwen35_family_aux) {
        resolved_arch =
            densecore::models::ResolveModelDescriptor(qwen38_named_variant ? "qwen38"
                                                    : qwen36_named_variant ? "qwen36"
                                                                           : "qwen35");
    } else if (resolved_arch.known && resolved_arch.arch == ModelArch::QWEN35 && qwen38_named_variant) {
        resolved_arch = densecore::models::ResolveModelDescriptor("qwen38");
    ...
    auto has_tensor_with_prefix = [&](const std::string& prefix) -> bool {
        bool found = false;
        model->ForEachWeightContext([&](ggml_context* weight_ctx) {
            if (found) {
                return;
            }
            for (ggml_tensor* tensor = ggml_get_first_tensor(weight_ctx); tensor;
                 tensor = ggml_get_next_tensor(weight_ctx, tensor)) {
                const char* name = tensor ? tensor->name : nullptr;
    ...
    model->gguf_declared_layer_count = model->hparams.n_layer;
    ...
    get_u32("feed_forward_length", model->hparams.n_ff);
    get_u32("ffn_intermediate_size", model->hparams.n_ff);

    if (model->variant == ModelVariant::QWEN38) {
        uint32_t nextn_layers = 0;
        get_u32("nextn_predict_layers", nextn_layers);
        if (model->gguf_declared_layer_count != 65 || nextn_layers != 1) {
            return fail_load("Qwen3.8 text support requires qwen35.block_count=65 with nextn_predict_layers=1");
        }
        const uint32_t executable_layers = model->gguf_declared_layer_count - nextn_layers;
        if (executable_layers != 64) {
            return fail_load("Qwen3.8 text support requires exactly 64 executable trunk layers");
        }
        const std::string mtp_prefix = "blk." + std::to_string(executable_layers) + ".";
        const std::string after_mtp_prefix = "blk." + std::to_string(executable_layers + nextn_layers) + ".";
        if (!has_tensor_with_prefix(mtp_prefix)) {
            return fail_load("Qwen3.8 GGUF is missing the expected trailing blk.64 NextN/MTP tensors");
        }
        if (has_tensor_with_prefix(after_mtp_prefix)) {
            return fail_load("Qwen3.8 GGUF has unexpected tensors after the single trailing NextN/MTP layer");
        }
        model->hparams.n_layer = executable_layers;
        model->skipped_mtp_layer_count = nextn_layers;
        std::cout << "[DenseCore] Qwen3.8 text-only load: gguf_layers=" << model->gguf_declared_layer_count
                  << " executable_trunk_layers=" << model->hparams.n_layer
                  << " skipped_mtp_layers=" << model->skipped_mtp_layer_count << std::endl;
    }
    ...
    if (model->arch_flags.is_hybrid_ssm) {
        ...
        if (!layer_types.empty() && layer_types.size() != static_cast<size_t>(model->hparams.n_layer) &&
            layer_types.size() != static_cast<size_t>(model->gguf_declared_layer_count)) {
        ...
        if (!layer_types.empty()) {
            ...
            const int full_attention_phase = 3;
        ...
        if (model->variant == ModelVariant::QWEN36 || model->variant == ModelVariant::QWEN38) {
            struct QwenFamilyExpectedProfile {
                const char* name = "dense-27b";
                int hidden_size = 5120;
                int n_layer = 64;
                int all_layer = 64;
                int skipped_mtp_layers = 0;
                int ffn_size = 0;
            ...
            const bool qwen_dense_profile =
                model->hparams.n_experts == 0 && model->hparams.n_experts_used == 0 && model->moe_n_shared_experts == 0;
            const QwenFamilyExpectedProfile expected = [&]() {
                QwenFamilyExpectedProfile profile;
                if (model->variant == ModelVariant::QWEN38) {
                    profile.name = "qwen3.8-27b-text";
                    profile.all_layer = 65;
                    profile.skipped_mtp_layers = 1;
                    profile.ffn_size = 17408;
                    profile.head_dim = 256;
                    return profile;
                }
                if (qwen_dense_profile) return profile;
                profile.name = "moe-35b-a3b";
                profile.hidden_size = 2048;
                profile.n_layer = 40;
                profile.all_layer = 40;
            ...
            const bool strict_qwen_profile = model->variant == ModelVariant::QWEN38;
#ifdef DENSECORE_TEST_BUILD
            const bool allow_qwen38_test_profile = model->variant == ModelVariant::QWEN38 &&
                                                   has_exact_key("densecore.test.allow_small_qwen38_fixture");
#else
            constexpr bool allow_qwen38_test_profile = false;
#endif
            constexpr bool allow_qwen36_test_profile = false;

            if (model->variant == ModelVariant::QWEN36 && !qwen_dense_profile && !allow_qwen36_test_profile) {
            ...
            if (static_cast<int>(model->gguf_declared_layer_count) != expected.all_layer) {
                mismatches.push_back("all_layer");
            }
            if (static_cast<int>(model->skipped_mtp_layer_count) != expected.skipped_mtp_layers) {
                mismatches.push_back("nextn_predict_layers");
            }
            if (expected.ffn_size > 0 && static_cast<int>(model->hparams.n_ff) != expected.ffn_size) {
                mismatches.push_back("feed_forward_length");
            }
            ...
            if (!mismatches.empty()) {
                const char* profile_family = model->variant == ModelVariant::QWEN38 ? "Qwen3.8" : "Qwen3.6";
                std::cerr << "[DenseCore] Warning: " << profile_family
                          << " profile metadata differs from the official " << expected.name << " text profile in: ";
                for (size_t i = 0; i < mismatches.size(); ++i) {
                    if (i != 0) std::cerr << ", ";
                    std::cerr << mismatches[i];
                }
                std::cerr << std::endl;
                if (strict_qwen_profile && !allow_qwen38_test_profile) {
                    return fail_hybrid_ssm("Qwen3.8 strict profile validation failed");
                }
            }
        }
    ...
    // Collect additional stop IDs for chat/instruct tokenizers (e.g. <|im_end|>, <|eot_id|>).
    {
        ...
        auto add_stop_key = [&](const char* key) {
            ...
            if (t == GGUF_TYPE_ARRAY) {
                ...
                for (int i = 0; i < n; ++i) {
                    switch (arr_type) {
                    case GGUF_TYPE_UINT8:
                        add_stop_id(static_cast<int32_t>(static_cast<const uint8_t*>(arr_data)[i]));
                        break;
                    case GGUF_TYPE_INT8:
                        add_stop_id(static_cast<int32_t>(static_cast<const int8_t*>(arr_data)[i]));
                        break;
                    case GGUF_TYPE_UINT16:
                        add_stop_id(static_cast<int32_t>(static_cast<const uint16_t*>(arr_data)[i]));
                        break;
                    case GGUF_TYPE_INT16:
                        add_stop_id(static_cast<int32_t>(static_cast<const int16_t*>(arr_data)[i]));
                        break;
                    case GGUF_TYPE_UINT32:
                        add_stop_id(static_cast<int32_t>(static_cast<const uint32_t*>(arr_data)[i]));
                        break;
                    case GGUF_TYPE_INT32: add_stop_id(static_cast<const int32_t*>(arr_data)[i]); break;
                    case GGUF_TYPE_UINT64:
                        add_stop_id(static_cast<int32_t>(static_cast<const uint64_t*>(arr_data)[i]));
                        break;
                    case GGUF_TYPE_INT64:
                        add_stop_id(static_cast<int32_t>(static_cast<const int64_t*>(arr_data)[i]));
                        break;
                    default: break;
                    }
                }
        ...
        const std::vector<std::string> stop_token_literals = {
            "<|im_end|>", "<|eot_id|>", "<|eom_id|>", "<|endoftext|>", "<turn|>", "<end_of_turn>",
        };
    ...
    // GGUF weight contexts have no capacity for loader-created tensor metadata.
    // Size the separate view context from the complete sharded tensor inventory
    // and resolved model topology instead of assuming one particular MoE size.
    size_t weight_tensor_count = 0;
    size_t inferred_expert_count = 0;
    std::unordered_set<size_t> moe_tensor_layers;
    const std::regex layer_name_re(R"(^blk\.([0-9]+)\.)");
    const std::regex individual_expert_re(R"((?:experts?|exps)[._]([0-9]+)[._])");
    model->ForEachWeightContext([&](ggml_context* weight_ctx) {
        for (ggml_tensor* tensor = ggml_get_first_tensor(weight_ctx); tensor;
             tensor = ggml_get_next_tensor(weight_ctx, tensor)) {
            ++weight_tensor_count;
            const std::string tensor_name = tensor->name;
            const std::string lower_name = ascii_lower_copy(tensor_name);
            const bool is_expert_tensor = lower_name.find("experts.") != std::string::npos ||
                                          lower_name.find(".experts_") != std::string::npos ||
                                          lower_name.find("_exps") != std::string::npos;
            if (!is_expert_tensor) {
                continue;
            }

            std::smatch layer_match;
            if (std::regex_search(tensor_name, layer_match, layer_name_re) && layer_match.size() >= 2) {
                try {
                    moe_tensor_layers.insert(static_cast<size_t>(std::stoull(layer_match[1].str())));
                } catch (const std::exception&) {
                    // Invalid layer indices are handled by the normal tensor mapping validation.
                }
            }
            if (tensor->ne[2] > 1) {
                inferred_expert_count = std::max(inferred_expert_count, static_cast<size_t>(tensor->ne[2]));
            }
            if (tensor->ne[3] > 1) {
                inferred_expert_count = std::max(inferred_expert_count, static_cast<size_t>(tensor->ne[3]));
            }
            std::smatch expert_match;
            if (std::regex_search(lower_name, expert_match, individual_expert_re) && expert_match.size() >= 2) {
                try {
                    const size_t expert_index = static_cast<size_t>(std::stoull(expert_match[1].str()));
                    if (expert_index < std::numeric_limits<size_t>::max()) {
                        inferred_expert_count = std::max(inferred_expert_count, expert_index + 1);
                    }
                } catch (const std::exception&) {
                    // Invalid expert indices are handled by the normal tensor mapping validation.
                }
            }
        }
    });

    const size_t metadata_expert_count = static_cast<size_t>(model->hparams.n_experts);
    const size_t effective_expert_count = std::max(metadata_expert_count, inferred_expert_count);
    if (metadata_expert_count > 0 && inferred_expert_count > 0 && metadata_expert_count != inferred_expert_count) {
        return fail_load("expert count mismatch between GGUF metadata (" + std::to_string(metadata_expert_count) +
                         ") and tensor layout (" + std::to_string(inferred_expert_count) + ")");
    }
    if (model->hparams.n_experts == 0 && effective_expert_count > 0) {
        if (effective_expert_count > std::numeric_limits<uint32_t>::max()) {
            return fail_load("inferred expert count exceeds the supported model metadata range");
        }
        model->hparams.n_experts = static_cast<uint32_t>(effective_expert_count);
        std::cout << "[DenseCore] Inferred expert count from tensors: " << effective_expert_count << std::endl;
    }

    const size_t first_moe_layer = static_cast<size_t>(std::max(0, model->moe_first_k_dense_replace));
    const size_t topology_moe_layers = effective_expert_count > 0 && first_moe_layer < model->hparams.n_layer
                                           ? static_cast<size_t>(model->hparams.n_layer) - first_moe_layer
                                           : 0;
    const size_t effective_moe_layers = std::max(moe_tensor_layers.size(), topology_moe_layers);
    ModelLoadMemoryPlan memory_plan{};
    std::string memory_plan_error;
    if (!ResolveModelLoadMemoryPlan(weight_tensor_count, model->hparams.n_layer, effective_moe_layers,
                                    effective_expert_count, &memory_plan, &memory_plan_error)) {
        return fail_load("failed to size loader view memory: " + memory_plan_error);
    }
    struct ggml_init_params view_params = {
        /*.mem_size   =*/memory_plan.view_context_bytes,
        /*.mem_buffer =*/nullptr,
        /*.no_alloc   =*/false,
    };
    model->ctx_views = ggml_init(view_params);
    if (!model->ctx_views) {
        return fail_load("failed to allocate " + std::to_string(memory_plan.view_context_bytes) +
                         " bytes for loader view metadata");
    }
    std::cout << "[DenseCore] Loader view memory plan: weights=" << memory_plan.weight_tensor_count
              << " layers=" << memory_plan.layer_count << " moe_layers=" << memory_plan.moe_layer_count
              << " experts=" << memory_plan.expert_count << " view_capacity=" << memory_plan.view_tensor_capacity
              << " bytes=" << memory_plan.view_context_bytes << std::endl;

    // 3. Initialize backend with error checking
    model->backend = ggml_backend_cpu_init();
    if (!model->backend) {
        return fail_load("failed to initialize CPU backend");
    }
    model->cpu_backend = model->backend;
    ...
    auto get_tensor = [&](const std::string& name) -> struct ggml_tensor* {
        return model->FindWeightTensor(name.c_str());
    };
    ...
    auto get_tensor_with_fallback = [&](const std::string& primary) -> struct ggml_tensor* {
        struct ggml_tensor* t = model->FindWeightTensor(primary.c_str());
        if (t) return t;
        ...
        if (primary.size() > suffix.size() &&
            ...
            t = model->FindWeightTensor(fallback.c_str());
        }
        return t;
    };
    ...
    auto populate_layer_tensors_from_gguf = [&]() {
        ...
        model->ForEachWeightContext([&](ggml_context* weight_ctx) {
            struct ggml_tensor* t = ggml_get_first_tensor(weight_ctx);
            while (t) {
    ...
    for (uint32_t i = 0; i < model->hparams.n_layer; ++i) {
        ...
        if (packed_gate_up && packed_down &&
            ...
            if (expert_count > 0 && layer.Get(model_keys::kMoeGate)) {
                ...
                struct ggml_context* vctx = model->ctx_views;
                for (int expert_idx = 0; expert_idx < expert_count; ++expert_idx) {
                    ...
                    if (!gate_up_slice) {
                        return fail_load("failed to materialize packed MoE gate/up view at layer " + std::to_string(i) +
                    ...
                    if (!gate_w || !up_w || !down_w) {
                        return fail_load("failed to materialize packed MoE projection views at layer " +
                                         std::to_string(i) + " expert " + std::to_string(expert_idx));
                    }
        ...
        if (layer.NumExperts() == 0 && layer.Get(model_keys::kMoeGate)) {
            ...
            if (sep_gate && sep_up && sep_down) {
                ...
                if (expert_count > 0 && i >= static_cast<uint32_t>(std::max(0, model->moe_first_k_dense_replace))) {
                    ...
                    struct ggml_context* vctx2 = model->ctx_views;
                    for (int ei = 0; ei < expert_count; ++ei) {
                        ...
                        if (!gw || !uw || !dw) {
                            return fail_load("failed to materialize stacked MoE expert views at layer " +
                                             std::to_string(i) + " expert " + std::to_string(ei));
                        }
    ...
    if (model->arch_flags.is_hybrid_ssm) {
        ...
        auto canonicalize_fused_ba = [&](const char* label, struct ggml_tensor* t, std::vector<float>& beta_out,
            ...
            if (Qwen35CanonicalizeFusedBA(raw.data(), t ? t->ne : nullptr, static_cast<int>(model->hparams.n_embd),
                ...
                if ((model->variant == ModelVariant::QWEN36 || model->variant == ModelVariant::QWEN38) &&
                    model->ssm_group_count > 0 && model->ssm_group_count != n_heads) {
        ...
        for (uint32_t i = 0; i < model->hparams.n_layer; ++i) {
            ...
            if (ssm_ordinal < static_cast<int>(model->ssm_layer_states.size())) {
                ...
                if (!canonicalize_conv1d("ssm_conv1d", conv, state.conv1d_f32, i) ||
                    ...
                    return fail_load("Qwen3.5 SSM tensor canonicalization failed");
                }
                if (ssm_ordinal == 0) {
                    const char* ssm_debug_values =
                        densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_QWEN36_SSM_VALUES");
    ...
    if (model->arch == ModelArch::QWEN3 || model->arch == ModelArch::QWEN35) {
        ...
        if (!has_qk_norms) {
            ...
            return fail_load(arch + " attention tensor validation failed");
        }
    ...
    if (model->arch_flags.is_hybrid_ssm) {
        ...
        for (const char* probe : {"visual.patch_embed.proj.weight", "vision_tower.patch_embed.proj.weight",
            ...
            if (model->FindWeightTensor(probe) != nullptr) {
    ...
    if (model->arch == ModelArch::VIT || model->arch == ModelArch::CLIP_VISION || model->arch == ModelArch::SIGLIP) {
        ...
        auto get_vision_tensor = [&](const std::vector<std::string>& names) -> struct ggml_tensor* {
            for (const auto& name : names) {
                struct ggml_tensor* t = model->FindWeightTensor(name.c_str());
                if (t) return t;
            }
            return nullptr;
        };
...

    {
        ...
        auto bind_expert_view_int4 = [&](ggml_tensor* tensor) {
            ...
            if (ResolveStackedInt4ViewBinding(model->ctx_views, root_tensor, root_binding, tensor, total_view_offs,
                                              &resolved)) {
    ...
    model->ForEachWeightContext([&](ggml_context* weight_ctx) {
        struct ggml_tensor* t = ggml_get_first_tensor(weight_ctx);
        while (t) {
            if (std::strcmp(t->name, "GGUF tensor data binary blob") != 0) {
    ...
    const bool prepopulate_weights = [total_weight_bytes]() {
        ...
        // Above this resident-weight size, faulting pages in lazily costs more
        // than the up-front prepopulation pass.
        constexpr size_t threshold_mb = 8192;
    ...
    if (densecore::NumaAllocator::IsNumaAvailable()) {
        ...
        model->ForEachWeightContext([&](ggml_context* weight_ctx) {
            struct ggml_tensor* t = ggml_get_first_tensor(weight_ctx);
            while (t) {
                if (std::strcmp(t->name, "GGUF tensor data binary blob") != 0 && t->data &&
                    ggml_nbytes(t) >= 1024 * 1024) {
...
static void MoveBufferPagesToNumaNode(void* base, size_t bytes, int target_node, NumaExpertPartitionStats* stats) {
    ...
    std::vector<void*> retry_pages(pages.size());
    std::vector<int> retry_nodes(pages.size(), target_node);
    std::vector<int> retry_status(pages.size(), -1);
    ...
    for (size_t batch_start = 0; batch_start < num_pages; batch_start += kBatch) {
        ...
        // A sparsely populated backend buffer can contain pages that have not
        // been faulted in yet. move_pages reports those slots as -ENOENT. The
        // tensor bytes are nevertheless part of an expert range, and leaving
        // them absent makes the later range-placement query reject the entire
        // expert. Materialize only those pages with an identical-byte write,
        // then retry the small failed subset once.
        size_t retry_count = 0;
        for (size_t i = 0; i < batch_size; ++i) {
            if (status[i] != -ENOENT) {
                continue;
            }
            auto* byte = reinterpret_cast<volatile unsigned char*>(pages[i]);
            const unsigned char value = *byte;
            *byte = value;
            retry_pages[retry_count] = pages[i];
            retry_nodes[retry_count] = target_node;
            retry_status[retry_count] = -1;
            ++retry_count;
        }
        if (retry_count > 0) {
            result =
                move_pages(0, retry_count, retry_pages.data(), retry_nodes.data(), retry_status.data(), MPOL_MF_MOVE);
            if (result < 0) {
                if (stats) stats->pages_failed += retry_count;
                retry_count = 0;
            }
        }
 ...
 */
static bool PartitionMoEExpertTensorsAcrossNumaNodes(TransformerModel* model, int num_nodes, int pinned_node) {
    ...
    const auto partition_fused = [&](struct ggml_tensor* t) {
        ...
        if (n_experts <= 1 || expert_stride == 0 || expert_stride * static_cast<size_t>(n_experts) > total_bytes) {
            return;
        }
        stats.tensors++;
    ...
    if (stats.expert_slices > 0) {
        ...
        std::cout << "[DenseCore] NUMA expert partition (" << (pinned_node >= 0 ? "pinned" : "round-robin")
                  << "): tensors=" << stats.tensors << " slices=" << stats.expert_slices
                  << " moved=" << (stats.pages_moved * static_cast<size_t>(std::max(1L, page_size_log)) / 1024 / 1024)
                  << " MB";
    ...
    const size_t addressed_pages = stats.pages_moved + stats.pages_failed;
    return stats.expert_slices > 0 && addressed_pages > 0 && stats.pages_moved * 100 >= addressed_pages * 99;
}
...
TransformerModel* LoadGGUFModelNuma(const char* path, int numa_node, bool use_huge_pages) {
    ...
    if (use_huge_pages) {
        ...
        model->ForEachWeightContext([&](ggml_context* weight_ctx) {
            ...
            while (t) {
                ...
                if (std::strcmp(t->name, "GGUF tensor data binary blob") != 0 && tensor_size >= huge_page_threshold &&
                    t->data) {
    ...
    if (num_nodes > 1 && IsNumaExpertPartitionEnabled()) {
        ...
        if (PartitionMoEExpertTensorsAcrossNumaNodes(model, num_nodes, expert_pinned_node)) {
            model->numa_expert_node_hints.resize(model->hparams.n_experts, -1);
            for (size_t expert = 0; expert < model->numa_expert_node_hints.size(); ++expert) {
                model->numa_expert_node_hints[expert] =
                    expert_pinned_node >= 0 ? expert_pinned_node : static_cast<int>(expert % num_nodes);
            }
        }
    }
...
TransformerModel* LoadModelFromExternal(const TransformerHParams& hparams, const std::vector<ExternalTensor>& tensors,
    ...
    for (const auto& et : tensors) {
        ...
        if (t) {
            t->data = et.data;  // Reference the externally owned mapped data.
...
bool IsMoELoaderDebugEnabled() {
    static const bool enabled = []() {
        const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_MOE_LOADER");
...
bool IsQwen36MoEBindingDebugEnabled() {
    static const bool enabled = []() {
        const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_QWEN36_MOE_BINDINGS");
        return env && env[0] != '\0' && std::strcmp(env, "0") != 0;
    }();
    return enabled;
}
