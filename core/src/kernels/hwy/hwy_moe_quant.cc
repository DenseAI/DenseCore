...
template <typename BlockT, bool Q5, bool SignedNibbles = true>
    ...
    for (int bi = 0; bi < nb; ++bi) {
        ...
        for (int sub = 0; sub < kMoEQK_K / 32; ++sub) {
            ...
            for (size_t off = 0; off < 32; off += quad_lanes) {
                ...
                // Q4/Q5 unpacking produces 0..15/31, so signed reinterpretation
                // is lossless and lets SVE use one SDOT instead of mixed UDOTs.
                if constexpr (SignedNibbles) {
                    sub_acc = hn::SumOfMulQuadAccumulate(di32, hn::BitCast(di8, wv), qv, sub_acc);
                } else
#endif
                {
...
bool SignedNibbleDotParityImpl(bool q5, const void* weight, const void* input, int64_t cols) {
    float mixed = 0.0f, candidate = 0.0f;
    const bool ok = q5
        ? (DotKQ8KImpl<MoEQ5KBlock, true, false>(weight, input, cols, &mixed) &&
           DotKQ8KImpl<MoEQ5KBlock, true, true>(weight, input, cols, &candidate))
        : (DotKQ8KImpl<MoEQ4KBlock, false, false>(weight, input, cols, &mixed) &&
           DotKQ8KImpl<MoEQ4KBlock, false, true>(weight, input, cols, &candidate));
    return ok && std::memcmp(&mixed, &candidate, sizeof(float)) == 0;
}
#endif
...
#ifdef DENSECORE_TEST_BUILD
HWY_EXPORT(SignedNibbleDotParityImpl);
#endif
...
bool SignedNibbleDotParityForTest(bool q5, const void* weight, const void* input, int64_t cols) {
    return HWY_DYNAMIC_DISPATCH(SignedNibbleDotParityImpl)(q5, weight, input, cols);
}
#endif
