...
void ExecuteMixedMatMulF32Accum(const Tensor& A, const Tensor& B, Tensor* C, ThreadPool& pool) {
...
void ExecuteMixedMatMulTransBF32Accum(const Tensor& A, const Tensor& B, Tensor* C, ThreadPool& pool) {
    const int M = static_cast<int>(A.shape[0]);
    const int K = static_cast<int>(A.shape[1]);
...
bool ExecuteTransBViaMatmulBackend(const Tensor& A, const Tensor& B, Tensor* C, ThreadPool& pool) {
    if (!IsF32OrBF16(A.dtype) || !IsF32OrBF16(B.dtype) || !C || C->dtype != DType::F32) {
        return false;
    }

    MatmulParams params;
    params.thread_pool = &pool;
    params.a = A.data;
    params.b = B.data;
    params.c = C->data;
    params.M = A.shape[0];
    ...
    if (!backend->Supports(params)) {
        ...
        if (!backend->Supports(params)) {
            ExecuteMixedMatMulTransBF32Accum(A, B, C, pool);
            return true;
        }
    }
...
public:
    ...
    void MatMul(const Tensor& A, const Tensor& B, Tensor* C) override {
        MatMul(A, B, C, GetCpuBackend().GetThreadPool(0));
    }

    void MatMul(const Tensor& A, const Tensor& B, Tensor* C, ThreadPool& pool) override {
        ...
        if (A.dtype == DType::BF16 || B.dtype == DType::BF16) {
            ExecuteMixedMatMulF32Accum(A, B, C, pool);
            return;
        }
...
#pragma GCC ivdep
    ...
    void MatMulTransB(const Tensor& A, const Tensor& B, Tensor* C) override {
        MatMulTransB(A, B, C, GetCpuBackend().GetThreadPool(0));
    }

    void MatMulTransB(const Tensor& A, const Tensor& B, Tensor* C, ThreadPool& pool) override {
        if (!A.IsValid() || !B.IsValid() || !C || !C->IsValid()) return;
        if (!IsF32OrBF16(A.dtype) || !IsF32OrBF16(B.dtype) || C->dtype != DType::F32) return;
        ...
        if (A.dtype != DType::F32 || B.dtype != DType::F32) {
            ExecuteTransBViaMatmulBackend(A, B, C, pool);
            return;
        }
