...
int FailQuantize(DenseCoreStatus status, const char* message, int code) {
    SetLastError(status, message);
    return code;
}

int FailQuantize(DenseCoreStatus status, const std::string& message, int code) {
    SetLastError(status, message);
    return code;
}
...
int QuantizeModel(const char* model_path, const char* output_path, const char* config_json) {
    try {
        ...
        if (!model_path || !output_path || model_path[0] == '\0' || output_path[0] == '\0') {
            return FailQuantize(DENSECORE_STATUS_INVALID_ARGUMENT,
                                "QuantizeModel: model_path and output_path are required", -1);
        }
        ...
        if (!input_check.good()) {
            return FailQuantize(DENSECORE_STATUS_MODEL_LOAD_FAILED,
                                std::string("QuantizeModel: input model not found: ") + input, -2);
        }
        ...
        if (parsed_format == ApiQuantFormat::UNSUPPORTED) {
            return FailQuantize(DENSECORE_STATUS_UNSUPPORTED_OPERATION,
                                std::string("QuantizeModel: unsupported quantization format: ") +
                                    (format.empty() ? "<empty>" : format),
                                -3);
        }

        if (parsed_format == ApiQuantFormat::PASSTHROUGH) {
            if (!CopyFileBinary(input, output)) {
                return FailQuantize(DENSECORE_STATUS_INTERNAL_ERROR,
                                    std::string("QuantizeModel: failed to copy model to output path: ") + output, -4);
            }
            ClearLastError();
            return 0;
        }
        ...
        const int rc =
            QuantizeGGUF(model_path, output_path, ToGGMLType(parsed_format), skip_output_layer, skip_embeddings);
        if (rc != 0) {
            return FailQuantize(DENSECORE_STATUS_INTERNAL_ERROR,
                                std::string("QuantizeModel: quantization failed with code ") + std::to_string(rc), rc);
        }
        ClearLastError();
        return rc;
    } catch (...) {
        return FailQuantize(DENSECORE_STATUS_INTERNAL_ERROR, "QuantizeModel: unexpected exception", -5);
    }
}
