...
bool ShouldCaptureSamplingDebugTrace(const SamplingParams& params) {
    ...
    const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_SAMPLER_TRACE");
...
void GrammarConstraint::UpdateState(const std::string& token_text) {
    ...
    for (char ch : token_text) {
        const bool in_string = state == JSONState::IN_KEY || state == JSONState::IN_STRING_VALUE;
...
std::string TrimJsonWhitespace(const std::string& token) {
...
bool IsExactJsonStructuralToken(const std::string& token, char expected) {
    const std::string trimmed = TrimJsonWhitespace(token);
    return trimmed.size() == 1 && trimmed[0] == expected;
}

bool IsJsonStringContentToken(const std::string& token) {
    if (token.empty()) return false;
...
void ApplyGrammarMask(float* logits, int n_vocab, const GrammarConstraint* grammar,
    ...
    case JSONState::EXPECT_OBJECT_START:
        for (int i = 0; i < n_vocab; i++) {
            if (IsExactJsonStructuralToken(vocab[i], '{')) {
                allowed[i] = true;
            }
        }
        break;
    ...
    case JSONState::EXPECT_KEY_OR_END:
        for (int i = 0; i < n_vocab; i++) {
            if (IsExactJsonStructuralToken(vocab[i], '"') || IsExactJsonStructuralToken(vocab[i], '}')) {
                allowed[i] = true;
            }
        }
        break;
    ...
    case JSONState::IN_STRING_VALUE:
        for (int i = 0; i < n_vocab; i++) {
            if (IsExactJsonStructuralToken(vocab[i], '"') || IsJsonStringContentToken(vocab[i])) {
                allowed[i] = true;
            }
        }
        break;
    ...
    case JSONState::EXPECT_VALUE:
        for (int i = 0; i < n_vocab; i++) {
            const std::string& token = vocab[i];
            if (IsExactJsonStructuralToken(token, '"') || IsExactJsonStructuralToken(token, '{') ||
                IsExactJsonStructuralToken(token, '[') ||
    ...
    case JSONState::IN_NUMBER:
        for (int i = 0; i < n_vocab; i++) {
            const std::string& token = vocab[i];
            if (IsDigitToken(token) || IsExactJsonStructuralToken(token, ',') || IsExactJsonStructuralToken(token, '}') ||
                IsExactJsonStructuralToken(token, ']')) {
                allowed[i] = true;
            }
        }
    ...
    case JSONState::EXPECT_COMMA_OR_END:
        for (int i = 0; i < n_vocab; i++) {
            if (IsExactJsonStructuralToken(vocab[i], ',') || IsExactJsonStructuralToken(vocab[i], '}') ||
                IsExactJsonStructuralToken(vocab[i], ']')) {
                allowed[i] = true;
            }
        }
        break;
...
int SampleToken(struct ggml_tensor* logits, int idx, const SamplingParams& params) {
    ...
    auto resolve_debug_top_n = [&]() -> int {
        const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_SAMPLE_TOP");
        if (!env || env[0] == '\0') {
            return 0;
        }
        char* end = nullptr;
    ...
    const bool debug_sample = (densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_SAMPLE") != nullptr);
    ...
    auto debug_dump_top_candidates = [&](const char* stage, int sampled_token) {
        ...
        const char* watch_env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_SAMPLE_WATCH_IDS");
