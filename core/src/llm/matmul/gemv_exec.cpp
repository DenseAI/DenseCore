...
void cb_gemv_custom(struct ggml_tensor* dst, int ith, int nth, void* userdata) {
    ...
    const InferenceExecutionPhase callback_phase =
        callback_work_ctx ? GetInferenceWorkContextPhase(callback_work_ctx) : ud->phase_snapshot;
    ...
    if (ith == 0 && callback_work_ctx) {
        auto& profile = (*GetInferenceWorkContextProfile(callback_work_ctx));
    ...
    static const bool debug_gemv_timing = []() {
        const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_GEMV_TIMING");
    ...
    const auto census_begin =
        matmul_dispatch_census_enabled ? std::chrono::steady_clock::now() : std::chrono::steady_clock::time_point{};
    const auto gemma4_decode_begin = (ith == 0 && ud->gemma4_decode_native) ? std::chrono::steady_clock::now()
                                                                            : std::chrono::steady_clock::time_point{};
    const auto lfm2_lm_head_begin = (ith == 0 && ud->lfm2_decode_lm_head) ? std::chrono::steady_clock::now()
                                                                          : std::chrono::steady_clock::time_point{};
    ...
    const auto maybe_log_gemv_timing = [&](const char* path) {
        ...
        std::fprintf(stderr, "[GEMV_TIMING] weight=%s type=%s path=%s K_out=%d N_in=%d nth=%d ith0_ms=%.3f\n",
    ...
    const auto record_gemv_dispatch_census = [&](const char* dispatch_path) {
        ...
        if (ith == 0 && weight_type == GGML_TYPE_Q6_K && callback_work_ctx && !q6k_decision_recorded) {
            ...
            const bool q6_direct_path = dispatch_path && (std::strcmp(dispatch_path, "q6k_direct_vecdot") == 0 ||
            ...
            if (quant_input_size > 0 && quant_input_size <= kMaxQuantInputBufferSize) {
                ...
                if (nth <= 1 || !has_valid_stamp) {
                    ...
                    if (!ud->quant_input_shared || !ud->quantized_stamp) {
                        ...
                        const int64_t token_pos = (batch && batch->num_seqs == 1 && !batch->pos.empty())
                                                      ? batch->pos.front()
                                                      : std::numeric_limits<int64_t>::min();
                        const uint8_t* cached_qact =
                            QuantizedActivationCacheEnabled(qact_config)
                                ? GetOrFillQuantizedActivationCache(
                                      work_ctx, src, src->data, x_f32, static_cast<int64_t>(N), ud->input_quant_type,
                                      quant_input_size, ud->slot_id, token_pos, input_type_traits)
                                : nullptr;
                        if (cached_qact) {
    ...
    if (ith == 0 && quant_input) {
        densecore::llm::attention::RecordSharedQuantReuse(used_shared_quant_buffer);
    }
    const bool q4k_candidate_shape_ok = N > 0 && K > 0 && (N % QK_K) == 0 && (K % 8) == 0;
    const bool q4k_repacked_candidate = weight_type == GGML_TYPE_Q4_K && ud->input_quant_type == GGML_TYPE_Q8_K &&
                                        quant_input && q4k_candidate_shape_ok;
    const bool q6k_candidate_shape_ok = N > 0 && K > 0 && (N % QK_K) == 0 && K >= 1024 && N >= 1024;
    const bool q6k_effective_decode_gemv = callback_phase == InferenceExecutionPhase::Decode || q6k_candidate_shape_ok;
    const bool q6k_decode_candidate = weight_type == GGML_TYPE_Q6_K && q6k_effective_decode_gemv &&
                                      ud->input_quant_type == GGML_TYPE_Q8_K && quant_input && q6k_candidate_shape_ok &&
                                      !ud->dynamic_lora_active && !ud->force_reference_scalar;
    if (ith == 0 && callback_work_ctx) {
        auto& profile = (*GetInferenceWorkContextProfile(callback_work_ctx));
        profile
            .gemv_custom_quant_input_type_hist[MatmulQuantInputTypeHistIndex(ud->input_quant_type,
                                                                             quant_input != nullptr)]
    ...
    if (weight_type == GGML_TYPE_Q4_K && ud->input_quant_type == GGML_TYPE_Q8_K && quant_input &&
        ...
        if (InferenceWorkContext* work_ctx = callback_work_ctx) {
            ...
            (*GetInferenceWorkContextProfile(work_ctx))
                .q4k_repacked_gemv_last_reject_reason.store(
    ...
    if (weight_type == GGML_TYPE_Q6_K) {
        ...
        if (q6_can_use_direct) {
            ...
            if (lfm2_argmax_candidate && ith == 0) {
                ...
                if (!callback_matmul_state->lfm2_greedy_lm_head_argmax_allowed) {
                    ...
                    profile.lfm2_greedy_lm_head_argmax_last_reject_reason.store(
                        static_cast<int>(callback_matmul_state->lfm2_greedy_lm_head_argmax_reject_reason),
                        std::memory_order_relaxed);
                    callback_matmul_state->lfm2_greedy_lm_head_argmax_generation = 0;
                    callback_matmul_state->lfm2_greedy_lm_head_argmax_token = -1;
                    callback_matmul_state->lfm2_greedy_lm_head_argmax_value = -std::numeric_limits<float>::infinity();
                    callback_matmul_state->lfm2_greedy_lm_head_argmax_vocab_size = 0;
                    callback_matmul_state->lfm2_greedy_lm_head_sparse_logits_ptr = nullptr;
                    callback_matmul_state->lfm2_greedy_lm_head_sparse_logits_vocab_size = 0;
                    callback_matmul_state->lfm2_greedy_lm_head_sparse_logits_token = -1;
                }
            }
            if (lfm2_argmax_candidate && callback_matmul_state->lfm2_greedy_lm_head_argmax_allowed) {
                const uintptr_t argmax_key =
                    reinterpret_cast<uintptr_t>(dst->data) ^ (reinterpret_cast<uintptr_t>(src->data) << 1) ^
                    (reinterpret_cast<uintptr_t>(weight_data) << 7) ^
                    (static_cast<uintptr_t>(GetInferenceWorkContextExecutionGeneration(callback_work_ctx)) *
                     0x9e3779b97f4a7c15ull);
                {
                    ...
                    if (ud->lfm2_argmax_q6k_cache_weight != q6_cache_weight || ud->lfm2_argmax_q6k_cache_nth != nth ||
                        ud->lfm2_argmax_q6k_cache_k != K || ud->lfm2_argmax_q6k_cache_n != N) {
                ...
                const auto& repeated_tokens = callback_matmul_state->lfm2_greedy_lm_head_argmax_repeated_tokens;
                const auto& disallowed_tokens = callback_matmul_state->lfm2_greedy_lm_head_argmax_disallowed_tokens;
                const float repetition_penalty = callback_matmul_state->lfm2_greedy_lm_head_argmax_repetition_penalty;
                const bool repetition_penalty_active = repetition_penalty != 1.0f && !repeated_tokens.empty();
                size_t repeat_pos = repetition_penalty_active
                                        ? static_cast<size_t>(std::lower_bound(repeated_tokens.begin(),
                                                                               repeated_tokens.end(), k_start) -
                                                              repeated_tokens.begin())
                                        : repeated_tokens.size();
                size_t disallowed_pos = !disallowed_tokens.empty()
                                            ? static_cast<size_t>(std::lower_bound(disallowed_tokens.begin(),
                                                                                   disallowed_tokens.end(), k_start) -
                                                                  disallowed_tokens.begin())
                                            : disallowed_tokens.size();
                int k = k_start;
                const bool q6_can_use_repacked_argmax = row_stride > 0 && (N % ggml_blck_size(weight_type)) == 0 &&
                                                        (k_start % 8) == 0 && ((k_end - k_start) % 8) == 0;
                ...
                if (q6_can_use_repacked_argmax) {
                    ...
                    const bool projected = densecore::RunQ6KRepackedMoEProjectionCached(
                        &densecore::llm::runtime::ResolveCpuBackend(GetCurrentBatch()), row_ptr,
                        static_cast<const uint8_t*>(quant_input), ggml_row_size(GGML_TYPE_Q8_K, N),
                        repacked_argmax_tile.data(), 1, row_count, N,
                ...
                if (q6_can_use_rowpair) {
                    for (; k + 1 < k_end; k += 2) {
                        ...
                        if (!second_disallowed) {
                            sums[1] = ApplyLFM2GreedyArgmaxRepetitionPenaltyIfDue(sums[1], k + 1, repeated_tokens,
                                                                                  repeat_pos, repetition_penalty,
                ...
                std::chrono::steady_clock::time_point begin;
                {
                    ...
                    if (ud->lfm2_argmax_key != argmax_key) {
                        (*GetInferenceWorkContextProfile(callback_work_ctx))
                            .lfm2_greedy_lm_head_argmax_rejected_ops.fetch_add(1, std::memory_order_relaxed);
                        (*GetInferenceWorkContextProfile(callback_work_ctx))
                            .lfm2_greedy_lm_head_argmax_last_reject_reason.store(
                ...
                if (last_thread) {
                    ...
                    RecordInferenceWorkContextLFM2GreedyLMHeadArgmaxToken(
                        callback_work_ctx, GetInferenceWorkContextExecutionGeneration(callback_work_ctx), best_token,
                        best_value, K);
                    auto& profile = (*GetInferenceWorkContextProfile(callback_work_ctx));
                    profile.lfm2_decode_lm_head_custom_gemv_used_ops.fetch_add(1, std::memory_order_relaxed);
                    ...
                    RecordQ6KGemvDecision(callback_work_ctx, /*candidate=*/true, /*used=*/true, "none", weight_name, 1,
                                          K, N, wall_ns, "decode", "decode_shape", MatmulPhaseName(callback_phase),
                                          used_repacked_argmax
                                              ? "lfm2_q6k_argmax_repacked"
                                              : (q6_can_use_rowpair ? "lfm2_q6k_argmax_rowpair" : "lfm2_q6k_argmax"));
                    q6k_decision_recorded = true;
                }
                return;
            }
        ...
        if (ith == 0 && callback_work_ctx) {
            ...
            RecordQ6KGemvDecision(callback_work_ctx, q6k_decode_candidate, /*used=*/false, q6_reject_reason,
                                  weight_name, 1, K, N, 0, effective_phase_name, graph_phase_name, callback_phase_name,
                                  nullptr);
            q6k_decision_recorded = true;
        }
    }
    if (weight_type == GGML_TYPE_Q4_K && ud->input_quant_type == GGML_TYPE_Q8_K && quant_input &&
        ...
        const BatchSpec* batch = (dispatch_work_ctx && GetInferenceWorkContextBatch(dispatch_work_ctx))
                                     ? GetInferenceWorkContextBatch(dispatch_work_ctx)
                                     : callback_batch;
        ...
        if (dispatch_work_ctx) {
            const int disable_reason = (*GetInferenceWorkContextProfile(dispatch_work_ctx))
                                           .q4k_repacked_gemv_primary_disable_reason.load(std::memory_order_relaxed);
        ...
        const bool phase_allows_repacked =
            ...
            (callback_phase == InferenceExecutionPhase::Decode ||
             (fast_config.q4k_repacked_gemv_allow_prefill && callback_phase == InferenceExecutionPhase::Prefill));
        ...
        if (real_repacked_enabled && phase_allows_repacked && reject == Q4KRepackedGemvRejectReason::None) {
            ...
            if (shape_ok) {
                ...
                if (reject == Q4KRepackedGemvRejectReason::None && packed && fast_config.q4k_repacked_gemv_probe) {
                    ...
                    if (dispatch_work_ctx) {
                        AddQwen36ProfileNs(
                            (*GetInferenceWorkContextProfile(dispatch_work_ctx)).q4k_repacked_gemv_probe_ns,
    ...
    const bool allow_native_q4k_vecdot = ShouldUseArmNativeQ4KVecDotValidated(weight_type, type_traits_cpu, weight_name,
                                                                              sample_row_ptr, quant_input, x_f32, N);
    ...
    if (!ud->force_reference_scalar && quant_input && type_traits_cpu && type_traits_cpu->vec_dot &&
        ...
        if (!ud->disable_q8_repacked_gemv && weight_type == GGML_TYPE_Q8_0 && ud->input_quant_type == GGML_TYPE_Q8_0 &&
            (K >= q8_repacked_min_rows || ud->force_q8_repacked_gemv) && (N % QK8_0) == 0 && (K % 4) == 0) {
            ...
            if (packed && packed->blocks_per_row > 0) {
                ...
                const bool gemma4_argmax_candidate = ud->gemma4_decode_lm_head && callback_work_ctx &&
                                                     callback_matmul_state->lfm2_greedy_lm_head_argmax_allowed;
                if (gemma4_argmax_candidate) {
                    ...
                    const float repetition_penalty =
                        callback_matmul_state->lfm2_greedy_lm_head_argmax_repetition_penalty;
                    const float final_logit_softcap =
                        callback_matmul_state->lfm2_greedy_lm_head_argmax_final_logit_softcap;
                    const bool repetition_penalty_active = repetition_penalty != 1.0f && !repeated_tokens.empty();
                    size_t repeat_pos = repetition_penalty_active
                                            ? static_cast<size_t>(std::lower_bound(repeated_tokens.begin(),
                                                                                   repeated_tokens.end(), k_start) -
                    ...
                    if (last_thread) {
                        WriteInferenceWorkContextLFM2GreedyLMHeadSparseLogits(callback_work_ctx, output, K, best_token,
                                                                              best_value);
                        RecordInferenceWorkContextLFM2GreedyLMHeadArgmaxToken(
                            callback_work_ctx, GetInferenceWorkContextExecutionGeneration(callback_work_ctx),
                            best_token, best_value, K);
                    }
                    uint64_t wall_ns = 0;
                    if (begin != std::chrono::steady_clock::time_point{}) {
                        wall_ns = static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(
                    ...
                    if (ith == 0) {
                        RecordGemma4DecodeNativeDecision(callback_work_ctx, /*candidate=*/false, /*used=*/true, nullptr,
                                                         ...
                                                         /*duplicate_work_detected=*/false);
                        if (matmul_dispatch_census_enabled) {
                            RecordMatmulDispatchCensus(callback_work_ctx, callback_phase, "gemma4_q8_0_lm_head_argmax",
                                                       weight_type, 1, K, N, wall_ns);
                        }
                    }
                    return;
                }
