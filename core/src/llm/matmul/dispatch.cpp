...
bool CpuRepackAliasHasLayout(const TransformerModel*, const ggml_tensor*, TransformerModel::CpuRepackAliasLayout);
}
using densecore::llm::graph::detail::CpuRepackAliasHasLayout;
...
static bool IsGemma4DenseWeightNameCandidate(const char* name, bool is_lm_head) {
    ...
    return std::strstr(name, "attn_q") || std::strstr(name, "attn_k") || std::strstr(name, "attn_v") ||
           std::strstr(name, "attn_output") || std::strstr(name, "attn_o") || std::strstr(name, "ffn_gate") ||
           std::strstr(name, "ffn_up") || std::strstr(name, "ffn_down") || std::strstr(name, "shared") ||
           std::strstr(name, "output") || std::strstr(name, "blk.");
}
...
static bool CanUseGemma4DensePrefillNative(const TransformerModel* model, const ggml_tensor* weight,
    ...
    if (!traits || !traits->vec_dot || !traits->from_float || (input->ne[0] % QK8_0) != 0 || (weight->ne[1] % 4) != 0 ||
        input->ne[1] < 4) {
...
static bool CanUseGemma4DecodeNative(const TransformerModel* model, const ggml_tensor* weight, const ggml_tensor* input,
                                     const BatchSpec* batch, InferenceExecutionPhase dispatch_phase,
                                     Gemma4NativeMatmulReject* reject_reason) {
    auto reject = [&](Gemma4NativeMatmulReject reason) {
        if (reject_reason) {
            *reject_reason = reason;
...
static densecore::models::ModelTensorExecutionRequirement ResolveSmartMatmulRequirement(const TransformerModel* model,
                                                                                        const ggml_tensor* weight) {
    return densecore::models::ResolveModelTensorExecutionRequirement(model, weight, SmartMatmulTensorName(weight),
...
static bool IsLFM2TargetProjectionWeight(const TransformerModel* model, const ggml_tensor* weight, bool is_lm_head) {
    ...
    return std::strstr(name, "blk.") || std::strstr(name, "ffn_gate") || std::strstr(name, "ffn_up") ||
           std::strstr(name, "ffn_down") || std::strstr(name, "ffn_gate_up") || std::strstr(name, "shortconv") ||
           std::strstr(name, "short_conv") || std::strstr(name, "attn_") || std::strstr(name, "token_embd") ||
...
static ggml_tensor* ResolveSmartMatmulInitialWeight(TransformerModel* model, ggml_tensor* weight,
    ...
    auto it_source = model->prepared_weights.cpu_repack_alias_sources.find(weight);
    ggml_tensor* source = (it_source != model->prepared_weights.cpu_repack_alias_sources.end() && it_source->second)
                              ? it_source->second
                              : nullptr;
    ...
    const bool qwen_target_projection = densecore::runtime::IsQwenTargetVariant(model->variant) && source &&
                                        ggml_is_quantized(source->type) && source->ne[0] == input->ne[0];
...
struct SmartMatmulAliasSelection {
    ...
    bool using_qwen38_amx_alias = false;
};
...
static SmartMatmulAliasSelection ResolveSmartMatmulAliasSelection(TransformerModel* model, ggml_tensor* weight,
                                                                  const ggml_tensor* input,
                                                                  const SmartMatmulOriginalTensorState& original,
                                                                  bool dispatch_is_prefill_phase) {
    ...
    const bool qwen_hybrid_small_prefill_q4k = dispatch_is_prefill_phase && original.input_cols > 1 &&
    ...
    const bool prefer_gemma4_quant_prefill_densecore_path =
        ...
        (selection.weight->type == GGML_TYPE_Q4_K || selection.weight->type == GGML_TYPE_Q8_0) && input->ne[1] > 1 &&
        input->ne[0] == selection.weight->ne[0] &&
        (selection.weight->type != GGML_TYPE_Q4_K || IsQ4KTrueBatchedKernelEnabled());
    const bool prefer_gemma4_quant_decode_densecore_path = model->arch_flags.is_gemma4 &&
                                                           ggml_is_quantized(selection.weight->type) &&
                                                           input->ne[1] <= 1 && input->ne[0] == selection.weight->ne[0];
    const bool prefer_lfm2_quant_prefill_densecore_path =
        dispatch_is_prefill_phase && original.lfm2_shortconv_semantic && ggml_is_quantized(selection.weight->type) &&
        input->ne[1] > 1 && input->ne[0] == selection.weight->ne[0];
    const bool prefer_lfm2_quant_decode_densecore_path = original.lfm2_shortconv_semantic &&
                                                         ggml_is_quantized(selection.weight->type) &&
                                                         input->ne[1] <= 1 && input->ne[0] == selection.weight->ne[0];
    const bool qwen35_point8b_quant_projection = densecore::runtime::IsQwen35Point8B(model) &&
                                                 ggml_is_quantized(selection.weight->type) &&
                                                 input->ne[0] == selection.weight->ne[0];
    const bool prefer_qwen_target_quant_prefill_densecore_path =
        ...
        ggml_is_quantized(selection.weight->type) && input->ne[1] > 1 && input->ne[0] == selection.weight->ne[0];
    // Decode can contain several concurrent sequences. Both GEMV and batched
    // quantized callbacks consume standard-layout weights; a generic repack
    // alias would select the unmaintained GGML fallback before those callbacks.
    // Keep Qwen3.8's separately admitted AMX alias path unchanged.
    const bool prefer_qwen_target_quant_decode_densecore_path =
        ((((model->variant == ModelVariant::QWEN35 || model->variant == ModelVariant::QWEN36) &&
          ...
          (!dispatch_is_prefill_phase || input->ne[1] <= 1)) ||
         qwen35_point8b_quant_projection) &&
        ggml_is_quantized(selection.weight->type) && input->ne[1] > 0 && input->ne[0] == selection.weight->ne[0];

    if (input->ne[1] <= 1) {
        auto it_decode_repack = model->prepared_weights.cpu_decode_repack_aliases.find(selection.weight);
        if (!prefer_gemma4_quant_decode_densecore_path && !prefer_lfm2_quant_decode_densecore_path &&
            ...
            it_decode_repack != model->prepared_weights.cpu_decode_repack_aliases.end() && it_decode_repack->second) {
            selection.weight = it_decode_repack->second;
            selection.using_cpu_repack_alias = true;
        }
    }
    auto it_repack = model->prepared_weights.cpu_repack_aliases.find(selection.weight);
    if (!selection.using_cpu_repack_alias && it_repack != model->prepared_weights.cpu_repack_aliases.end() &&
        it_repack->second) {
        const bool amx_alias = model->prepared_weights.cpu_amx_aliases.find(it_repack->second) !=
                               model->prepared_weights.cpu_amx_aliases.end();
        const bool qwen38_amx_alias = model->variant == ModelVariant::QWEN38 && amx_alias && input->ne[1] > 0;
        ...
        if (!prefer_gemma4_quant_prefill_densecore_path && !prefer_lfm2_quant_prefill_densecore_path &&
            (!prefer_qwen_target_quant_prefill_densecore_path || qwen38_amx_alias) &&
            !prefer_gemma4_quant_decode_densecore_path && !prefer_lfm2_quant_decode_densecore_path &&
            !prefer_qwen_target_quant_decode_densecore_path && !prefer_qwen_hybrid_ssm_quant_batched_over_repack &&
            !amx_decode_regression_risk) {
            selection.weight = it_repack->second;
            selection.using_cpu_repack_alias = true;
            selection.using_qwen38_amx_alias = qwen38_amx_alias;
        }
    }
    return selection;
}

static SmartMatmulOriginalTensorState
CaptureSmartMatmulOriginalTensorState(const TransformerModel* model, ggml_tensor* weight, const ggml_tensor* input) {
...
static densecore::runtime::DenseCoreMatmulPhase ResolveSmartMatmulPlanPhase(InferenceExecutionPhase dispatch_phase,
                                                                            bool dispatch_is_prefill_phase) {
...
static SmartMatmulExecutionPlan ResolveSmartMatmulExecutionPlan(TransformerModel* model, ggml_tensor* weight,
    ...
    plan.dispatch_phase = plan.work_ctx ? GetInferenceWorkContextPhase(plan.work_ctx) : GetCurrentExecutionPhase();
    ...
    plan.using_qwen38_amx_alias = alias_selection.using_qwen38_amx_alias;
    return plan;
}

static void RecordSmartMatmulRuntimeModes(InferenceWorkContext* work_ctx,
                                          const densecore::llm::config::FastPathRuntimeConfig& fast_path_config) {
    if (!work_ctx) {
        return;
    }
    (*GetInferenceWorkContextProfile(work_ctx))
        .qwen36_prefill_q4k_batched_mode.store(static_cast<int>(fast_path_config.qwen36_prefill_q4k_batched),
                                               std::memory_order_relaxed);
    (*GetInferenceWorkContextProfile(work_ctx))
        .qwen36_ssm_q8_prefill_amx_mode.store(static_cast<int>(fast_path_config.qwen36_ssm_q8_prefill_amx),
                                              std::memory_order_relaxed);
}

static void ApplySmartMatmulQwen36Q8PrefillAlias(TransformerModel* model, const ggml_tensor* input,
                                                 const densecore::llm::config::FastPathRuntimeConfig& fast_path_config,
                                                 SmartMatmulExecutionPlan* plan) {
    ...
    const bool original_qwen_ssm_q8_projection =
        qwen_hybrid_ssm_model && plan->original.hybrid_ssm_projection && qwen_ssm_projection_kind != 0 &&
        original_weight && original_weight->type == GGML_TYPE_Q8_0 && original_weight->ne[0] == input->ne[0];
    if (original_qwen_ssm_q8_projection && plan->original.input_cols <= 1 && plan->weight == original_weight) {
        ...
        if (plan->work_ctx) {
            (*GetInferenceWorkContextProfile(plan->work_ctx))
                .qwen36_ssm_q8_decode_used_original_q8_path.store(1, std::memory_order_relaxed);
            const Qwen36SSMQ8PrefillAMXRejectReason decode_alias_reason =
                ResolveQwen36SSMQ8PrefillAMXReason(fast_path_config.qwen36_ssm_q8_prefill_amx, plan->dispatch_phase,
    ...
    auto reject_q8_prefill = [&](Qwen36SSMQ8PrefillAMXRejectReason reason, bool candidate) {
        if (candidate && plan->work_ctx) {
            (*GetInferenceWorkContextProfile(plan->work_ctx))
                .qwen36_ssm_q8_prefill_amx_rejected_ops.fetch_add(1, std::memory_order_relaxed);
        }
        RecordQwen36SSMQ8PrefillAMXReject(plan->work_ctx, reason);
    };
    const Qwen36SSMQ8PrefillAMXRejectReason q8_prefill_gate =
        ResolveQwen36SSMQ8PrefillAMXReason(fast_path_config.qwen36_ssm_q8_prefill_amx, plan->dispatch_phase,
                                           plan->current_batch && !plan->current_batch->lora_map.empty());
    ...
    if (plan->work_ctx) {
        (*GetInferenceWorkContextProfile(plan->work_ctx))
            .qwen36_ssm_q8_prefill_amx_candidate_ops.fetch_add(1, std::memory_order_relaxed);
    }
    auto it_prefill_amx = model->prepared_weights.qwen36_ssm_q8_prefill_amx_aliases.find(original_weight);
    if (it_prefill_amx == model->prepared_weights.qwen36_ssm_q8_prefill_amx_aliases.end() || !it_prefill_amx->second) {
        reject_q8_prefill(Qwen36SSMQ8PrefillAMXRejectReason::AliasUnavailable, true);
    } else if (model->prepared_weights.cpu_amx_aliases.find(it_prefill_amx->second) ==
               model->prepared_weights.cpu_amx_aliases.end()) {
...
struct SmartMatmulGgmlCompatibilityPlan {
    ...
    bool qwen_hybrid_ssm_q8_native_prefill = false;
...
static SmartMatmulDispatchState
ResolveSmartMatmulDispatchState(const TransformerModel* model, const ggml_tensor* weight, const ggml_tensor* input,
                                const ggml_tensor* original_weight, InferenceExecutionPhase dispatch_phase,
                                bool dispatch_is_prefill_phase, bool using_cpu_repack_alias) {
    ...
    state.host_caps =
        densecore::runtime::CompileTimeHostKernelCapabilities(IsQ4KTrueBatchedKernelEnabled(), using_cpu_repack_alias);
    state.requirement = ResolveSmartMatmulRequirement(model, weight);
    ...
    state.lfm2_shortconv_semantic = state.semantic_op == densecore::runtime::DenseCoreSemanticOp::Lfm2ShortConvMixer;
    ...
    state.qwen38_target = model && model->variant == ModelVariant::QWEN38;
    state.qwen36_target = model && (model->variant == ModelVariant::QWEN36 || model->variant == ModelVariant::QWEN38);
    ...
    state.lfm2_shortconv_semantic =
        state.lfm2_shortconv_semantic ||
        IsLFM2TargetProjectionWeight(model, weight,
                                     model && (model->output == weight || model->output == original_weight));
    return state;
}

static SmartMatmulPrefillProjectionPlan
ResolveSmartMatmulPrefillProjectionPlan(const SmartMatmulDispatchState& dispatch, const ggml_tensor* weight,
                                        const ggml_tensor* input, const BatchSpec* current_batch) {
    ...
    const bool q4k_batched_prefill_shape_ready = q4k_batched_prefill_selected && (input->ne[0] % QK_K == 0);
    ...
    plan.qwen_hybrid_ssm_q8_prefill =
        ((dispatch.qwen35_hybrid_ssm && dispatch.hybrid_ssm_qkv) || dispatch.qwen35_hybrid_ssm_gate ||
         dispatch.qwen35_hybrid_ssm_out || dispatch.qwen36_hybrid_ssm_qkv || dispatch.qwen36_hybrid_ssm_gate ||
         dispatch.qwen36_hybrid_ssm_out) &&
        dispatch.prefill_phase && weight->type == GGML_TYPE_Q8_0 && input->type == GGML_TYPE_F32 && dispatch.m > 1 &&
    ...
    plan.qwen36_lm_head_q4k_prefill_relevant = dispatch.qwen36_lm_head && q4k_batched_prefill_targeted;
    if (plan.qwen36_lm_head_q4k_prefill_relevant && weight->type == GGML_TYPE_Q4_K && input->type == GGML_TYPE_F32 &&
        weight->ne[0] == input->ne[0] && dispatch.host_caps.q4k_true_batched) {
        const auto* type_traits_cpu = ggml_get_type_traits_cpu(weight->type);
        plan.qwen36_lm_head_q4k_prefill_fast_path_eligible = type_traits_cpu && type_traits_cpu->vec_dot &&
                                                             type_traits_cpu->vec_dot_type == GGML_TYPE_Q8_K &&
                                                             (input->ne[0] % QK_K == 0);
    }
    plan.lfm2_prefill_q4k_relevant = dispatch.lfm2_shortconv_semantic && q4k_batched_prefill_targeted;
    plan.lfm2_prefill_q6k_relevant = dispatch.prefill_phase && dispatch.lfm2_shortconv_semantic && dispatch.m > 1 &&
                                     input->type == GGML_TYPE_F32 && weight->type == GGML_TYPE_Q6_K;
    plan.lfm2_prefill_quant_nrc_unsafe = dispatch.prefill_phase && dispatch.lfm2_shortconv_semantic && dispatch.m > 1 &&
    ...
    plan.q4k_batched_prefill_relevant = plan.qwen36_hybrid_ssm_q4k_prefill_relevant ||
                                        plan.qwen36_lm_head_q4k_prefill_relevant || plan.lfm2_prefill_q4k_relevant;
    ...
    plan.qwen36_hybrid_ssm_q4k_prefill_probe_candidate = plan.q4k_batched_prefill_relevant &&
                                                         q4k_batched_prefill_kernel_ready &&
                                                         !plan.qwen36_hybrid_ssm_q4k_prefill_lora_active;
    return plan;
}

static SmartMatmulQuantCapabilityPlan ResolveSmartMatmulQuantCapabilityPlan(const ggml_tensor* weight,
                                                                            const ggml_tensor* input, int input_cols) {
    ...
    if ((weight->type == GGML_TYPE_Q4_K || weight->type == GGML_TYPE_Q5_K || weight->type == GGML_TYPE_Q6_K) &&
...
static SmartMatmulQ4KBatchedAdmissionPlan
ResolveSmartMatmulQ4KBatchedAdmissionPlan(const TransformerModel* model, const ggml_tensor* weight,
                                          const ggml_tensor* input, const SmartMatmulDispatchState& dispatch,
    ...
    plan.admission_key =
        prefill.qwen36_hybrid_ssm_q4k_prefill_probe_candidate
            ? HashQwen36Q4KBatchedAdmissionKey(model, weight, input, dispatch.m, dispatch.n, dispatch.k)
            : 0;
    plan.admission =
        plan.admission_key ? LookupQwen36Q4KBatchedAdmission(plan.admission_key) : Qwen36Q4KBatchedAdmissionValue{};
    const bool qwen36_q4k_relevant =
        prefill.qwen36_hybrid_ssm_q4k_prefill_relevant || prefill.qwen36_lm_head_q4k_prefill_relevant;
    plan.mode_off = qwen36_q4k_relevant && fast_path_config.qwen36_prefill_q4k_batched ==
                                               densecore::llm::config::Qwen36PrefillQ4KBatchedMode::Off;
    plan.mode_on = qwen36_q4k_relevant && fast_path_config.qwen36_prefill_q4k_batched ==
    ...
    // config directly.
    plan.mode_probe = (qwen36_q4k_relevant && fast_path_config.qwen36_prefill_q4k_batched ==
                                                  densecore::llm::config::Qwen36PrefillQ4KBatchedMode::Probe) ||
                      prefill.lfm2_prefill_q4k_relevant;
    plan.probe_rejected = plan.mode_probe && plan.admission.state == Qwen36Q4KBatchedAdmissionState::Reject;
    plan.candidate_ready = prefill.q4k_batched_prefill_relevant && quant.has_quant_vec_dot &&
                           ...
                           quant.quant_true_batched_kernel_ready && !prefill.qwen36_hybrid_ssm_q4k_prefill_lora_active;
    plan.reject_reason = ResolveQwen36PrefillQ4KBatchedReason(
        prefill.q4k_batched_prefill_relevant, plan.mode_off, prefill.qwen36_hybrid_ssm_q4k_prefill_lora_active,
        prefill.qwen36_hybrid_ssm_q4k_prefill_weight_is_q4k, prefill.qwen36_hybrid_ssm_q4k_prefill_shape_supported,
        prefill.qwen36_hybrid_ssm_q4k_prefill_kernel_available, quant.has_quant_vec_dot, plan.candidate_ready,
        plan.mode_on, plan.mode_probe, plan.admission.state);
    plan.probe_admitted =
        plan.candidate_ready && (plan.mode_on || prefill.lfm2_prefill_q4k_relevant ||
                                 (plan.mode_probe && plan.admission.state != Qwen36Q4KBatchedAdmissionState::Reject));
    plan.qwen36_prefers_ggml_quant = dispatch.prefill_phase && dispatch.qwen36_hybrid_ssm && !dispatch.qwen38_target &&
                                     dispatch.m > 1 && !prefill.qwen_hybrid_ssm_q8_prefill &&
                                     (!plan.probe_admitted || plan.mode_off || plan.probe_rejected);
    plan.lfm2_prefers_ggml_quant = dispatch.prefill_phase && dispatch.lfm2_shortconv_semantic && dispatch.m > 1 &&
                                   weight->type == GGML_TYPE_Q4_K && (!plan.probe_admitted || plan.probe_rejected);
    plan.lfm2_q4k_true_batched_admitted = prefill.lfm2_prefill_q4k_relevant && dispatch.compatible &&
                                          dispatch.m <= kMaxSmallBatchQuantCols && plan.candidate_ready &&
                                          plan.probe_admitted;
    return plan;
}

static SmartMatmulGemma4NativePlan ResolveSmartMatmulGemma4NativePlan(const TransformerModel* model,
                                                                      const ggml_tensor* weight,
                                                                      const ggml_tensor* input, const BatchSpec* batch,
    ...
    plan.dense_prefill_candidate = model && model->arch_flags.is_gemma4 && input->type == GGML_TYPE_F32 &&
                                   dispatch.m > 1 && ggml_is_quantized(weight->type) &&
    ...
    plan.decode_allowed = CanUseGemma4DecodeNative(model, weight, input, batch, dispatch_phase, &plan.decode_reject);
    plan.decode_candidate = model && model->arch_flags.is_gemma4 && input->type == GGML_TYPE_F32 && dispatch.m <= 1 &&
...
static SmartMatmulLfm2PrefillCompatibilityPlan
ResolveSmartMatmulLfm2PrefillCompatibilityPlan(const ggml_tensor* weight, const ggml_tensor* input,
                                               const SmartMatmulDispatchState& dispatch) {
    ...
    plan.q5k_q6k_or_q8_relevant =
        dispatch.prefill_phase && dispatch.lfm2_shortconv_semantic && input->type == GGML_TYPE_F32 && dispatch.m > 1 &&
        (weight->type == GGML_TYPE_Q5_K || weight->type == GGML_TYPE_Q6_K || weight->type == GGML_TYPE_Q8_0);
    plan.force_non_q4k_reference_on_arm = dispatch.prefill_phase && dispatch.lfm2_shortconv_semantic &&
                                          ggml_is_quantized(weight->type) && weight->type != GGML_TYPE_Q4_K &&
                                          !plan.q5k_q6k_or_q8_relevant && input->type == GGML_TYPE_F32 &&
                                          dispatch.m > 1 &&
#if defined(__aarch64__) || defined(_M_ARM64)
                                          true;
#else
...
static SmartMatmulAliasRoutingPlan ResolveSmartMatmulAliasRoutingPlan(const ggml_tensor* weight,
                                                                      ...
                                                                      bool using_qwen38_amx_alias) {
    SmartMatmulAliasRoutingPlan plan;
    if (!weight || !input || !using_cpu_repack_alias || using_qwen38_amx_alias) {
        return plan;
    }
...
static SmartMatmulGgmlCompatibilityPlan ResolveSmartMatmulGgmlCompatibilityPlan(
    ...
    const SmartMatmulPrefillProjectionPlan& prefill, const SmartMatmulQuantCapabilityPlan& quant,
    ...
    plan.qwen_hybrid_ssm_q8_native_prefill =
        dispatch.prefill_phase && prefill.qwen_hybrid_ssm_q8_prefill && input->type == GGML_TYPE_F32 &&
        weight->type == GGML_TYPE_Q8_0 && dispatch.m > 1 &&
        !ShouldUseArmQwen36LargeQ8Prefill(dispatch.qwen36_target && !dispatch.qwen38_target &&
                                              prefill.qwen_hybrid_ssm_q8_prefill,
                                          dispatch.m, static_cast<int>(input->ne[0]), static_cast<int>(weight->ne[1]));
    const bool qwen38_quant_custom_prefill_ready = dispatch.prefill_phase && dispatch.qwen38_target &&
                                                   dispatch.hybrid_ssm_projection && ggml_is_quantized(weight->type) &&
                                                   input->type == GGML_TYPE_F32 && dispatch.m > 1 &&
                                                   dispatch.m <= kMaxSmallBatchQuantCols && quant.has_quant_vec_dot &&
                                                   quant.has_quant_from_float && quant.quant_input_size_ok;
    plan.plain_hybrid_ssm_prefill =
        ...
        ((dispatch.hybrid_ssm_qkv && !prefill.qwen_hybrid_ssm_quant_prefill_fast_path_eligible &&
         ...
         ((dispatch.qwen36_hybrid_ssm_qkv || dispatch.qwen36_hybrid_ssm_gate || dispatch.qwen36_hybrid_ssm_out) &&
          !qwen38_quant_custom_prefill_ready)) &&
        !prefill.qwen_hybrid_ssm_q8_prefill &&
        (!prefill.qwen_hybrid_ssm_quant_prefill_fast_path_eligible || plan.qwen36_hybrid_ssm_q4k_native_prefill) &&
        ggml_is_quantized(weight->type) && input->type == GGML_TYPE_F32 && dispatch.m > 1;
    const auto* lm_head_traits = ggml_get_type_traits_cpu(weight->type);
    const bool qwen36_q6k_compact_lm_head = dispatch.prefill_phase && dispatch.qwen36_lm_head &&
                                            weight->type == GGML_TYPE_Q6_K && input->type == GGML_TYPE_F32 &&
                                            dispatch.m > 1 && dispatch.m <= 4 && lm_head_traits &&
                                            lm_head_traits->vec_dot && lm_head_traits->vec_dot_type == GGML_TYPE_Q8_K;
    plan.qwen36_lm_head_prefill = dispatch.prefill_phase && dispatch.qwen36_lm_head &&
                                  ggml_is_quantized(weight->type) && dispatch.m > 1 &&
                                  !prefill.qwen36_lm_head_q4k_prefill_fast_path_eligible && !qwen36_q6k_compact_lm_head;
    return plan;
}
...
static SmartMatmulCandidatePlan ResolveSmartMatmulCandidatePlan(
    ...
    plan.small_batch_f32 = dispatch.m > 1 && dispatch.m <= kMaxSmallBatchColsHard && input->type == GGML_TYPE_F32 &&
                           weight->type == GGML_TYPE_F32;
    plan.moe_router_f32_prefill = model && dispatch.m > 1 && input->type == GGML_TYPE_F32 &&
                                  weight->type == GGML_TYPE_F32 && dispatch.moe_router &&
                                  weight->ne[0] == input->ne[0] && weight->ne[1] <= 512 && weight->ne[0] >= 512 &&
                                  dispatch.fallback_free_target;
    plan.small_batch_quant = dispatch.m > 1 &&
                             (dispatch.m <= kMaxSmallBatchQuantCols || prefill.lfm2_prefill_q6k_relevant) &&
                             input->type == GGML_TYPE_F32 && ggml_is_quantized(weight->type) &&
    ...
    plan.true_batched_q4k =
        plan.small_batch_quant && quant.quant_true_batched_kernel_ready && weight->type == GGML_TYPE_Q4_K;
...
static SmartMatmulGgmlEmitPlan ResolveSmartMatmulGgmlEmitPlan(
    ...
    const SmartMatmulLfm2PrefillCompatibilityPlan& lfm2_compatibility, bool force_plain_hybrid_ssm_qkv,
    bool using_qwen36_ssm_q8_prefill_amx_alias, bool using_qwen38_amx_alias) {
    ...
    if (using_qwen38_amx_alias) {
        plan.active = true;
        plan.selected_path = "DENSECORE_QWEN38_AMX";
        plan.dispatch_reason = "qwen38_quant_amx";
        plan.graph_path = "custom_batched_gemv";
        plan.emit_reason = "maintained_qwen38_quant_amx";
        return plan;
    }
    ...
    if (compatibility.qwen_hybrid_ssm_q8_native_prefill) {
        plan.active = true;
        plan.selected_path = "DENSECORE_QWEN_SSM_Q8_NATIVE_PREFILL";
        plan.dispatch_reason = "qwen_hybrid_ssm_q8_native_prefill";
        plan.graph_path = "ssm_projection";
        plan.emit_reason = "maintained_qwen_hybrid_ssm_q8_native_prefill";
        return plan;
    }
...
static Qwen36PrefillQ4KBatchedRejectReason
ResolveSmartMatmulEffectiveQ4KRejectReason(const SmartMatmulPrefillProjectionPlan& prefill,
                                           const SmartMatmulQ4KBatchedAdmissionPlan& q4k_admission) {
    if (prefill.qwen36_hybrid_ssm_q4k_prefill_relevant && !prefill.qwen36_hybrid_ssm_q4k_prefill_weight_is_q4k) {
...
static const char*
ResolveSmartMatmulLfm2Q4KRequiredRejectReason(bool compatible, int input_cols,
                                              const SmartMatmulQ4KBatchedAdmissionPlan& q4k_admission,
...
static SmartMatmulPathPlan ResolveSmartMatmulPathPlan(
    const TransformerModel* model, const ggml_tensor* weight, const ggml_tensor* input, const BatchSpec* current_batch,
    const SmartMatmulDispatchState& dispatch, const densecore::llm::config::FastPathRuntimeConfig& fast_path_config,
    bool using_cpu_repack_alias, InferenceExecutionPhase dispatch_phase, bool force_plain_hybrid_ssm_qkv,
    bool using_qwen36_ssm_q8_prefill_amx_alias, bool using_qwen38_amx_alias) {
    ...
    plan.alias =
        ResolveSmartMatmulAliasRoutingPlan(weight, input, dispatch, using_cpu_repack_alias, using_qwen38_amx_alias);
    plan.gemma4_native =
        ResolveSmartMatmulGemma4NativePlan(model, weight, input, current_batch, dispatch, dispatch_phase);
    ...
    const SmartMatmulGgmlCompatibilityPlan ggml_compatibility =
        ResolveSmartMatmulGgmlCompatibilityPlan(weight, input, dispatch, plan.prefill, plan.quant, plan.q4k);
    plan.candidate =
        ResolveSmartMatmulCandidatePlan(model, weight, input, dispatch, plan.prefill, plan.quant, plan.q4k);
    plan.ggml_emit =
        ResolveSmartMatmulGgmlEmitPlan(plan.alias, ggml_compatibility, lfm2_compatibility, force_plain_hybrid_ssm_qkv,
                                       using_qwen36_ssm_q8_prefill_amx_alias, using_qwen38_amx_alias);
    return plan;
}
...
static const ggml_tensor* ResolveQwen38Q4K8x8Alias(const TransformerModel* model, const ggml_tensor* weight) {
    if (!model || !weight || model->variant != ModelVariant::QWEN38 || weight->type != GGML_TYPE_Q4_K) {
        return nullptr;
    }

    const auto valid_alias = [&](const ggml_tensor* alias) {
        return alias && alias->type == GGML_TYPE_Q4_K && alias->ne[0] == weight->ne[0] &&
               alias->ne[1] == weight->ne[1] &&
               CpuRepackAliasHasLayout(model, alias, TransformerModel::CpuRepackAliasLayout::Q4K8x8Q8K);
    };
    auto exact = model->prepared_weights.cpu_repack_aliases.find(weight);
    if (exact != model->prepared_weights.cpu_repack_aliases.end() && valid_alias(exact->second)) {
        return exact->second;
    }

    const char* weight_name = SmartMatmulTensorName(weight);
    for (const auto& [source, alias] : model->prepared_weights.cpu_repack_aliases) {
        if (!source || !valid_alias(alias)) {
            continue;
        }
        const bool same_view = weight->view_src == source || source->view_src == weight;
        const bool same_shape =
            source->type == weight->type && source->ne[0] == weight->ne[0] && source->ne[1] == weight->ne[1];
        const char* source_name = SmartMatmulTensorName(source);
        const bool same_name =
            weight_name && source_name && weight_name[0] && std::strcmp(weight_name, source_name) == 0;
        if (same_view || (same_shape && same_name)) {
            return alias;
        }
    }
    return nullptr;
...
static void ConfigureSmartMatmulBatchedUserData(GemvBatchedUserData* ud, const TransformerModel* model,
                                                const ggml_tensor* weight, const SmartMatmulDispatchState& dispatch,
                                                const SmartMatmulPrefillProjectionPlan& prefill,
                                                const SmartMatmulQ4KBatchedAdmissionPlan& q4k_admission,
                                                bool gemma4_dense_prefill_native, bool gemma4_prefill_safe_batched) {
    if (!ud || !weight) {
        return;
    }
    ud->force_reference_scalar = false;
    ud->qwen36_prefill_q4k_admission_key = q4k_admission.admission_key;
    ud->qwen36_prefill_q4k_probe = !prefill.lfm2_prefill_q4k_relevant && q4k_admission.mode_probe &&
    ...
    ud->disable_quant_nrc_fast = dispatch.matmul_plan.qwen.qwen35_0_8b || prefill.lfm2_prefill_quant_nrc_unsafe ||
                                 prefill.lfm2_prefill_q4k_relevant || gemma4_prefill_safe_batched;
    ud->gemma4_dense_prefill_native = gemma4_dense_prefill_native;
    ud->lfm2_q8_repacked_batched = dispatch.lfm2_shortconv_semantic && weight->type == GGML_TYPE_Q8_0 && dispatch.m > 1;
    ...
    // Graph construction can classify a batched projection as decode-shaped
    // even though the callback executes during prefill. Prepare the immutable
    // model-owned alias from the batch shape; the callback still gates use on
    // the authoritative execution phase.
    if (model && model->variant == ModelVariant::QWEN38 && weight->type == GGML_TYPE_Q4_K && dispatch.m >= 4) {
        if (const ggml_tensor* alias = ResolveQwen38Q4K8x8Alias(model, weight)) {
            ud->qwen38_q4k_8x8_weight = alias;
            ud->qwen38_q4k_8x8_rows = weight->ne[1];
            ud->qwen38_q4k_8x8_cols = weight->ne[0];
        }
    }
}

static SmartMatmulGemvUserDataPlan ResolveSmartMatmulGemvUserDataPlan(const TransformerModel* model,
                                                                      const ggml_tensor* weight,
                                                                      const ggml_tensor* input,
                                                                      const SmartMatmulDispatchState& dispatch) {
    ...
    plan.lfm2_tied_lm_head =
        ...
        (model->output == weight || std::strcmp(dispatch.weight_name, "token_embd.weight") == 0 ||
         std::strcmp(dispatch.weight_name, "output.weight") == 0 || std::strcmp(dispatch.weight_name, "output") == 0);
...
static void ConfigureSmartMatmulGemvUserData(GemvUserData* ud, const TransformerModel* model,
    ...
    ud->lfm2_decode_lm_head = (gemv_plan.lfm2_tied_lm_head || gemv_plan.qwen_c4a_q6k_lm_head) && matmul_expected_decode;
}

static void RecordSmartMatmulGraphCensus(InferenceWorkContext* dispatch_work_ctx,
                                         InferenceExecutionPhase dispatch_phase,
                                         const SmartMatmulOriginalTensorState& original,
                                         const SmartMatmulDispatchState& dispatch, ggml_type weight_type,
                                         bool matmul_expected_decode, const char* selected_path) {
...
static ggml_tensor* EmitMatmulFromPlan(ggml_context* ctx, ggml_tensor* weight, ggml_tensor* input,
                                       ...
                                       InferenceExecutionPhase dispatch_phase, const SmartMatmulDispatchState& dispatch,
    ...
    const bool maintained_qwen_ssm_q8_amx_prefill =
        ...
        (dispatch.qwen35_hybrid_ssm || dispatch.qwen36_hybrid_ssm) && dispatch.hybrid_ssm_projection && weight &&
        model->prepared_weights.cpu_amx_aliases.find(weight) != model->prepared_weights.cpu_amx_aliases.end();
    const bool maintained_qwen_ssm_q8_native_prefill =
        reason && std::strcmp(reason, "maintained_qwen_hybrid_ssm_q8_native_prefill") == 0 && model &&
        (model->variant == ModelVariant::QWEN35 || model->variant == ModelVariant::QWEN36) &&
        model->arch_flags.is_hybrid_ssm && dispatch_phase == InferenceExecutionPhase::Prefill &&
        (dispatch.qwen35_hybrid_ssm || dispatch.qwen36_hybrid_ssm) && dispatch.hybrid_ssm_projection && weight &&
        weight->type == GGML_TYPE_Q8_0;
    const bool maintained_qwen38_quant_amx =
        reason && std::strcmp(reason, "maintained_qwen38_quant_amx") == 0 && model &&
        model->variant == ModelVariant::QWEN38 && model->arch_flags.is_hybrid_ssm && dispatch.m > 0 && weight &&
        ggml_is_quantized(weight->type) &&
        model->prepared_weights.cpu_amx_aliases.find(weight) != model->prepared_weights.cpu_amx_aliases.end();
    if (maintained_qwen_ssm_q8_amx_prefill || maintained_qwen_ssm_q8_native_prefill || maintained_qwen38_quant_amx) {
    ...
    RecordQwenTargetGgmlComputeFallback(work_ctx, model, op, reason, dispatch.weight_name, dispatch_phase,
                                        weight ? weight->type : GGML_TYPE_COUNT, dispatch.m, dispatch.n, dispatch.k);
    if (dispatch.kernel_resolution.fallback_policy ==
        ...
        throw densecore::InvalidArgumentException(
            ...
            " semantic_op=" + densecore::runtime::DenseCoreSemanticOpName(dispatch.kernel_resolution.semantic_op) +
...
struct ggml_tensor* smart_mul_mat(struct ggml_context* ctx, struct ggml_tensor* weight, struct ggml_tensor* input,
    ...
    const bool using_qwen36_ssm_q8_prefill_amx_alias = execution_plan.using_qwen36_ssm_q8_prefill_amx_alias;
    const bool using_qwen38_amx_alias = execution_plan.using_qwen38_amx_alias;
    ...
    const bool force_plain_hybrid_ssm_qkv = dispatch_state.hybrid_ssm_qkv && ShouldForcePlainGgmlForHybridSSMQkv();
    if (matmul_plan.target_qwen_hot_path && IsDebugMatmulDispatchEnabled()) {
        LogMatmulDispatch(w_name, MatmulWeightTypeLabel(weight->type, false, false), M, N_dim, K_dim, "DENSECORE_PLAN",
    ...
    const auto emit_ggml_matmul = [&](const SmartMatmulGgmlEmitPlan& route) -> ggml_tensor* {
        LogMatmulDispatch(w_name,
                          route.type_label ? route.type_label : MatmulWeightTypeLabel(weight->type, false, false), M,
                          N_dim, K_dim, route.selected_path, route.dispatch_reason);
        if (route.log_hybrid) {
        ...
        return EmitMatmulFromPlan(ctx, weight, input, model, dispatch_work_ctx, dispatch_phase, dispatch_state,
                                  densecore::runtime::GgmlComputeOp::Matmul, route.emit_reason);
    };
    ...
    const SmartMatmulPathPlan path_plan = ResolveSmartMatmulPathPlan(
        ...
        force_plain_hybrid_ssm_qkv, using_qwen36_ssm_q8_prefill_amx_alias, using_qwen38_amx_alias);
        ...
        if (it_int4 != model->int4_weight_bindings.end()) {
            ...
            if (binding.k > 0 && binding.n > 0 && binding.group_size > 0 && binding.packed && binding.scales &&
                ...
                if (dispatch_work_ctx && model->arch_flags.is_gemma4 && M <= 1) {
                    (*GetInferenceWorkContextProfile(dispatch_work_ctx))
                        .gemma4_native_int4_gemv_candidate_ops.fetch_add(1, std::memory_order_relaxed);
                }
    ...
    if (candidate_plan.gemv && is_compatible) {
        ...
        if (dispatch_work_ctx && gemv_userdata_plan.gemma4_model && ggml_is_quantized(weight->type)) {
            (*GetInferenceWorkContextProfile(dispatch_work_ctx))
                .ggml_delegated_quant_gemv_ops.fetch_add(1, std::memory_order_relaxed);
        }
        LogHybridSSMQkvDispatch(w_name, weight->type, M, N_dim, K_dim,
    ...
    if (candidate_plan.small_batch && is_compatible) {
        ...
        ConfigureSmartMatmulBatchedUserData(ud, model, weight, dispatch_state, prefill_projection_plan,
        ...
        if (gemma4_native_plan.dense_prefill_allowed) {
            RecordGemma4DensePrefillNativeDecision(dispatch_work_ctx ? dispatch_work_ctx : GetCurrentWorkContext(),
                                                   /*candidate=*/false,
                                                   /*used=*/true, nullptr, weight->type, /*replaced_mul_mat_ops=*/1,
                                                   /*duplicate_work_detected=*/false);
        }
        const char* batched_path_label =
    ...
    if (IsDebugMatmulDispatchEnabled() && input_cols > 1 && ggml_is_quantized(weight->type) && is_compatible &&
        !candidate_plan.small_batch_quant) {
        LogMatmulDispatch(w_name, wtype_label, M, N_dim, K_dim, "GGML_FALLBACK", candidate_plan.quant_reject_reason);
    }
...
bool ShouldUseGemma4PrefillSafeBatchedForTest(bool is_gemma4, bool prefill_phase, int m, bool dense_prefill_allowed) {
    TransformerModel model{};
    model.arch_flags.is_gemma4 = is_gemma4;
    SmartMatmulDispatchState dispatch{};
    dispatch.prefill_phase = prefill_phase;
    dispatch.m = m;
    return ::ShouldUseGemma4PrefillSafeBatched(&model, dispatch, dense_prefill_allowed);
}
}  // namespace densecore::testing
#endif
