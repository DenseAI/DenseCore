...
bool IsQwen36ProfilingEnabled() {
    static const bool enabled = []() { return densecore::env::ParseDiagnosticEnv("DENSECORE_QWEN36_PROFILE"); }();
    return enabled;
}
...
static const char* MatmulModelFamilyName(ModelVariant variant) {
    ...
    case ModelVariant::QWEN38: return "qwen38";
    case ModelVariant::GEMMA4: return "gemma4";
    case ModelVariant::LFM2MOE: return "lfm2";
...
static const char* MatmulPathHistLabel(std::size_t index) {
    switch (index) {
    case 0: return "ggml_mul_mat";
    case 1: return "ggml_mul_mat_id";
    case 2: return "custom_gemv";
    case 3: return "custom_batched_gemv";
    case 4: return "moe_native";
    case 5: return "ssm_projection";
    default: return "other";
    }
}
...
const char* Qwen35MoEPathName(int code) {
    switch (code) {
    case 1: return "native_graph";
    case 2: return "cb_moe_forward";
    case 3: return "ggml_mul_mat_id";
    case 4: return "fused_rowblock";
    case 5: return "other";
    case 0:
    default: return "none";
    }
...
const char* Q6KGemvRejectReasonName(int code) {
    switch (code) {
    case 1: return "not_q6_k";
    case 2: return "not_decode";
    case 3: return "kernel_unavailable";
    case 4: return "unsupported_shape";
    case 5: return "fallback";
    case 6: return "disabled";
    case 7: return "dynamic_lora";
    case 8: return "reference_forced";
    case 9: return "quant_input_null";
    case 10: return "unsupported_quant_input";
...
template <typename THist> static void ResetAtomicHistogram(THist& hist) {
    for (auto& value : hist) {
...
void ResetQwen36Profile(InferenceWorkContext* ctx) {
    ...
    // Profiling counters do not own the model identity of a cached graph.
    ...
    p.kleidiai_compiled_enabled.store(densecore::runtime::KleidiAICompiledEnabled() ? 1 : 0, std::memory_order_relaxed);
    p.kleidiai_last_reject_reason.store(static_cast<int>(densecore::runtime::KernelAdmissionRejectReason::None),
...
Qwen36ProfileSnapshot GetQwen36ProfileSnapshot(const InferenceWorkContext* ctx) {
    ...
    snapshot.q4k_repacked_gemv_cache_evictions = p.q4k_repacked_gemv_cache_evictions.load(std::memory_order_relaxed);
    ...
    snapshot.q4k_repacked_gemv_resident_bytes = p.q4k_repacked_gemv_resident_bytes.load(std::memory_order_relaxed);
    ...
    snapshot.q8_batched_activation_quant_ns = p.q8_batched_activation_quant_ns.load(std::memory_order_relaxed);
    ...
    snapshot.qwen36_prefill_q4k_probe_failures = p.qwen36_prefill_q4k_probe_failures.load(std::memory_order_relaxed);
    ...
    snapshot.qwen36_ssm_q8_prefill_amx_prepared = p.qwen36_ssm_q8_prefill_amx_prepared.load(std::memory_order_relaxed);
    ...
    snapshot.qwen36_ssm_q8_prefill_amx_used_ops = p.qwen36_ssm_q8_prefill_amx_used_ops.load(std::memory_order_relaxed);
    ...
    SnapshotAtomicHistogram(snapshot.qwen36_ssm_projection_weight_type_hist, p.qwen36_ssm_projection_weight_type_hist);
    ...
    snapshot.q4k_repacked_gemv_candidate_ops = p.q4k_repacked_gemv_candidate_ops.load(std::memory_order_relaxed);
    snapshot.q4k_repacked_gemv_used_ops = p.q4k_repacked_gemv_used_ops.load(std::memory_order_relaxed);
    snapshot.q4k_repacked_gemv_rejected_ops = p.q4k_repacked_gemv_rejected_ops.load(std::memory_order_relaxed);
    ...
    snapshot.lfm2_decode_lm_head_custom_gemv_ns = p.lfm2_decode_lm_head_custom_gemv_ns.load(std::memory_order_relaxed);
    ...
    snapshot.lfm2_greedy_lm_head_argmax_ns = p.lfm2_greedy_lm_head_argmax_ns.load(std::memory_order_relaxed);
    ...
    snapshot.lfm2_w1w3_q4k_repacked_used_ops = p.lfm2_w1w3_q4k_repacked_used_ops.load(std::memory_order_relaxed);
    ...
    snapshot.qwen_target_ggml_quant_vecdot_ops = p.qwen_target_ggml_quant_vecdot_ops.load(std::memory_order_relaxed);
    snapshot.qwen_target_ggml_quantize_kv_ops = p.qwen_target_ggml_quantize_kv_ops.load(std::memory_order_relaxed);
    ...
    snapshot.moe_small_decode_parallel_used_ops = p.moe_small_decode_parallel_used_ops.load(std::memory_order_relaxed);
    ...
    snapshot.gemma4_native_moe_prefill_down_ns = p.gemma4_native_moe_prefill_down_ns.load(std::memory_order_relaxed);
    snapshot.gemma4_native_moe_prefill_total_ns = p.gemma4_native_moe_prefill_total_ns.load(std::memory_order_relaxed);
    ...
    snapshot.gemma4_dense_prefill_native_ns = p.gemma4_dense_prefill_native_ns.load(std::memory_order_relaxed);
    ...
    snapshot.gemma4_decode_native_candidate_ops = p.gemma4_decode_native_candidate_ops.load(std::memory_order_relaxed);
    snapshot.gemma4_decode_native_used_ops = p.gemma4_decode_native_used_ops.load(std::memory_order_relaxed);
    snapshot.gemma4_decode_native_rejected_ops = p.gemma4_decode_native_rejected_ops.load(std::memory_order_relaxed);
    snapshot.gemma4_decode_native_moe_used_ops = p.gemma4_decode_native_moe_used_ops.load(std::memory_order_relaxed);
    ...
    snapshot.gemma4_native_int4_gemv_used_ops = p.gemma4_native_int4_gemv_used_ops.load(std::memory_order_relaxed);
    ...
    snapshot.gemma4_native_int4_repacked_bytes = p.gemma4_native_int4_repacked_bytes.load(std::memory_order_relaxed);
    ...
    snapshot.gemma4_native_paged_attention_ns = p.gemma4_native_paged_attention_ns.load(std::memory_order_relaxed);
    snapshot.gemma4_ggml_attention_fallback_ops = p.gemma4_ggml_attention_fallback_ops.load(std::memory_order_relaxed);
    snapshot.gemma4_paged_attention_cache_type = p.gemma4_paged_attention_cache_type.load(std::memory_order_relaxed);
    snapshot.gemma4_paged_attention_context_len = p.gemma4_paged_attention_context_len.load(std::memory_order_relaxed);
    snapshot.gemma4_paged_attention_head_range = p.gemma4_paged_attention_head_range.load(std::memory_order_relaxed);
    snapshot.native_moe_fast_decode_candidate_ops =
        p.native_moe_fast_decode_candidate_ops.load(std::memory_order_relaxed);
    snapshot.native_moe_fast_decode_used_ops = p.native_moe_fast_decode_used_ops.load(std::memory_order_relaxed);
    ...
    snapshot.native_moe_fast_decode_w2_used_ops = p.native_moe_fast_decode_w2_used_ops.load(std::memory_order_relaxed);
    ...
    snapshot.native_moe_fast_w1w3_used_ops = p.native_moe_fast_w1w3_used_ops.load(std::memory_order_relaxed);
    ...
    snapshot.native_moe_fast_w2_q5k_used_ops = p.native_moe_fast_w2_q5k_used_ops.load(std::memory_order_relaxed);
    ...
    snapshot.qwen_native_moe_fused_router_ns = p.qwen_native_moe_fused_router_ns.load(std::memory_order_relaxed);
    ...
    snapshot.qwen35_moe_selected_expert_count = p.qwen35_moe_selected_expert_count.load(std::memory_order_relaxed);
    snapshot.qwen35_moe_top_k = p.qwen35_moe_top_k.load(std::memory_order_relaxed);
    snapshot.qwen35_moe_instrumentation_missing = p.qwen35_moe_instrumentation_missing.load(std::memory_order_relaxed);
    snapshot.qwen36_prefill_total_ns = p.qwen36_prefill_total_ns.load(std::memory_order_relaxed);
    snapshot.qwen36_prefill_ssm_projection_ns = p.qwen36_prefill_ssm_projection_ns.load(std::memory_order_relaxed);
    snapshot.qwen36_prefill_ssm_delta_state_ns = p.qwen36_prefill_ssm_delta_state_ns.load(std::memory_order_relaxed);
    ...
    {
        std::lock_guard<std::mutex> lock(ctx->profile_string_mutex);
        snapshot.moe_small_decode_parallel_last_reject_reason = ctx->moe_small_decode_parallel_last_reject_reason;
        ...
        snapshot.gemma4_native_moe_prefill_last_reject_reason = ctx->gemma4_native_moe_prefill_last_reject_reason;
        snapshot.gemma4_dense_prefill_native_last_reject_reason = ctx->gemma4_dense_prefill_native_last_reject_reason;
        snapshot.gemma4_decode_native_last_reject_reason = ctx->gemma4_decode_native_last_reject_reason;
        snapshot.qwen_target_ggml_compute_last_reason = ctx->qwen_target_ggml_compute_last_reason;
        snapshot.qwen_target_ggml_compute_last_op = ctx->qwen_target_ggml_compute_last_op;
        snapshot.qwen_target_ggml_compute_target = ctx->qwen_target_ggml_compute_target;
        snapshot.native_moe_fast_decode_last_reject_reason = ctx->native_moe_fast_decode_last_reject_reason;
        snapshot.native_moe_fast_w2_q5k_last_reject_reason = ctx->native_moe_fast_w2_q5k_last_reject_reason;
    ...
    snapshot.flash_attention_avx512_tiled_calls = p.flash_attention_avx512_tiled_calls.load(std::memory_order_relaxed);
...
void RecordKleidiAIAdmissionDecision(InferenceWorkContext* ctx,
                                     const densecore::runtime::KernelAdmissionDecision& decision) {
    if (!ctx) {
        return;
    }
    auto& p = ctx->qwen36_profile;
...
void RecordQwen36SSMProjectionWeightType(InferenceWorkContext* ctx, ggml_type weight_type) {
    ...
    ctx->qwen36_profile.qwen36_ssm_projection_weight_type_hist[MatmulWeightTypeHistIndex(weight_type)].fetch_add(
...
void RecordMoEExpertMatmulWeightType(InferenceWorkContext* ctx, ggml_type weight_type) {
    ...
    ctx->qwen36_profile.moe_expert_matmul_weight_type_hist[MatmulWeightTypeHistIndex(weight_type)].fetch_add(
        1, std::memory_order_relaxed);
}
...
void RecordGemma4DensePrefillNativeDecision(InferenceWorkContext* ctx, bool candidate, bool used,
    ...
    if (replaced_mul_mat_ops > 0) {
        p.gemma4_dense_prefill_replaced_ggml_mul_mat_ops.fetch_add(replaced_mul_mat_ops, std::memory_order_relaxed);
    }
    if (duplicate_work_detected) {
...
void RecordGemma4DecodeNativeDecision(InferenceWorkContext* ctx, bool candidate, bool used, const char* reject_reason,
    ...
    if (replaced_mul_mat_id_ops > 0) {
        p.gemma4_decode_replaced_ggml_mul_mat_id_ops.fetch_add(replaced_mul_mat_id_ops, std::memory_order_relaxed);
    }
    if (duplicate_work_detected) {
...
void RecordNativeMoEFastDecodeDecision(InferenceWorkContext* ctx, bool candidate, bool used, const char* reject_reason,
                                       bool w1w3_used, bool w2_used, uint64_t wall_ns) {
    if (!ctx) {
        return;
    }
    auto& p = ctx->qwen36_profile;
...
void RecordQwenTargetGgmlComputeFallback(InferenceWorkContext* ctx, const TransformerModel* model,
                                         ...
                                         const char* tensor_name, InferenceExecutionPhase phase, ggml_type weight_type,
                                         int m, int n, int k) {
    ...
    if (densecore::runtime::ShouldRejectTargetGgmlCompute(plan, reason)) {
        ...
        message += " weight_type=";
        message += ggml_type_name(weight_type);
        message += " shape=M" + std::to_string(m) + "xN" + std::to_string(n) + "xK" + std::to_string(k);
...
void RecordGemma4NativeInt4Timing(InferenceWorkContext* ctx, uint64_t elapsed_ns) {
    if (!ctx) return;
    ctx->qwen36_profile.gemma4_native_int4_gemv_used_ops.fetch_add(1, std::memory_order_relaxed);
    if (IsQwen36ProfilingEnabled() && elapsed_ns != 0) {
        ctx->qwen36_profile.gemma4_native_int4_gemv_ns.fetch_add(elapsed_ns, std::memory_order_relaxed);
    }
}
...
void RecordQ6KGemvDecision(InferenceWorkContext* ctx, bool candidate, bool used, const char* reject_reason,
    ...
    }
    {
        ...
        entry.dispatch_path = used ? (dispatch_path && dispatch_path[0] ? dispatch_path : "q6k_direct_vecdot")
...
void RecordQwen35MoEGraphPath(InferenceWorkContext* ctx, const char* path, int top_k, int selected_expert_count,
    ...
    while (current_top_k < top_k && !p.qwen35_moe_top_k.compare_exchange_weak(
                                        current_top_k, top_k, std::memory_order_relaxed, std::memory_order_relaxed)) {}
