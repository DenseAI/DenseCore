...
static void cb_lfm2_shortconv_inout_q4k_decode(struct ggml_tensor* dst, int ith, int nth, void* userdata) {
    auto* ud = static_cast<LFM2ShortConvUserData*>(userdata);
    ...
    if (input->type != GGML_TYPE_F32 || in_proj->type != GGML_TYPE_Q4_K || out_proj->type != GGML_TYPE_Q4_K ||
        dst->type != GGML_TYPE_F32 || input->ne[1] != 1 || dst->ne[1] != 1 || ud->channels <= 0 ||
        ud->kernel <= 1 || in_proj->ne[0] != input->ne[0] || in_proj->ne[1] < 3 * ud->channels ||
        out_proj->ne[0] != ud->channels || dst->ne[0] != out_proj->ne[1]) {
        return;
    }
    const int task_count = std::max(1, nth);
    if (ith < 0 || ith >= task_count) {
    ...
    const auto* q8_traits = ggml_get_type_traits_cpu(GGML_TYPE_Q8_K);
    if (!q8_traits || !q8_traits->from_float || input_q8_row_size == 0 || y_q8_row_size == 0 ||
        (in_rows % 8) != 0 || (in_cols % ggml_blck_size(GGML_TYPE_Q4_K)) != 0 ||
        (out_dim % 8) != 0 || (out_cols % ggml_blck_size(GGML_TYPE_Q4_K)) != 0 ||
        !densecore::kernels::Q4KRepackedGemvIsaSupported() ||
        !densecore::kernels::Q4KRealPackedGemvKernelAvailable()) {
        return;
    }
    ...
    const uint64_t stamp = reinterpret_cast<uintptr_t>(dst->data) ^
                           (reinterpret_cast<uintptr_t>(input->data) << 1) ^
                           (reinterpret_cast<uintptr_t>(in_proj->data) << 7) ^
                           (reinterpret_cast<uintptr_t>(out_proj->data) << 13) ^
                           (static_cast<uint64_t>(ud->token_positions ? ud->token_positions[0] : 0) *
                            0x9e3779b97f4a7c15ull);
    InferenceWorkContext* work_ctx = ud->work_ctx;
    if (ith == 0) {
    ...
    if (out_packed &&
        densecore::kernels::RunQ4KRepackedGemv(out_packed, ud->decode_y_q8.data(), y_q8_row_size,
                                               reinterpret_cast<float*>(dst->data), out_dim, out_tile_start,
                                               out_tile_end)) {
        if (ith == 0 && work_ctx) {
            work_ctx->qwen36_profile.q4k_repacked_gemv_used.store(1, std::memory_order_relaxed);
            work_ctx->qwen36_profile.q4k_repacked_gemv_used_ops.fetch_add(2, std::memory_order_relaxed);
            work_ctx->qwen36_profile.q4k_repacked_gemv_last_reject_reason.store(
                static_cast<int>(Q4KRepackedGemvRejectReason::None), std::memory_order_relaxed);
        }
    }
