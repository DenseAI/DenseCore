...
static void cb_ssm_conv1d(struct ggml_tensor* dst, const struct ggml_tensor* src, int ith, int nth, void* userdata) {
    ...
    const auto finish_profile = [&]() {
        ...
        AddQwen36ProfileNs(ud_profile->profile->ssm_conv1d_ns,
                           static_cast<uint64_t>(
...
static void cb_lfm2_shortconv_out_q4k_decode(struct ggml_tensor* dst, int ith, int nth, void* userdata) {
    auto* ud = static_cast<LFM2ShortConvUserData*>(userdata);
    ...
    const uint64_t stamp = reinterpret_cast<uintptr_t>(dst->data) ^
                           (reinterpret_cast<uintptr_t>(bcx_tensor->data) << 1) ^
                           (reinterpret_cast<uintptr_t>(out_proj->data) << 7) ^
                           (static_cast<uint64_t>(ud->token_positions ? ud->token_positions[0] : 0) *
    ...
    const bool repacked_shape_ok = (out_dim % 8) == 0 &&
                                   (input_dim % ggml_blck_size(GGML_TYPE_Q4_K)) == 0;
    if (ith == 0 && work_ctx) {
        work_ctx->qwen36_profile.q4k_repacked_gemv_seen_ops.fetch_add(1, std::memory_order_relaxed);
    }
    const bool repacked_available =
        repacked_shape_ok && densecore::kernels::Q4KRepackedGemvIsaSupported() &&
        densecore::kernels::Q4KRealPackedGemvKernelAvailable();
    if (repacked_available) {
        if (ith == 0 && work_ctx) {
            work_ctx->qwen36_profile.q4k_repacked_gemv_candidate_ops.fetch_add(1, std::memory_order_relaxed);
        }
        if (ith == 0) {
            if (!ud->decode_out_proj_q4k_packed || ud->decode_out_proj_weight_data != weight_data ||
                ...
                ud->decode_out_proj_q4k_packed =
                    densecore::kernels::GetOrCreateQ4KRepackedGemvWeight(weight_data, out_dim, input_dim,
                                                                         &cache_lookup);
                ...
                if (!ud->decode_out_proj_q4k_packed && work_ctx) {
                    ...
                    work_ctx->qwen36_profile.q4k_repacked_gemv_last_reject_reason.store(static_cast<int>(reject),
                                                                                        std::memory_order_relaxed);
                }
            }
        ...
        if (packed && densecore::kernels::RunQ4KRepackedGemv(packed, q8, q8_row_size, output, out_dim, tile_start,
                                                             tile_end)) {
            if (ith == 0 && work_ctx) {
                work_ctx->qwen36_profile.q4k_repacked_gemv_used.store(1, std::memory_order_relaxed);
                work_ctx->qwen36_profile.q4k_repacked_gemv_used_ops.fetch_add(1, std::memory_order_relaxed);
        ...
        const auto reject = repacked_shape_ok
...
void cb_ssm_qwen35_delta_z_qkv_alpha_beta(struct ggml_tensor* dst, const struct ggml_tensor* z,
                                          const struct ggml_tensor* qkv, const struct ggml_tensor* alpha_beta,
                                          int ith, int nth, void* userdata) {
    auto* ud = static_cast<SSMQwen35DeltaUserData*>(userdata);
...
void cb_ssm_qwen35_delta(struct ggml_tensor* dst, const struct ggml_tensor* a, const struct ggml_tensor* b,
    ...
    const int head_begin = (num_v_heads * ith) / task_count;
    const int head_end = (num_v_heads * (ith + 1)) / task_count;
    ...
    const bool qkv_direct_canonical =
        projection_contract.raw_layout == Qwen35SSMQkvRawLayout::QKV &&
    ...
    for (int t = 0; t < N; ++t) {
        ...
        for (int h = head_begin; h < head_end; ++h) {
            ...
            if (alpha_beta && alpha_beta_stride >= 2 * num_v_heads + 3 * num_k_heads) {
                const float* qk_norm_t =
                    alpha_beta + static_cast<ptrdiff_t>(t) * alpha_beta_stride + 2 * num_v_heads;
            ...
            if (ssm_nonfinite_debug) {
                ...
                CheckSSMFiniteScalar(ud->layer_idx, t, seq_idx, h, "alpha_projection", "alpha", step_stats.alpha,
                                     dbg);
                CheckSSMFiniteScalar(ud->layer_idx, t, seq_idx, h, "beta_projection", "beta", step_stats.beta, dbg);
                CheckSSMFiniteScalar(ud->layer_idx, t, seq_idx, h, "decay", "softplus_alpha",
                                     step_stats.softplus_alpha, dbg);
                ...
                CheckSSMFiniteScalar(ud->layer_idx, t, seq_idx, h, "qk_norm", "q_sum_sq", step_stats.q_sum_sq,
                                     dbg);
                CheckSSMFiniteScalar(ud->layer_idx, t, seq_idx, h, "qk_norm", "k_sum_sq", step_stats.k_sum_sq,
                                     dbg);
                ...
                CheckSSMFiniteVector(ud->layer_idx, t, seq_idx, h, "state_update", "state", state, state_stride,
                                     dbg);
                ...
                CheckSSMFiniteVector(ud->layer_idx, t, seq_idx, h, "y_post_norm", "y_head", y_head, head_v_dim,
                                     dbg);
            }
