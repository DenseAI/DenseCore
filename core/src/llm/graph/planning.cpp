...
bool IsQwen35FamilyHybridSSM(const TransformerModel* model) {
    return model && model->arch_flags.is_hybrid_ssm &&
           (model->variant == ModelVariant::QWEN35 || model->variant == ModelVariant::QWEN36 ||
            model->variant == ModelVariant::QWEN38 || model->arch == ModelArch::QWEN35);
}
...
densecore::TransformerGraphExecutionPlan densecore::llm::graph::ResolveExecutionPlan(const TransformerModel* model) {
    ...
    const bool qwen_hybrid_ssm = IsQwen35FamilyHybridSSM(model);
    if (qwen_hybrid_ssm) {
        ...
        capabilities.required_prefill_logits_policies.push_back(
            models::DecoderPrefillLogitsPolicy::LastTokenEnvDefaultOn);
