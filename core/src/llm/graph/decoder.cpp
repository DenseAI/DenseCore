...
struct ggml_tensor* BuildTransformerGraphInlineImpl(TransformerModel* model, PagedKVCache* cache,
    ...
    auto apply_weighted_rms_norm = [&](struct ggml_tensor * src, struct ggml_tensor * norm_weight,
        ...
        const bool use_qwen_weighted_rms_norm =
            ...
            src->type == GGML_TYPE_F32 && effective_norm_weight->type == GGML_TYPE_F32 &&
            src->nb[0] == static_cast<int64_t>(sizeof(float));
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        if (ShouldRunHiddenSnapshotProbe(il, "layer_input")) {
            ...
            if (hidden_ud) {
                ...
                hidden_ud->token_idx =
                    densecore::env::ParseDiagnosticEnvInt("DENSECORE_DEBUG_HIDDEN_SNAPSHOT_TOKEN", -1);
        ...
        if (is_conv_layer) {
            ...
            conv_ud->profile = GetInferenceWorkContextProfile(current_work_ctx);
            ...
            const bool lfm2_decode_fused_inout =
                ...
                out_proj->type == GGML_TYPE_Q4_K && in_proj->ne[1] >= 3 * channels && in_proj->ne[0] == cur->ne[0] &&
                out_proj->ne[0] == channels && densecore::kernels::Q4KRepackedGemvIsaSupported() &&
            ...
            const bool lfm2_decode_fused_out = !mixer_out && N == 1 && bcx->type == GGML_TYPE_F32 &&
                                               out_proj->type == GGML_TYPE_Q4_K && out_proj->ne[0] == channels;
            ...
            if (IsSSMNonFiniteDebugEnabled()) {
                auto cb_check_ssm_input = [](struct ggml_tensor* dst, const struct ggml_tensor* src, int ith, int nth,
                    ...
                    const int* seq_ids =
                        (GetCurrentWorkContext() && GetCurrentBatch()) ? GetCurrentBatch()->seq_id.data() : nullptr;
            ...
            if ((model->variant == ModelVariant::QWEN35 || model->variant == ModelVariant::QWEN36 ||
                ...
                if (ggml_tensor* fused_w = layer.Get("attn_qkv_gate.cpu_fused_decode")) {
                    ...
                    ggml_set_name(fused_qkv_gate,
                                  model->variant == ModelVariant::QWEN38   ? "qwen38_ssm_qkv_gate_fused_proj"
                                  : model->variant == ModelVariant::QWEN36 ? "qwen36_ssm_qkv_gate_fused_proj"
            ...
            } else if (model->variant == ModelVariant::QWEN38) {
                ggml_set_name(qkv_mixed, "qwen38_ssm_qkv_proj");
            }
            if (IsDebugSSMQkvReferenceEnabled() || IsDebugSSMProjectionReferenceEnabled()) {
                ...
                struct ggml_tensor* qkv_probe =
                    ggml_map_custom1(ctx_c, qkv_mixed, cb_projection_reference_probe, 1, qkv_ref_ud);
                if (gf) {
            ...
            // Qwen3.5-family gated-delta blocks apply SiLU after SSM conv before the recurrent block.
            ...
            conv_ud->token_seq_ids = batch.seq_id.data();
            conv_ud->runtime_states = &batch.hybrid_ssm_runtime_states;
            conv_ud->profile = GetInferenceWorkContextProfile(GetCurrentWorkContext());
            const int ssm_conv_tasks =
                ResolveSsmConvTaskCount(model->variant, batch.num_seqs, N, conv_channels,
                                        ResolveInferenceConfig(&batch).num_threads, IsSerialMultiSequenceSsmForced());
            ...
            if (model->variant == ModelVariant::QWEN35) {
                ...
                ggml_set_name(qkv_conv, "qwen38_ssm_conv1d");
            }
            ...
            if (model->variant == ModelVariant::QWEN36) {
                ...
                ggml_set_name(z, "qwen38_ssm_gate_proj");
            }
            ...
            scan_ud->projection_profile =
                model->variant == ModelVariant::QWEN38   ? Qwen35SSMQkvProjectionProfile::QWEN38_OFFICIAL
                : model->variant == ModelVariant::QWEN36 ? Qwen35SSMQkvProjectionProfile::QWEN36_OFFICIAL
            ...
            scan_ud->profile = GetInferenceWorkContextProfile(GetCurrentWorkContext());
            ...
            const bool precompute_qwen_hybrid_ssm_scalars = qwen_hybrid_ssm_model && batch.num_seqs == 1 && N > 1;
            if (precompute_qwen_hybrid_ssm_scalars) {
                ...
                const int alpha_beta_tasks = ResolveTaskCount(&batch, std::max(1, N * (num_v_heads + n_groups)));
                ...
                if (model->variant == ModelVariant::QWEN35) {
                    ...
                    ggml_set_name(alpha_beta, "qwen38_ssm_alpha_beta_qk");
                }
                scan_ud->alpha_beta_tensor = alpha_beta;
            }
            const int ssm_delta_tasks =
                ResolveSsmDeltaTaskCount(model->variant, batch.num_seqs, num_v_heads,
                                         ResolveInferenceConfig(&batch).num_threads, IsSerialMultiSequenceSsmForced());
            ...
            if (model->variant == ModelVariant::QWEN35) {
                ...
                ggml_set_name(y, "qwen38_ssm_delta");
            }
            ...
            if (model->variant == ModelVariant::QWEN36) {
                ...
                ggml_set_name(cur, "qwen38_ssm_out_proj");
            }
            ...
            if (IsDebugSSMProjectionReferenceEnabled()) {
                ...
                struct ggml_tensor* out_probe =
                    ggml_map_custom1(ctx_c, cur, cb_projection_reference_probe, 1, out_ref_ud);
                if (gf) {
            ...
            if (model->arch_flags.is_gemma4 && IsFusedQKVEnabled() && cur->type == GGML_TYPE_F32) {
                ...
                if (gemma4_fused_qkv_repack && (gemma4_fused_qkv_repack->ne[0] != cur->ne[0] ||
            ...
            if (!KQV) {
                ...
                const auto attention_dispatch = densecore::llm::attention::ResolveDecodeAttentionDispatchDecision(
                    ...
                    IsFlashAttentionIsaSupported(), true, densecore::OpsRegistry::IsInitialized(),
                    /*force_safe_gqa_decode=*/true, -1, ggml_is_contiguous(Qcur), ggml_is_contiguous(K_all),
                ...
                if (use_cache && !use_paged_decode_attention) {
                    ...
                    *k_ud = {cache, kv_cache_layer, head_dim_kv, true};  // batch accessed via GetCurrentBatch()
                    k_ud->work_ctx = GetCurrentWorkContext();
                    ...
                    *v_ud = {cache, kv_cache_layer, head_dim_v, false};  // batch accessed via GetCurrentBatch()
                    v_ud->work_ctx = GetCurrentWorkContext();
                ...
                if (IsQwen36ProfilingEnabled()) {
                    if (InferenceWorkContext* work_ctx = GetCurrentWorkContext()) {
                        if (use_paged_decode_attention) {
                            ...
                            MarkQwen36ProfileFlag(GetInferenceWorkContextProfile(work_ctx)->attention_path_hal);
                        } else if (attention_dispatch.use_portable_cpu_flash_attention) {
                            MarkQwen36ProfileFlag(
                                GetInferenceWorkContextProfile(work_ctx)->attention_path_portable_flash);
                        } else if (attention_dispatch.use_flash_attention) {
                            MarkQwen36ProfileFlag(
                                GetInferenceWorkContextProfile(work_ctx)->attention_path_native_flash);
                        } else {
                            MarkQwen36ProfileFlag(GetInferenceWorkContextProfile(work_ctx)->attention_path_standard);
                        }
                    }
                }

                if (use_paged_decode_attention) {
                    KQV = ExecutePagedDecodeAttentionPath(
                        ctx_c, model, cache, Qcur, Kcur, Vcur, il, kv_cache_layer, gemma4_shared_kv_layer,
                        gemma4_shared_kv_source_layer, head_dim_q, head_dim_v, n_head, n_head_kv, n_total_tokens,
                        fast_attn_sliding_window, fast_attn_logit_softcap, use_explicit_attention_scale);
                } else if (attention_dispatch.use_hal_attention_dispatch) {
            ...
            if (ShouldCaptureAttentionForTest(il)) {
                KQV_merged = ggml_map_custom1(ctx_c, KQV_merged, cb_test_capture_attention_tensor, 1,
                                              AttentionCaptureUserDataForTest());
            }
#endif
            ...
            if (il == static_cast<int>(model->layers.size()) - 1 && ShouldUsePrefillLastLogitsOnly(model, batch, N)) {
                ...
                const size_t residual_last_token_offset = static_cast<size_t>(N - 1) * static_cast<size_t>(inpL->nb[1]);
        ...
        if (layer_uses_moe) {
            ...
            densecore::ComputeBackend* moe_preferred_backend = moe_backend_registry.Get(ResolvePreferredDevice(&batch));
            ...
            if (!native_moe_backend) {
                native_moe_backend = &densecore::llm::runtime::ResolveCpuBackend(GetCurrentBatch());
            }
            if (native_moe_backend) {
            ...
            if (native_moe) {
                ...
                if (moe_ud) {
                    moe_ud->model = model;
                    moe_ud->work_ctx = GetCurrentWorkContext();
                    moe_ud->layer = &model->layers[il];
                    moe_ud->layer_idx = il;
                    moe_ud->k = moe_top_k;
                    if (IsQwen36ProfilingEnabled()) {
                        if (InferenceWorkContext* work_ctx = GetCurrentWorkContext()) {
                            moe_ud->profile = GetInferenceWorkContextProfile(work_ctx);
                            SetQwen36ProfileMax(GetInferenceWorkContextProfile(work_ctx)->moe_task_count, 1);
                        }
                    }
                    ...
                    if (!moe_ud->backend) {
                        moe_ud->backend =
                            dynamic_cast<densecore::CpuBackend*>(registry.Get(densecore::DeviceType::CPU));
                    }
                    if (!moe_ud->backend) {
                        moe_ud->backend = &densecore::llm::runtime::ResolveCpuBackend(GetCurrentBatch());
                    }
                    if (moe_ud->backend) {
                ...
                RecordMoEGraphWiring();
                if (model->variant == ModelVariant::QWEN35) {
                    ggml_tensor* gate_exps =
                        GetLayerTensorAny(&model->layers[il], {"ffn_gate_exps.weight", "ffn_gate_exps"});
            ...
            if (ShouldRunMoESharedDenseBranch(model, layer_spec, is_gemma4_moe, ffn_gate, ffn_up, ffn_down)) {
                ...
                struct ggml_tensor* shared_gate_up_fused =
                    ((is_gemma4_moe || lfm2_shortconv_moe || qwen_prefill_fused_shared_expert) &&
                     !prefer_plain_shared_expert_matmul && shared_input->type == GGML_TYPE_F32)
                        ...
                        : nullptr;
                if (shared_gate_up_fused && (shared_gate_up_fused->ne[0] != shared_input->ne[0] ||
                ...
                if (shared_gate_up_fused) {
                    ...
                    shared_up = prefer_plain_shared_expert_matmul ? ggml_mul_mat(ctx_c, ffn_up, shared_input)
            ...
            const bool prefer_separate_qwen35_hybrid_ffn_gate_up = densecore::runtime::IsQwen35Point8B(model);
            ...
            if (dense_gate_up_fused && (dense_gate_up_fused->ne[0] != cur->ne[0] ||
            ...
            } else if (model->arch_flags.is_gemma4 && GetCurrentExecutionPhase() == InferenceExecutionPhase::Decode) {
    ...
    if (ShouldRunHiddenSnapshotProbe(static_cast<int>(model->layers.size()), "pre_output_norm_hidden")) {
        ...
        if (hidden_ud) {
            ...
            hidden_ud->token_idx = densecore::env::ParseDiagnosticEnvInt("DENSECORE_DEBUG_HIDDEN_SNAPSHOT_TOKEN", -1);
            hidden_ud->token_ids = batch.tokens.data();
            hidden_ud->token_seq_ids = batch.seq_id.data();
    ...
    const bool per_sequence_prefill_logits =
        ShouldUsePrefillPerSequenceLastLogits(model, batch, static_cast<int>(cur->ne[1]));
    if (per_sequence_prefill_logits) {
        struct ggml_tensor* gathered = nullptr;
        for (int seq = 0; seq < batch.num_seqs; ++seq) {
            auto it = std::find(batch.seq_id.rbegin(), batch.seq_id.rend(), seq);
            if (it == batch.seq_id.rend()) {
                throw densecore::InvalidArgumentException("missing sequence column for compact prefill logits");
            }
            const size_t token_index =
                batch.seq_id.size() - 1 - static_cast<size_t>(std::distance(batch.seq_id.rbegin(), it));
            const size_t offset = token_index * static_cast<size_t>(cur->nb[1]);
            struct ggml_tensor* column = ggml_view_2d(ctx_c, cur, cur->ne[0], 1, cur->nb[1], offset);
            gathered = gathered ? ggml_concat(ctx_c, gathered, column, 1) : column;
        }
        cur = ggml_cont(ctx_c, gathered);
    }
    ...
    if (!per_sequence_prefill_logits && cur->ne[1] > 1 && ShouldUsePrefillLastLogitsOnly(model, batch, N)) {
    ...
    const bool debug_lm_head =
        IsDebugInferenceStatsEnabled() || densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_LM_HEAD_TOP") != nullptr;
    if (debug_lm_head) {
        auto cb_check_lm_head_out = [](struct ggml_tensor* dst, const struct ggml_tensor* src, int ith, int nth,
            ...
            if (src && src->data) {
                ...
                const char* top_env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_LM_HEAD_TOP");
                int top_n = 0;
                if (top_env && *top_env) {
                    char* end = nullptr;
                ...
                if (top_n > 0 && n_tok > 0) {
                    ...
                    const char* watch_env = densecore::env::GetDiagnosticEnv("DENSECORE_DEBUG_LM_HEAD_WATCH_IDS");
