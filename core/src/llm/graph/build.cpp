...
static bool IsVerboseGraphBuildLoggingEnabled() {
    static const bool enabled = []() {
        const char* env = densecore::env::GetDiagnosticEnv("DENSECORE_VERBOSE_GRAPH_BUILD");
...
struct ggml_tensor* BuildTransformerGraph(TransformerModel* model, PagedKVCache* cache, struct ggml_context* ctx_c,
    ...
    if (model && model->arch == ModelArch::BERT) {
        ...
        (void)cache;
        return densecore::llm::models::bert::BuildEmbeddingGraph(model, ctx_c, batch, gf, out_embd, out_pos);
    }
