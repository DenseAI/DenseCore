...
get_filename_component(_dense_series_repo_root "${CMAKE_CURRENT_LIST_DIR}/../.." ABSOLUTE)
set(_dense_series_arm_profiles "${_dense_series_repo_root}/cmake/dense_series_arm_profiles.cmake")
if(EXISTS "${_dense_series_arm_profiles}")
    include("${_dense_series_arm_profiles}")
endif()

if(NOT COMMAND DenseSeriesInitArmTargetProfiles)
    # Standalone DenseCore builds still need ARM profile defaults even when the
    # shared Dense Series definitions are not present next to this repo.
...
function(DenseCoreInitArmTargetProfiles)
    DenseSeriesInitArmTargetProfiles()
endfunction()

function(DenseCoreResolveArmProfileMarch out_var)
    if(NOT out_var)
        message(FATAL_ERROR "DenseCoreResolveArmProfileMarch requires an output variable.")
    endif()

    DenseSeriesResolveArmProfile(_densecore_arm_profile_march _densecore_arm_profile_ggml _densecore_arm_is_jetson)
    set(${out_var} "${_densecore_arm_profile_march}" PARENT_SCOPE)
    set(DENSECORE_ARM_PROFILE_GGML_ARCH "${_densecore_arm_profile_ggml}" PARENT_SCOPE)
    set(DENSECORE_ARM_PROFILE_IS_JETSON "${_densecore_arm_is_jetson}" PARENT_SCOPE)
endfunction()
