...
TEST_F(BlockManagerTest, PrefixCachingProgressivelyExtendsFullPromptBlocks) {
    ...
    const int block_b = manager->AllocateSingle();
    ASSERT_GE(block_b, 0);
    const uint64_t hash_b = BlockManager::ComputeTokenHash(prompt_ab.data(), 2 * BLOCK_SIZE);
...
TEST_F(BlockManagerTest, PrefixCachingDoesNotReuseLocalBlockUnderDifferentLeftContext) {
    std::vector<int> left_a(BLOCK_SIZE);
    std::vector<int> left_b(BLOCK_SIZE);
    std::vector<int> shared_block(BLOCK_SIZE);
    for (int i = 0; i < BLOCK_SIZE; ++i) {
        left_a[i] = 1000 + i;
        left_b[i] = 2000 + i;
        shared_block[i] = 3000 + i;
    }

    std::vector<int> prefix_a = left_a;
    prefix_a.insert(prefix_a.end(), shared_block.begin(), shared_block.end());
    std::vector<int> prefix_b = left_b;
    prefix_b.insert(prefix_b.end(), shared_block.begin(), shared_block.end());
    prefix_b.push_back(4000);

    const int block_a = manager->AllocateSingle();
    const int block_b = manager->AllocateSingle();
    const int shared_under_a = manager->AllocateSingle();
    ASSERT_GE(block_a, 0);
    ASSERT_GE(block_b, 0);
    ASSERT_GE(shared_under_a, 0);
    manager->RegisterPrefixBlockWithTokens(block_a, BlockManager::ComputeTokenHash(left_a.data(), BLOCK_SIZE),
                                           left_a.data(), BLOCK_SIZE);
    manager->RegisterPrefixBlockWithTokens(shared_under_a,
                                           BlockManager::ComputeTokenHash(prefix_a.data(), 2 * BLOCK_SIZE),
                                           shared_block.data(), BLOCK_SIZE);
    manager->RegisterPrefixBlockWithTokens(block_b, BlockManager::ComputeTokenHash(left_b.data(), BLOCK_SIZE),
                                           left_b.data(), BLOCK_SIZE);

    auto wrong_context_hit = manager->FindLongestCachedPrefixWithVerification(
        prefix_b.data(), static_cast<int>(prefix_b.size()), /*require_hybrid_ssm_snapshot=*/false);
    EXPECT_EQ(wrong_context_hit.cached_tokens, BLOCK_SIZE);
    ASSERT_EQ(wrong_context_hit.cached_block_ids.size(), 1u);
    EXPECT_EQ(wrong_context_hit.cached_block_ids[0], block_b);

    manager->Free(wrong_context_hit.cached_block_ids);
    manager->FreeSingle(block_a);
    manager->FreeSingle(block_b);
    manager->FreeSingle(shared_under_a);
}
...
TEST_F(BlockManagerTest, HybridPrefixReuseTrimsToTerminalSnapshotBlock) {
    ...
    manager->RegisterPrefixBlockWithTokens(block_b, BlockManager::ComputeTokenHash(prompt_abcd.data(), 2 * BLOCK_SIZE),
                                           prompt_b.data(), BLOCK_SIZE, &terminal_snapshot);
    manager->RegisterPrefixBlockWithTokens(block_c, BlockManager::ComputeTokenHash(prompt_abcd.data(), 3 * BLOCK_SIZE),
...
TEST_F(BlockManagerTest, HybridPrefixReuseKeepsEarlierBlocksWithoutSnapshotsWhenTerminalHasSnapshot) {
    ...
    manager->RegisterPrefixBlockWithTokens(block_b,
                                           BlockManager::ComputeTokenHash(prompt_ab_next.data(), 2 * BLOCK_SIZE),
                                           prompt_b.data(), BLOCK_SIZE, &terminal_snapshot);
