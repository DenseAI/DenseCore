...
TEST_F(GraphExecutorIntegrationTest, RejectsNodeWithOutOfRangeTensorIndex) {
    std::vector<float> a(4, 1.0f);
    std::vector<float> c(4, 0.0f);

    OperationGraph graph;
    const size_t a_idx = graph.RegisterTensor(Tensor::Make2D(a.data(), 2, 2, DType::F32));
    const size_t c_idx = graph.RegisterTensor(Tensor::Make2D(c.data(), 2, 2, DType::F32));
    graph.AddNode(OpType::Add, {a_idx, 99}, {c_idx}, "invalid_input_index", {});

    GraphExecutor executor;
    EXPECT_THROW(executor.Execute(graph, DeviceType::CPU), InvalidArgumentException);
}

TEST_F(GraphExecutorIntegrationTest, RejectsReferencedTensorWithNonPositiveShape) {
    std::vector<float> a(4, 1.0f);
    std::vector<float> c(4, 0.0f);

    Tensor invalid = Tensor::Make2D(a.data(), 2, 0, DType::F32);
    Tensor output = Tensor::Make2D(c.data(), 2, 2, DType::F32);

    OperationGraph graph;
    const size_t invalid_idx = graph.RegisterTensor(invalid);
    const size_t output_idx = graph.RegisterTensor(output);
    graph.AddNode(OpType::Copy, {invalid_idx}, {output_idx}, "invalid_shape", {});
