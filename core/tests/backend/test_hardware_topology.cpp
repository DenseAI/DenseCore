...
TEST(HardwareTopology, SysfsFallbackExcludesSmtSiblings) {
    std::set<std::pair<int, int>> sysfs_physical_cores;
    const int logical_cores = densecore::simd::GetNumCores();
    for (int cpu = 0; cpu < logical_cores; ++cpu) {
        int package_id = -1;
        int core_id = -1;
        std::ifstream("/sys/devices/system/cpu/cpu" + std::to_string(cpu) +
                      "/topology/physical_package_id") >> package_id;
        std::ifstream("/sys/devices/system/cpu/cpu" + std::to_string(cpu) + "/topology/core_id") >> core_id;
        if (package_id < 0 || core_id < 0) {
            GTEST_SKIP() << "Linux CPU topology files are unavailable";
        }
        sysfs_physical_cores.emplace(package_id, core_id);
    }

    EXPECT_EQ(densecore::HardwareTopology::GetInstance().GetPhysicalCoreCount(),
              static_cast<int>(sysfs_physical_cores.size()));
}
#endif
