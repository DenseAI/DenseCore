...
TEST_F(BackendPluginLoaderTest, EngineRegistrySharesProcessPluginVisibilityButOwnsItsCpu) {
    auto engine = BackendRegistry::CreateForEngine();
    auto* owned_cpu = engine->Get(DeviceType::CPU);
    ASSERT_NE(owned_cpu, nullptr);
    EXPECT_FALSE(engine->IsRegistered(DeviceType::ASIC));
    ASSERT_EQ(DenseCoreLoadPlugin(DENSECORE_TEST_BACKEND_PLUGIN_PATH), DENSECORE_STATUS_OK);
    auto* plugin = BackendRegistry::Instance().Get(DeviceType::ASIC);
    ASSERT_NE(plugin, nullptr);
    EXPECT_EQ(engine->Get(DeviceType::ASIC), plugin);
    EXPECT_TRUE(engine->IsRegistered(DeviceType::ASIC));
    EXPECT_EQ(engine->SupportsProfile(DeviceType::ASIC, InferenceProfile::LLMCore),
              BackendRegistry::Instance().SupportsProfile(DeviceType::ASIC, InferenceProfile::LLMCore));
    EXPECT_EQ(engine->Get(DeviceType::CPU), owned_cpu);
    EXPECT_NE(owned_cpu, BackendRegistry::Instance().Get(DeviceType::CPU));
    ASSERT_TRUE(engine->SetDefault(DeviceType::ASIC));
    EXPECT_EQ(engine->GetDefault(), plugin);
    engine.reset();
    EXPECT_EQ(BackendRegistry::Instance().Get(DeviceType::ASIC), plugin);
    EXPECT_STREQ(plugin->Name(), "DenseCoreTestPlugin");
}
