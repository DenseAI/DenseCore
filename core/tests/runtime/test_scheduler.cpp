...
TEST(SchedulerArchitecture, PrefillConfigCapsChunkSize) {
    SchedulerConfig cfg = MakeTestConfig();
    cfg.enable_chunked_prefill = true;
...
TEST(SchedulerArchitecture, DecodeBatchUsesSingleContextBucket) {
    SchedulerConfig cfg = MakeTestConfig();
    cfg.decode_homogeneous_batch_n_past = true;
    BlockManager block_manager(/*num_blocks=*/512, BLOCK_SIZE);
    Scheduler scheduler(&block_manager, cfg);
...
TEST(SchedulerArchitecture, PreemptionDoesNotDrainAllRunningSeqsWhenFreesAreDeferred) {
    SchedulerConfig cfg = MakeTestConfig();
    cfg.max_num_seqs = 2;
    cfg.max_prefill_seqs = 2;
    cfg.max_num_batched_tokens = 64;
    cfg.watermark_high = 0.9f;
    cfg.watermark_low = 0.8f;

    BlockManager block_manager(/*num_blocks=*/2, BLOCK_SIZE);
    Scheduler scheduler(&block_manager, cfg);

    const int seq1 = scheduler.AddRequest(/*request_id=*/301, /*prompt_len=*/BLOCK_SIZE, /*max_output_len=*/32);
    const int seq2 = scheduler.AddRequest(/*request_id=*/302, /*prompt_len=*/BLOCK_SIZE, /*max_output_len=*/32);
    ASSERT_GT(seq1, 0);
    ASSERT_GT(seq2, 0);

    SchedulerOutput prefill = scheduler.Schedule();
    ASSERT_EQ(prefill.prefill_seq_ids.size(), 2u);
    ASSERT_EQ(prefill.prefill_chunk_info.size(), 2u);
    for (const auto& chunk : prefill.prefill_chunk_info) {
        scheduler.UpdateProgress(chunk.seq_id, chunk.chunk_tokens);
    }

    SchedulerOutput after_pressure = scheduler.Schedule();
    EXPECT_EQ(after_pressure.preempted_seq_ids.size(), 1u);
    EXPECT_EQ(after_pressure.decode_seq_ids.size(), 1u);

    const Scheduler::Stats stats = scheduler.GetStats();
    EXPECT_EQ(stats.running_count, 1);
    EXPECT_EQ(stats.swapped_count, 1);
}
...
namespace densecore {
namespace {
TEST(SchedulerArchitecture, InitialCohortFillRespectsCapacityAndPreservesExistingStreamFairness) {
    for (bool mixed : {false, true}) {
        SCOPED_TRACE(mixed);
        auto config = MakeTestConfig();
        config.max_num_seqs = 2;
        config.max_prefill_seqs = 1;
        config.max_consecutive_decode_batches = 8;
        config.fill_slots_before_first_decode = true;
        config.enable_mixed_prefill_decode = mixed;
        BlockManager blocks(512, BLOCK_SIZE);
        Scheduler scheduler(&blocks, config);
        const int a = scheduler.AddRequest(1001, 8, 32);
        const int b = scheduler.AddRequest(1002, 8, 32);
        const int c = scheduler.AddRequest(1003, 8, 32);
        ASSERT_GT(a, 0); ASSERT_GT(b, 0); ASSERT_GT(c, 0);
        auto first = scheduler.Schedule();
        ASSERT_EQ(first.prefill_seq_ids, std::vector<int>({a}));
        scheduler.UpdateProgress(a, 8);
        auto second = scheduler.Schedule();
        EXPECT_TRUE(second.decode_seq_ids.empty());
        ASSERT_EQ(second.prefill_seq_ids, std::vector<int>({b}));
        scheduler.UpdateProgress(b, 8);
        auto full = scheduler.Schedule();
        EXPECT_TRUE(full.prefill_seq_ids.empty());
        ASSERT_EQ(full.decode_seq_ids.size(), 2u);
        for (int seq : full.decode_seq_ids) scheduler.UpdateProgress(seq, 1);
        scheduler.RemoveRequest(a);
        auto existing = scheduler.Schedule();
        EXPECT_TRUE(existing.prefill_seq_ids.empty());
        EXPECT_EQ(existing.decode_seq_ids, std::vector<int>({b}));
        scheduler.UpdateProgress(b, 1);
        bool admitted = false;
        for (int step = 0; step < config.max_consecutive_decode_batches; ++step) {
            auto next = scheduler.Schedule();
            if (!next.prefill_seq_ids.empty()) {
                EXPECT_EQ(next.prefill_seq_ids, std::vector<int>({c}));
                admitted = true;
                break;
            }
            EXPECT_EQ(next.decode_seq_ids, std::vector<int>({b}));
            scheduler.UpdateProgress(b, 1);
        }
...
namespace densecore {
namespace {
TEST(SchedulerArchitecture, InitialCohortFillDoesNotDelayDecodeForChunkedPrompts) {
    for (bool per_request_limit : {false, true}) {
        auto config = MakeTestConfig();
        config.max_num_seqs = 2;
        config.max_prefill_seqs = 1;
        config.max_prefill_tokens = per_request_limit ? 32 : 8;
        config.max_consecutive_decode_batches = 8;
        config.fill_slots_before_first_decode = true;
        ...
        BlockManager blocks(512, BLOCK_SIZE);
        Scheduler scheduler(&blocks, config);
        const int a = scheduler.AddRequest(2001, 8, 32);
        ASSERT_GT(a, 0);
        ASSERT_GT(scheduler.AddRequest(2002, 24, 32, 100, nullptr, true, false,
                                      per_request_limit ? 8 : -1), 0);
        ASSERT_EQ(scheduler.Schedule().prefill_seq_ids, std::vector<int>({a}));
        scheduler.UpdateProgress(a, 8);
        const auto next = scheduler.Schedule();
        EXPECT_TRUE(next.prefill_seq_ids.empty());
        EXPECT_EQ(next.decode_seq_ids, std::vector<int>({a}));
    }
}
}  // namespace
}  // namespace densecore
