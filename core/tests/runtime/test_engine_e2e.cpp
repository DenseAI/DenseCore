...
TEST(RuntimeOptimizationStateTest, ReportsEligiblePrefixAndRecurrentSnapshotPolicies) {
    struct PolicyCase {
        const char* name;
        ...
        bool enable_qwen36_prefix;
        bool enable_qwen36_snapshot;
        int expected_prefix;
        int expected_snapshot;
    };
    const PolicyCase cases[] = {
        {"attention only", ModelVariant::LLAMA, false, false, false, false, false, false, 1, 0},
        {"Qwen35 unqualified", ModelVariant::QWEN35, true, false, false, false, true, true, 0, 0},
        {"Qwen38 unqualified", ModelVariant::QWEN38, true, false, false, false, true, true, 0, 0},
        {"Qwen36 default", ModelVariant::QWEN36, true, false, false, false, false, false, 0, 0},
        {"Qwen36 snapshot only", ModelVariant::QWEN36, true, false, false, false, false, true, 0, 0},
        {"Qwen36 prefix only", ModelVariant::QWEN36, true, false, false, false, true, false, 1, 0},
        {"Qwen36 opted in", ModelVariant::QWEN36, true, false, false, false, true, true, 1, 1},
        {"Qwen36 prefix disabled", ModelVariant::QWEN36, true, false, true, false, true, true, 0, 0},
        {"Qwen36 snapshot disabled", ModelVariant::QWEN36, true, false, false, true, true, true, 1, 0},
        {"LFM2 shortconv", ModelVariant::LFM2MOE, false, true, false, false, false, false, 1, 1},
        {"LFM2 prefix disabled", ModelVariant::LFM2MOE, false, true, true, false, false, false, 0, 0},
        {"LFM2 snapshot disabled", ModelVariant::LFM2MOE, false, true, false, true, false, false, 1, 0},
    };
    for (const auto& policy : cases) {
        SCOPED_TRACE(policy.name);
        EngineState state;
        auto entry = std::make_unique<ModelEntry>();
        entry->model = std::make_unique<TransformerModel>();
        entry->model->variant = policy.variant;
        entry->model->arch_flags.is_hybrid_ssm = policy.hybrid;
        entry->model->arch_flags.is_lfm2_shortconv = policy.shortconv;
        state.default_model_id = "policy-test";
        state.models.emplace(state.default_model_id, std::move(entry));
        auto& config = state.fast_path_config.worker;
        config.prefix_cache_reuse_disabled = policy.disable_prefix;
        config.hybrid_ssm_snapshot_restore_disabled = policy.disable_snapshot;
        config.qwen36_prefix_cache_reuse_enabled = policy.enable_qwen36_prefix;
        config.qwen36_hybrid_ssm_snapshot_restore_enabled = policy.enable_qwen36_snapshot;

        DenseCoreRuntimeOptimizationState output{};
        ASSERT_EQ(DenseCoreGetRuntimeOptimizationState(&state, &output), DENSECORE_STATUS_OK);
        EXPECT_EQ(output.struct_size, sizeof(output));
        EXPECT_EQ(output.token_id_submit_supported, 1);
        EXPECT_EQ(output.prefix_cache_reuse_enabled, policy.expected_prefix);
        EXPECT_EQ(output.hybrid_ssm_snapshot_restore_enabled, policy.expected_snapshot);
    }
}
...
TEST_F(EngineE2ETest, AdditionalModelPreservesEngineKVPolicy) {
    engine_ = InitEngineWithKVType("mock", nullptr, 2, -1, 0, DENSECORE_KV_INT8);
    ASSERT_NE(engine_, nullptr) << DenseCoreGetLastError();

    auto* state = static_cast<EngineState*>(engine_);
    ASSERT_EQ(state->requested_kv_cache_type, GGML_TYPE_Q8_0);
    ASSERT_EQ(LoadModel(engine_, "secondary", "mock", 0), DENSECORE_STATUS_OK) << DenseCoreGetLastError();

    ModelEntry* secondary = state->GetModel("secondary");
    ASSERT_NE(secondary, nullptr);
    ASSERT_NE(secondary->kv_cache, nullptr);
    EXPECT_EQ(secondary->kv_cache->cache_type, GGML_TYPE_Q8_0);
}
...
TEST_F(EngineE2ETest, QuantizeModel_InvalidArgsSetLastError) {
    const int rc = QuantizeModel(nullptr, "out.gguf", "{\"format\":\"q4_k_m\"}");
    EXPECT_EQ(rc, -1);
    const char* err = DenseCoreGetLastError();
    ASSERT_NE(err, nullptr);
    EXPECT_NE(std::string(err).find("required"), std::string::npos);
}

TEST_F(EngineE2ETest, QuantizeModel_MissingInputSetsLastError) {
    const int rc = QuantizeModel("missing-model.gguf", "out.gguf", "{\"format\":\"q4_k_m\"}");
    EXPECT_EQ(rc, -2);
    const char* err = DenseCoreGetLastError();
    ASSERT_NE(err, nullptr);
    EXPECT_NE(std::string(err).find("input model not found"), std::string::npos);
}

TEST_F(EngineE2ETest, QuantizeModel_UnsupportedFormatSetsLastError) {
    int rc = QuantizeModel("missing-model.gguf", "out.gguf", "{\"format\":\"totally_invalid\"}");
    EXPECT_EQ(rc, -2);
    const char* err = DenseCoreGetLastError();
    ASSERT_NE(err, nullptr);
    EXPECT_NE(std::string(err).find("input model not found"), std::string::npos);

    const std::string existing_file = "quantize_invalid_input.gguf";
    {
        std::ofstream out(existing_file, std::ios::binary);
        ASSERT_TRUE(out.good());
        out << "not-a-real-gguf";
    }
    rc = QuantizeModel(existing_file.c_str(), "out.gguf", "{\"format\":\"totally_invalid\"}");
    EXPECT_EQ(rc, -3);
    err = DenseCoreGetLastError();
    ASSERT_NE(err, nullptr);
    EXPECT_NE(std::string(err).find("unsupported quantization format"), std::string::npos);
