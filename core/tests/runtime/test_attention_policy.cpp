...
extern bool ShouldUsePrefillPerSequenceLastLogitsForTest(const TransformerModel* model, int num_seqs, int n_tokens);
extern int ResolveSsmConvTaskCountForTest(int variant, int num_seqs, int n_tokens, int conv_channels,
                                          int num_threads, bool force_serial_multi_seq);
extern int ResolveSsmDeltaTaskCountForTest(int variant, int num_seqs, int num_v_heads, int num_threads,
...
extern bool RunDenseCoreQ8PackedTileForTest(int cols, int rows, int pattern);
...
TEST(AttentionPolicyTest, Gemma4NativeQ8PrefillDispatchMatchesVecDotOracle) {
    bool matches = false;
    ...
    // The fixture has N=2048; ARM's profitable true-GEMM threshold is 4096.
    EXPECT_EQ(true_gemm_ops, 0u);
    EXPECT_EQ(gemv_ops, 4u);
#else
...
TEST(AttentionPolicyTest, Q8PackedTileMatchesScalarOracleAndPreservesOutputPadding) {
    for (int cols : {32, 96, 2048}) {
        for (int rows : {4, 12, 128}) {
            for (int pattern = 0; pattern < 4; ++pattern) {
                ASSERT_TRUE(densecore::testing::RunDenseCoreQ8PackedTileForTest(cols, rows, pattern))
                    << "cols=" << cols << " rows=" << rows << " pattern=" << pattern;
            }
        }
    }
}
...
TEST(AttentionPolicyTest, Gemma4Q8PrefillCpuRepackAliasUsesRawCustomBatchedPath) {
    ...
    gemma4.prepared_weights.cpu_repack_aliases[original] = alias;
...
TEST(AttentionPolicyTest, Gemma4Q8DecodeCpuRepackAliasUsesRawCustomGemvPath) {
    ...
    gemma4.prepared_weights.cpu_decode_repack_aliases[original] = alias;
    gemma4.prepared_weights.cpu_repack_aliases[original] = alias;
...
TEST(AttentionPolicyTest, Qwen35MoEPrefillCpuRepackAliasUsesRawQ4KBatchedPath) {
    ...
    qwen35.prepared_weights.cpu_repack_aliases[original] = alias;
    qwen35.prepared_weights.cpu_repack_alias_sources[alias] = original;
...
TEST(AttentionPolicyTest, Qwen35TwoBDecodeCpuRepackAliasUsesRawQ4KPath) {
    ggml_init_params params{};
    params.mem_size = 32 * 1024 * 1024;
    params.no_alloc = false;
    ggml_context* ctx = ggml_init(params);
    ...
    ggml_tensor* original = ggml_new_tensor_2d(ctx, GGML_TYPE_Q4_K, /*ne0=*/2048, /*ne1=*/2048);
    ASSERT_NE(original, nullptr);
    ggml_set_name(original, "blk.0.attn_gate.weight");
    ggml_tensor* alias = ggml_new_tensor_2d(ctx, GGML_TYPE_Q4_K, /*ne0=*/2048, /*ne1=*/2048);
    ASSERT_NE(alias, nullptr);
    ggml_set_name(alias, "blk.0.attn_gate.weight.fast_matmul");
    qwen35.prepared_weights.cpu_repack_aliases[original] = alias;
    qwen35.prepared_weights.cpu_repack_alias_sources[alias] = original;

    ggml_tensor* input = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, /*ne0=*/2048, /*ne1=*/1);
    ASSERT_NE(input, nullptr);
...
TEST(AttentionPolicyTest, Qwen35ParallelPrefillCompactsOneLogitsColumnPerSequence) {
    TransformerModel qwen35 = MakeModel(ModelArch::QWEN35);
    qwen35.variant = ModelVariant::QWEN35;
    qwen35.arch_flags.is_hybrid_ssm = true;

    EXPECT_TRUE(densecore::testing::ShouldUsePrefillPerSequenceLastLogitsForTest(
        &qwen35, /*num_seqs=*/4, /*n_tokens=*/768));
    EXPECT_FALSE(densecore::testing::ShouldUsePrefillPerSequenceLastLogitsForTest(
        &qwen35, /*num_seqs=*/4, /*n_tokens=*/4));
}

TEST(AttentionPolicyTest, HybridSsmDeltaScanStaysHeadParallelForConcurrentDecode) {
    const int qwen36 = static_cast<int>(ModelVariant::QWEN36);
    const int qwen35 = static_cast<int>(ModelVariant::QWEN35);
    const bool kSplitMultiSeq = false;

    // Single-sequence decode keeps the granularity it was qualified with.
    EXPECT_EQ(densecore::testing::ResolveSsmDeltaTaskCountForTest(qwen36, /*num_seqs=*/1, /*num_v_heads=*/32,
                                                                 /*num_threads=*/16, kSplitMultiSeq),
              8);
    // A concurrent decode batch must not fall back to a single task: it carries
    // one token per sequence through every head, so it needs at least as many
    // tasks as the single-sequence case.
    EXPECT_EQ(densecore::testing::ResolveSsmDeltaTaskCountForTest(qwen36, /*num_seqs=*/2, /*num_v_heads=*/32,
                                                                 /*num_threads=*/16, kSplitMultiSeq),
              16);
    EXPECT_EQ(densecore::testing::ResolveSsmDeltaTaskCountForTest(qwen35, /*num_seqs=*/4, /*num_v_heads=*/32,
                                                                 /*num_threads=*/16, kSplitMultiSeq),
              16);
    // The thread budget still bounds the split.
    EXPECT_EQ(densecore::testing::ResolveSsmDeltaTaskCountForTest(qwen36, /*num_seqs=*/4, /*num_v_heads=*/32,
                                                                 /*num_threads=*/4, kSplitMultiSeq),
              4);
    // Head-poor and non-hybrid models degrade to a single task instead of
    // requesting more tasks than there is work for.
    EXPECT_EQ(densecore::testing::ResolveSsmDeltaTaskCountForTest(qwen36, /*num_seqs=*/4, /*num_v_heads=*/1,
                                                                 /*num_threads=*/16, kSplitMultiSeq),
              1);
    EXPECT_EQ(densecore::testing::ResolveSsmDeltaTaskCountForTest(static_cast<int>(ModelVariant::LFM2MOE),
                                                                 /*num_seqs=*/4, /*num_v_heads=*/32,
                                                                 /*num_threads=*/16, kSplitMultiSeq),
              1);
    // The A/B switch restores the previous serial multi-sequence behaviour and
    // leaves single-sequence decode untouched.
    EXPECT_EQ(densecore::testing::ResolveSsmDeltaTaskCountForTest(qwen36, /*num_seqs=*/4, /*num_v_heads=*/32,
                                                                 /*num_threads=*/16,
                                                                 /*force_serial_multi_seq=*/true),
              1);
    EXPECT_EQ(densecore::testing::ResolveSsmDeltaTaskCountForTest(qwen36, /*num_seqs=*/1, /*num_v_heads=*/32,
                                                                 /*num_threads=*/16,
                                                                 /*force_serial_multi_seq=*/true),
...
TEST(AttentionPolicyTest, HybridSsmConvSplitsChannelsWheneverTheBatchCarriesTokens) {
    const int qwen36 = static_cast<int>(ModelVariant::QWEN36);
    const bool kSplitMultiSeq = false;

    // Single-token decode keeps the serial conv path.
    EXPECT_EQ(densecore::testing::ResolveSsmConvTaskCountForTest(qwen36, /*num_seqs=*/1, /*n_tokens=*/1,
                                                                /*conv_channels=*/8192, /*num_threads=*/16,
                                                                kSplitMultiSeq),
              1);
    // Multi-token steps (concurrent decode and prefill alike) split by channel.
    EXPECT_EQ(densecore::testing::ResolveSsmConvTaskCountForTest(qwen36, /*num_seqs=*/4, /*n_tokens=*/4,
                                                                /*conv_channels=*/8192, /*num_threads=*/16,
                                                                kSplitMultiSeq),
              16);
    EXPECT_EQ(densecore::testing::ResolveSsmConvTaskCountForTest(qwen36, /*num_seqs=*/1, /*n_tokens=*/36,
                                                                /*conv_channels=*/8192, /*num_threads=*/16,
                                                                kSplitMultiSeq),
              16);
    EXPECT_EQ(densecore::testing::ResolveSsmConvTaskCountForTest(qwen36, /*num_seqs=*/4, /*n_tokens=*/4,
                                                                /*conv_channels=*/8, /*num_threads=*/16,
                                                                kSplitMultiSeq),
              8);
    EXPECT_EQ(densecore::testing::ResolveSsmConvTaskCountForTest(static_cast<int>(ModelVariant::LFM2MOE),
                                                                /*num_seqs=*/4, /*n_tokens=*/4,
                                                                /*conv_channels=*/8192, /*num_threads=*/16,
                                                                kSplitMultiSeq),
              1);
    // The A/B switch only serializes the multi-sequence case; single-sequence
    // prefill keeps its channel split.
    EXPECT_EQ(densecore::testing::ResolveSsmConvTaskCountForTest(qwen36, /*num_seqs=*/4, /*n_tokens=*/4,
                                                                /*conv_channels=*/8192, /*num_threads=*/16,
                                                                /*force_serial_multi_seq=*/true),
              1);
...
TEST(AttentionPolicyTest, Qwen36CompactParallelPrefillLmHeadUsesOwnedQ6KPath) {
    ggml_init_params params{};
    ...
    ggml_tensor* output = ggml_new_tensor_2d(ctx, GGML_TYPE_Q6_K, /*ne0=*/256, /*ne1=*/512);
    ASSERT_NE(output, nullptr);
    ggml_set_name(output, "output.weight");
    qwen36.output = output;
    ggml_tensor* input = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, /*ne0=*/256, /*ne1=*/4);
    ASSERT_NE(input, nullptr);

    ggml_tensor* result = densecore::testing::SmartMulMatWithPhaseTest(
        ctx, output, input, &qwen36, static_cast<int>(InferenceExecutionPhase::Prefill));
    ASSERT_NE(result, nullptr);
    EXPECT_EQ(result->op, GGML_OP_CUSTOM);
...
#if defined(__aarch64__) || defined(_M_ARM64)
    constexpr bool expected_safe_batched = false;
#else
    constexpr bool expected_safe_batched = true;
#endif
    EXPECT_EQ(expected_safe_batched, densecore::testing::ShouldUseGemma4PrefillSafeBatchedForTest(
...
TEST(AttentionPolicyTest, Q4KTrueBatchedAutoPolicy) {
    ...
    constexpr bool compiled_with_x86_kernel = true;
#else
    constexpr bool compiled_with_x86_kernel = false;
#endif
    EXPECT_EQ(compiled_with_x86_kernel, densecore::testing::ResolveQ4KTrueBatchedKernelPolicyForTest(
    ...
    EXPECT_TRUE(densecore::testing::ResolveQ4KTrueBatchedKernelPolicyForTest(
        static_cast<int>(SimdLevel::SVE), /*compiled_with_sve=*/true));
    EXPECT_FALSE(densecore::testing::ResolveQ4KTrueBatchedKernelPolicyForTest(
        static_cast<int>(SimdLevel::SVE2), /*compiled_with_sve=*/false));
    EXPECT_FALSE(densecore::testing::ResolveQ4KTrueBatchedKernelPolicyForTest(
...
TEST(PreparedWeights, ClearingPrefillAliasesPreservesCanonicalAndDecodeViews) {
    TransformerModel model;
    ggml_init_params params{4096, nullptr, true};
    model.ctx_w = ggml_init(params);
    model.prepared_weights.ctx_qwen36_ssm_q8_prefill_amx = ggml_init(params);
    ASSERT_NE(model.ctx_w, nullptr);
    ASSERT_NE(model.prepared_weights.ctx_qwen36_ssm_q8_prefill_amx, nullptr);
    auto* canonical = ggml_new_tensor_1d(model.ctx_w, GGML_TYPE_F32, 4);
    auto* decode = ggml_new_tensor_1d(model.ctx_w, GGML_TYPE_F32, 4);
    auto* prefill = ggml_new_tensor_1d(model.prepared_weights.ctx_qwen36_ssm_q8_prefill_amx, GGML_TYPE_F32, 4);
    model.prepared_weights.cpu_decode_repack_aliases[canonical] = decode;
    model.prepared_weights.cpu_repack_aliases[canonical] = decode;
    model.prepared_weights.qwen36_ssm_q8_prefill_amx_aliases[canonical] = prefill;
    model.prepared_weights.cpu_amx_aliases[prefill] = true;

    ClearQwen36SSMQ8PrefillAMXAliases(&model);
    EXPECT_EQ(model.prepared_weights.ctx_qwen36_ssm_q8_prefill_amx, nullptr);
    EXPECT_TRUE(model.prepared_weights.qwen36_ssm_q8_prefill_amx_aliases.empty());
    EXPECT_TRUE(model.prepared_weights.cpu_amx_aliases.empty());
    EXPECT_EQ(model.prepared_weights.cpu_decode_repack_aliases.at(canonical), decode);
    EXPECT_EQ(model.prepared_weights.cpu_repack_aliases.at(canonical), decode);
    EXPECT_EQ(canonical->type, GGML_TYPE_F32);
    ClearQwen36SSMQ8PrefillAMXAliases(&model);
    EXPECT_EQ(model.prepared_weights.cpu_repack_aliases.at(canonical), decode);
}

TEST(PreparedWeights, MoveTransfersContextsBuffersAndAliasMapsExactlyOnce) {
    using densecore::llm::weights::PreparedWeights;
    PreparedWeights source;
    ggml_init_params params{4096, nullptr, true};
    source.ctx_cpu_repack = ggml_init(params);
    ASSERT_NE(source.ctx_cpu_repack, nullptr);
    auto* tensor = ggml_new_tensor_1d(source.ctx_cpu_repack, GGML_TYPE_F32, 4);
    source.cpu_repack_aliases[tensor] = tensor;
    source.cpu_decode_repack_aliases[tensor] = tensor;
    source.cpu_repack_alias_layouts[tensor] = PreparedWeights::CpuRepackAliasLayout::Q4K8x8Q8K;
    auto* buffer = ggml_backend_buft_alloc_buffer(ggml_backend_cpu_buffer_type(), 64);
    ASSERT_NE(buffer, nullptr);
    source.cpu_repack_buffers.push_back(buffer);
    void* host = densecore::NumaAllocator::AllocateAlignedOnNode(64, 64, -1);
    ASSERT_NE(host, nullptr);
    source.host_buffers.push_back({host, 64, densecore::AllocationType::Aligned});
    auto* context = source.ctx_cpu_repack;

    PreparedWeights moved(std::move(source));
    EXPECT_EQ(source.ctx_cpu_repack, nullptr);
    EXPECT_TRUE(source.cpu_repack_buffers.empty());
    EXPECT_TRUE(source.cpu_repack_aliases.empty());
    EXPECT_TRUE(source.host_buffers.empty());
    EXPECT_EQ(moved.host_buffers.front().ptr, host);
    EXPECT_EQ(moved.ctx_cpu_repack, context);
    EXPECT_EQ(moved.cpu_repack_buffers.front(), buffer);
    EXPECT_EQ(moved.cpu_decode_repack_aliases.at(tensor), tensor);

    PreparedWeights destination;
    destination.ctx_cpu_repack = ggml_init(params);
    ASSERT_NE(destination.ctx_cpu_repack, nullptr);
    destination = std::move(moved);
    EXPECT_EQ(moved.ctx_cpu_repack, nullptr);
    EXPECT_TRUE(moved.cpu_repack_buffers.empty());
    EXPECT_TRUE(moved.host_buffers.empty());
    EXPECT_EQ(destination.host_buffers.front().ptr, host);
    EXPECT_EQ(destination.ctx_cpu_repack, context);
    EXPECT_EQ(destination.cpu_repack_aliases.at(tensor), tensor);
    EXPECT_EQ(destination.cpu_repack_alias_layouts.at(tensor), PreparedWeights::CpuRepackAliasLayout::Q4K8x8Q8K);
    destination.Reset();
    EXPECT_EQ(destination.ctx_cpu_repack, nullptr);
    EXPECT_TRUE(destination.cpu_repack_buffers.empty());
    EXPECT_TRUE(destination.cpu_repack_aliases.empty());
    destination.Reset();
}

TEST(PreparedWeights, DecodeRetiresScopedViewsWhileEmbeddingPreservesThem) {
    TransformerModel model;
    ggml_init_params params{4096, nullptr, true};
    model.prepared_weights.ctx_qwen36_ssm_q8_prefill_amx = ggml_init(params);
    ASSERT_NE(model.prepared_weights.ctx_qwen36_ssm_q8_prefill_amx, nullptr);
    const auto* context = model.prepared_weights.ctx_qwen36_ssm_q8_prefill_amx;
    densecore::llm::config::FastPathRuntimeConfig config;
    using densecore::llm::weights::WeightExecutionPhase;
    const auto embedding = model.prepared_weights.PrepareForExecution(model, WeightExecutionPhase::Embedding, 1, config);
    EXPECT_FALSE(embedding.prefill_aliases_prepared);
    EXPECT_EQ(model.prepared_weights.ctx_qwen36_ssm_q8_prefill_amx, context);
    const auto decode = model.prepared_weights.PrepareForExecution(model, WeightExecutionPhase::Decode, 1, config);
    EXPECT_FALSE(decode.prefill_aliases_prepared);
    EXPECT_EQ(model.prepared_weights.ctx_qwen36_ssm_q8_prefill_amx, nullptr);
}

TEST(PreparedWeights, PrefillReusesPreparedBuffersAndDecodeLeavesCanonicalWeights) {
    TransformerModel model;
    ...
    ASSERT_NE(model.prepared_weights.ctx_qwen36_ssm_q8_prefill_amx, nullptr);
    auto* canonical = ggml_new_tensor_1d(model.ctx_w, GGML_TYPE_F32, 4);
    auto* alias = ggml_new_tensor_1d(model.prepared_weights.ctx_qwen36_ssm_q8_prefill_amx, GGML_TYPE_F32, 4);
    auto* buffer = ggml_backend_buft_alloc_buffer(ggml_backend_cpu_buffer_type(), 64);
    ASSERT_NE(buffer, nullptr);
    model.prepared_weights.qwen36_ssm_q8_prefill_amx_aliases[canonical] = alias;
    ...
    densecore::llm::config::FastPathRuntimeConfig config;
    config.qwen36_ssm_q8_prefill_amx = densecore::llm::config::Qwen36SSMQ8PrefillAMXMode::On;
    config.qwen36_ssm_q8_prefill_amx_min_tokens = 8;
    using densecore::llm::weights::WeightExecutionPhase;
    const auto short_prompt = model.prepared_weights.PrepareForExecution(model, WeightExecutionPhase::Prefill, 7, config);
    EXPECT_FALSE(short_prompt.prefill_aliases_prepared);
    EXPECT_EQ(model.prepared_weights.qwen36_ssm_q8_prefill_amx_buffers.front(), buffer);
    const auto prefill = model.prepared_weights.PrepareForExecution(model, WeightExecutionPhase::Prefill, 8, config);
#if defined(__aarch64__) || defined(_M_ARM64)
    EXPECT_FALSE(prefill.prefill_aliases_prepared);
#else
    EXPECT_TRUE(prefill.prefill_aliases_prepared);
#endif
    ASSERT_EQ(model.prepared_weights.qwen36_ssm_q8_prefill_amx_buffers.size(), 1u);
    EXPECT_EQ(model.prepared_weights.qwen36_ssm_q8_prefill_amx_aliases.at(canonical), alias);
    model.prepared_weights.PrepareForExecution(model, WeightExecutionPhase::Decode, 1, config);
    EXPECT_TRUE(model.prepared_weights.qwen36_ssm_q8_prefill_amx_buffers.empty());
    EXPECT_TRUE(model.prepared_weights.qwen36_ssm_q8_prefill_amx_aliases.empty());
    EXPECT_EQ(canonical->type, GGML_TYPE_F32);
    EXPECT_EQ(canonical->ne[0], 4);
}
