...
TEST(PagedAttentionThreadPolicy, NodeTaskCountLimitsGraphWideCallbackThreads) {
    const PagedAttentionCallbackThreadSelection leader = ResolvePagedAttentionCallbackThreads(0, 96, 16);
    EXPECT_TRUE(leader.participates);
    EXPECT_EQ(leader.threads, 16);

    EXPECT_TRUE(ResolvePagedAttentionCallbackThreads(15, 96, 16).participates);
    EXPECT_FALSE(ResolvePagedAttentionCallbackThreads(16, 96, 16).participates);
    EXPECT_FALSE(ResolvePagedAttentionCallbackThreads(95, 96, 16).participates);
}
