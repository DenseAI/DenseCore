...
TEST(Int4Qwen36KernelTest, ArmSplitNBackendGateUpLikeShapesMatchReference) {
    ...
    for (int M : {1, 2, 4}) {
        ...
        std::vector<float> scales;
        std::vector<float> zeros;
        FillRandomInt4Problem(M, K, N, group_size, static_cast<uint32_t>(6000 + M), &input, &weights, &scales, &zeros);
...
TEST(Int4Qwen36KernelTest, ArmSplitNBackendDownLikeShapesMatchReference) {
    ...
    for (int M : {1, 2, 4}) {
        ...
        std::vector<float> scales;
        std::vector<float> zeros;
        FillRandomInt4Problem(M, K, N, group_size, static_cast<uint32_t>(7000 + M), &input, &weights, &scales, &zeros);
...
TEST(Int4GraphCallbackTest, DecodePrefillAndStridedInputMatchReference) {
    struct CustomPrefix {
    ...
    for (int tokens : {1, 3}) {
        for (bool strided_input : {false, true}) {
            for (bool strided_output : {false, true}) {
                SCOPED_TRACE(::testing::Message() << "tokens=" << tokens << " strided_input=" << strided_input
                                                  << " strided_output=" << strided_output);
                const std::unique_ptr<ggml_context, decltype(&ggml_free)> arena(
                    ggml_init({1024 * 1024, nullptr, false}), ggml_free);
                ASSERT_NE(arena, nullptr);
                constexpr int k = 32, n = 9;
                constexpr float guard = 12345.0f;
                const int input_element_stride = strided_input ? 2 : 1;
                auto* input_storage = ggml_new_tensor_2d(arena.get(), GGML_TYPE_F32, k * input_element_stride, tokens);
                auto* input = ggml_view_2d(arena.get(), input_storage, k, tokens, input_storage->nb[1], 0);
                input->nb[0] = sizeof(float) * input_element_stride;
                auto* weights = ggml_new_tensor_2d(arena.get(), GGML_TYPE_I8, k / 2, n);
                auto* scales = ggml_new_tensor_2d(arena.get(), GGML_TYPE_F32, 1, n);
                auto* zeros = ggml_new_tensor_2d(arena.get(), GGML_TYPE_F32, 1, n);
                for (int row = 0; row < n; ++row) {
                    static_cast<float*>(scales->data)[row] = 0.125f;
                    static_cast<float*>(zeros->data)[row] = 1.0f;
                    for (int col = 0; col < k / 2; ++col) {
                        const int low = (row + col * 2) % 8 - 4;
                        const int high = (row + col * 2 + 1) % 8 - 4;
                        static_cast<uint8_t*>(weights->data)[row * (k / 2) + col] =
                            static_cast<uint8_t>((low & 15) | ((high & 15) << 4));
                    }
                }
                for (int row = 0; row < tokens; ++row) {
                    for (int col = 0; col < k; ++col) {
                        auto* value = reinterpret_cast<float*>(static_cast<char*>(input->data) + row * input->nb[1] +
                                                               col * input->nb[0]);
                        *value = static_cast<float>((row + col) % 7 - 3) / 4.0f;
                    }
                }
                TransformerModel::Int4WeightBinding binding{weights, scales, zeros, k, k, n};
                auto* output = ggml_mul_mat_int4(arena.get(), weights, input, binding);
                ASSERT_NE(output, nullptr);
                ggml_tensor output_view = *output;
                ggml_tensor* output_storage = output;
                if (strided_output) {
                    // A prefix, inter-element gaps and a suffix protect every
                    // row while exercising the callback's scatter path.
                    output_storage = ggml_new_tensor_2d(arena.get(), GGML_TYPE_F32, n * 2 + 2, tokens);
                    output_view.data = static_cast<float*>(output_storage->data) + 1;
                    output_view.nb[0] = 2 * sizeof(float);
                    output_view.nb[1] = output_storage->nb[1];
                    output_view.nb[2] = output_storage->nb[2];
                    output_view.nb[3] = output_storage->nb[3];
                    output = &output_view;
                }
                ...
                for (int task_count : {2, 4}) {
                    SCOPED_TRACE(::testing::Message() << "task_count=" << task_count);
                    auto* storage_values = static_cast<float*>(output_storage->data);
                    const auto storage_elements = ggml_nelements(output_storage);
                    std::fill(storage_values, storage_values + storage_elements, guard);
                    // Invoke every GGML task partition explicitly, independent
                    // of host cores and the graph builder's thread selection.
                    for (int task = 0; task < task_count; ++task) {
                        callback.fun(output, task, task_count, callback.userdata);
                    }
                    for (int row = 0; row < tokens; ++row) {
                        for (int out = 0; out < n; ++out) {
                            ...
                            for (int col = 0; col < k; ++col) {
                                const float value = *reinterpret_cast<const float*>(
                                    static_cast<const char*>(input->data) + row * input->nb[1] + col * input->nb[0]);
                                const int quant = (out + col) % 8 - 4;
                                expected += value * 0.125f * static_cast<float>(quant - 1);
                            }
                            const float actual = *reinterpret_cast<const float*>(
                                static_cast<const char*>(output->data) + row * output->nb[1] + out * output->nb[0]);
                            EXPECT_NEAR(actual, expected, 1e-5f);
                        }
                        if (strided_output) {
                            const auto* storage_row = reinterpret_cast<const float*>(
                                static_cast<const char*>(output_storage->data) + row * output_storage->nb[1]);
                            for (int index = 0; index < n * 2 + 2; ++index) {
                                const bool written_element = index >= 1 && index < n * 2 && index % 2 == 1;
                                if (!written_element) EXPECT_EQ(storage_row[index], guard) << "guard index=" << index;
                            }
                        }
                    }
                }
