...
bool RunQwen36SmallDecodeGateUpGroupingForTest(int type, int tokens, int overlap, float input_scale, float* max_error);
...
TEST(MoETrace, StickyFallbackNarrowsCallbacksOnlyDuringDecode) {
    const int unknown = static_cast<int>(InferenceExecutionPhase::Unknown);
    const int prefill = static_cast<int>(InferenceExecutionPhase::Prefill);
    const int decode = static_cast<int>(InferenceExecutionPhase::Decode);

    EXPECT_FALSE(densecore::testing::ShouldNarrowNativeMoECallbackTasksForStickyFallbackForTest(
        true, prefill, false));
    EXPECT_TRUE(densecore::testing::ShouldNarrowNativeMoECallbackTasksForStickyFallbackForTest(
        true, decode, false));
    EXPECT_FALSE(densecore::testing::ShouldNarrowNativeMoECallbackTasksForStickyFallbackForTest(
        true, decode, true));
    EXPECT_FALSE(densecore::testing::ShouldNarrowNativeMoECallbackTasksForStickyFallbackForTest(
...
TEST(MoETrace, Q4KGateUpRepackedDecodePathRespectsArchitecture) {
    ...
    constexpr bool lfm2_repacked = true;
    constexpr bool qwen_repacked = false;
#else
    constexpr bool lfm2_repacked = false;
    constexpr bool qwen_repacked = true;
#endif
    EXPECT_EQ(lfm2_repacked, densecore::testing::ShouldUseQwenLikeGateUpQ4KRepackedSwiGLUForTest(
        /*qwen_native_moe=*/false, /*lfm2_native_moe=*/true, decode_phase, /*kernel_available=*/true));
    EXPECT_EQ(qwen_repacked, densecore::testing::ShouldUseQwenLikeGateUpQ4KRepackedSwiGLUForTest(
        /*qwen_native_moe=*/true, /*lfm2_native_moe=*/false, decode_phase, /*kernel_available=*/true));
    for (bool qwen : {false, true}) {
        EXPECT_FALSE(densecore::testing::ShouldUseQwenLikeGateUpQ4KRepackedSwiGLUForTest(
            qwen, !qwen, prefill_phase, /*kernel_available=*/true));
        EXPECT_FALSE(densecore::testing::ShouldUseQwenLikeGateUpQ4KRepackedSwiGLUForTest(
            qwen, !qwen, decode_phase, /*kernel_available=*/false));
    }
}
...
TEST(MoETrace, Qwen36SmallDecodeGateUpGroupingIsArmDecodeOnly) {
...
#endif
    for (auto variant : {ModelVariant::QWEN35, ModelVariant::QWEN36, ModelVariant::QWEN38, ModelVariant::LFM2MOE}) {
        const bool qwen_family = variant == ModelVariant::QWEN36;
        EXPECT_EQ(arm && qwen_family,
                  densecore::testing::Qwen36SmallDecodeGateUpGroupingPolicyForTest(
                      static_cast<int>(variant), static_cast<int>(InferenceExecutionPhase::Decode), true));
        EXPECT_FALSE(densecore::testing::Qwen36SmallDecodeGateUpGroupingPolicyForTest(
            static_cast<int>(variant), static_cast<int>(InferenceExecutionPhase::Prefill), true));
        EXPECT_FALSE(densecore::testing::Qwen36SmallDecodeGateUpGroupingPolicyForTest(
            static_cast<int>(variant), static_cast<int>(InferenceExecutionPhase::Decode), false));
    }
}

TEST(MoETrace, Qwen36SmallDecodeGateUpGroupingPreservesTokenAndTopKScatter) {
    for (auto type : {GGML_TYPE_Q4_K, GGML_TYPE_Q5_K}) {
        for (int tokens : {2, 3, 4}) {
            for (int overlap : {0, 1, 2}) {
                for (float scale : {0.05f, 1.0f, 4.0f}) {
                    SCOPED_TRACE(::testing::Message() << "type=" << type << " tokens=" << tokens
                                                     << " overlap=" << overlap << " scale=" << scale);
                    float max_error = 0.0f;
                    ASSERT_TRUE(densecore::testing::RunQwen36SmallDecodeGateUpGroupingForTest(
                        static_cast<int>(type), tokens, overlap, scale, &max_error));
                    EXPECT_LE(max_error, 1e-4f);
                }
            }
        }
    }
...
TEST(MoETrace, Qwen36PrefillDownKeepsRawRouteWithoutWorkerTLS) {
    bool exact = false;
    uint64_t repacked_bytes = 0;
    ASSERT_TRUE(densecore::testing::RunQwen36PrefillDownWithoutWorkerTLSForTest(&exact, &repacked_bytes));
    EXPECT_TRUE(exact);
    EXPECT_EQ(repacked_bytes, 0u);
}

TEST(MoETrace, GraphBoundCallbackCannotFallBackWithoutCpuDependencies) {
    std::unique_ptr<ggml_context, decltype(&ggml_free)> tensors(
        ggml_init({1 << 20, nullptr, false}), ggml_free);
    ASSERT_NE(tensors, nullptr);
    auto* input = ggml_new_tensor_2d(tensors.get(), GGML_TYPE_F32, 2, 1);
    auto* gates = ggml_new_tensor_2d(tensors.get(), GGML_TYPE_F32, 1, 1);
    auto* output = ggml_new_tensor_2d(tensors.get(), GGML_TYPE_F32, 2, 1);
    std::fill_n(static_cast<float*>(input->data), 2, 0.0f);
    std::fill_n(static_cast<float*>(gates->data), 1, 0.0f);
    std::fill_n(static_cast<float*>(output->data), 2, 123.0f);
    std::unique_ptr<InferenceWorkContext, decltype(&DestroyInferenceWorkContext)> context(
        CreateInferenceWorkContext(), DestroyInferenceWorkContext);
    BatchSpec unbound_batch{};
    BindInferenceWorkContextBatch(context.get(), &unbound_batch);
    TransformerLayer layer{};
    MoEUserData userdata{};
    userdata.layer = &layer;
    userdata.work_ctx = context.get();
    ResetMoECallbackEntryCounter();
    cb_moe_forward(output, input, gates, 0, 1, &userdata);
    EXPECT_EQ(userdata.backend, nullptr);
    EXPECT_EQ(GetMoECallbackMissingBackendCounter(), 1u);
    EXPECT_EQ(GetMoECallbackMissingExpertsCounter(), 0u);
    EXPECT_FLOAT_EQ(static_cast<const float*>(output->data)[0], 0.0f);
    EXPECT_FLOAT_EQ(static_cast<const float*>(output->data)[1], 0.0f);
    ResetMoEStrictFailure();
}
