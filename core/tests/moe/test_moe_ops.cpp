...
bool SignedNibbleDotParityForTest(bool q5, const void* weight, const void* input, int64_t cols);
}

TEST(SignedNibbleDot, Q4AndQ5MatchOriginalMixedIntegerPathBitwise) {
    ggml_cpu_init();
    constexpr int cols = 2048;
    std::vector<float> weights(cols), input(cols);
    std::vector<uint8_t> qinput(ggml_row_size(GGML_TYPE_Q8_K, cols));
    ...
    for (auto type : {GGML_TYPE_Q4_K, GGML_TYPE_Q5_K}) {
        std::vector<uint8_t> quantized(ggml_row_size(type, cols));
        for (float scale : {0.05f, 1.0f, 4.0f}) {
            for (int pattern = 0; pattern < 64; ++pattern) {
                SCOPED_TRACE(::testing::Message() << "type=" << type << " scale=" << scale << " pattern=" << pattern);
                for (int c = 0; c < cols; ++c) {
                    weights[c] = 0.05f * sample(uint32_t(pattern * cols + c) + 1u);
                    input[c] = scale * sample(uint32_t(pattern * cols + c) + 0x12345u);
                    if (pattern == 0) weights[c] = c % 2 ? 0.05f : -0.05f;
                    if (pattern == 1) input[c] = c % 2 ? scale : -scale;
                }
                ggml_quantize_chunk(type, weights.data(), quantized.data(), 0, 1, cols, nullptr);
                ASSERT_TRUE(hwy_kernels::QuantizeRowQ8K_Hwy(input.data(), qinput.data(), cols));
                ASSERT_TRUE(hwy_kernels::SignedNibbleDotParityForTest(
                    type == GGML_TYPE_Q5_K, quantized.data(), qinput.data(), cols));
            }
        }
    }
}
...
TEST_F(MoEOpsTest, Q6KRawBatchedFusedSwiGLUWritesEveryRow) {
    for (const int64_t M : {2, 4}) {
        constexpr int64_t K = 256;
        constexpr int64_t N = 19;
        ...
        std::vector<uint8_t> qup;
        std::vector<uint8_t> qinput;
        QuantizeRowsCpu(GGML_TYPE_Q6_K, gate_f32, N, K, &qgate);
        QuantizeRowsCpu(GGML_TYPE_Q6_K, up_f32, N, K, &qup);
        QuantizeRowsCpu(GGML_TYPE_Q8_K, input_f32, M, K, &qinput);

        std::vector<float> actual(static_cast<size_t>(M) * static_cast<size_t>(N), 12345.0f);
        CpuBackend& backend = GetCpuBackend();
        ASSERT_TRUE(RunMoEKQuantRawBatchedFusedSwiGLU(&backend, static_cast<int>(GGML_TYPE_Q6_K), qgate.data(), qup.data(),
                                                      qinput.data(), ggml_row_size(GGML_TYPE_Q8_K, K), actual.data(), M, N,
                                                      K, /*numa_node=*/0, /*allow_parallel=*/true));

        for (int64_t m = 0; m < M; ++m) {
            for (int64_t n = 0; n < N; ++n) {
                const float gate = DequantizedQuantDotReference(GGML_TYPE_Q6_K, qgate, qinput, n, m, K);
                const float up = DequantizedQuantDotReference(GGML_TYPE_Q6_K, qup, qinput, n, m, K);
                const float expected = MoETestSiLU(gate) * up;
                EXPECT_NEAR(actual[static_cast<size_t>(m) * static_cast<size_t>(N) + static_cast<size_t>(n)], expected,
                            7e-3f)
                    << "m=" << m << " n=" << n;
...
TEST_F(MoEOpsTest, Q4KPrepackedDecodeFusedSwiGLUMatchesRepackedProjection) {
    if (!kernels::Q4KRealPackedGemvKernelAvailable()) {
        GTEST_SKIP() << "Q4_K repacked fused SwiGLU projection is unavailable in this CPU profile";
    }
...
TEST_F(MoEOpsTest, Q6KRawBatchedProjectionMatchesStridedReferenceAcrossShapes) {
    CpuBackend& backend = GetCpuBackend();
    for (const int64_t K : {256, 768, 2048}) {
        constexpr int64_t N = 65;
        const auto weight = MakePatternedFloats(N, K, 0.071f);
        std::vector<uint8_t> qweight;
        QuantizeRowsCpu(GGML_TYPE_Q6_K, weight, N, K, &qweight);
        const auto* traits = ggml_get_type_traits_cpu(GGML_TYPE_Q6_K);
        for (const int64_t M : {1, 2, 7, 16, 17, 33}) {
            const auto input = MakePatternedFloats(M, K, 0.043f);
            std::vector<uint8_t> packed;
            QuantizeRowsCpu(GGML_TYPE_Q8_K, input, M, K, &packed);
            const size_t row_bytes = ggml_row_size(GGML_TYPE_Q8_K, K);
            const size_t stride = row_bytes + 64;
            std::vector<uint8_t> padded(static_cast<size_t>(M) * stride, 0xa5);
            for (int64_t m = 0; m < M; ++m) {
                std::memcpy(padded.data() + m * stride, packed.data() + m * row_bytes, row_bytes);
            }
            std::vector<float> expected(static_cast<size_t>(M * N));
            for (int64_t m = 0; m < M; ++m) {
                for (int64_t n = 0; n < N; ++n) {
                    traits->vec_dot(static_cast<int>(K), &expected[m * N + n], 0,
                                    qweight.data() + n * ggml_row_size(GGML_TYPE_Q6_K, K), 0,
                                    padded.data() + m * stride, 0, 1);
                }
            }
            ASSERT_GT(std::fabs(expected.front()), 0.01f) << "Q6 reference must have initialized FP16 tables";
            for (const bool parallel : {false, true}) {
                std::vector<float> actual(static_cast<size_t>(M * N), 0.0f);
                ASSERT_TRUE(RunMoEKQuantRawBatchedProjection(
                    &backend, GGML_TYPE_Q6_K, qweight.data(), padded.data(), stride, actual.data(), M, N, K,
                    0, parallel));
                for (size_t i = 0; i < actual.size(); ++i) {
                    ASSERT_NEAR(actual[i], expected[i], 1e-5f + 2e-6f * std::fabs(expected[i]))
                        << "M=" << M << " K=" << K << " index=" << i << " parallel=" << parallel;
                }
            }
        }
    }
...
TEST(Qwen36GateUpRowPair, ExactTilesOddTailsGenerationsAndDeniedGate) {
    ggml_cpu_init();
    using densecore::kernels::ParityGate;
    for (int cols : {512, 2048}) for (int rows : {17, 32}) for (int tokens : {2, 3, 4}) {
        SCOPED_TRACE(::testing::Message() << cols << ":" << rows << ":" << tokens);
        ParityGate::ResetForTest();
        const size_t ws = ggml_row_size(GGML_TYPE_Q4_K, cols);
        const size_t qs = (ggml_row_size(GGML_TYPE_Q8_K, cols) + 63) & ~size_t(63);
        std::vector<uint8_t> gate(ws * rows), up(ws * rows), input(qs * tokens);
        std::vector<float> values(cols), reference(rows * tokens), guarded(rows * tokens + 16, 12345.0f);
        const auto* wt = ggml_get_type_traits_cpu(GGML_TYPE_Q4_K);
        const auto* qt = ggml_get_type_traits_cpu(GGML_TYPE_Q8_K);
        for (int r = 0; r < rows; ++r) {
            for (int c = 0; c < cols; ++c) values[c] = 0.125f * std::sin((r * 17 + c) * 0.037f);
            wt->from_float(values.data(), gate.data() + r * ws, cols);
            for (int c = 0; c < cols; ++c) values[c] = 0.125f * std::cos((r * 29 + c) * 0.029f);
            wt->from_float(values.data(), up.data() + r * ws, cols);
        }
        const auto key = densecore::runtime::Qwen36GateUpRowPairKey(gate.data(), up.data(), cols, rows, tokens, qs);
        for (int generation = 0; generation < 4; ++generation) {
            for (int m = 0; m < tokens; ++m) {
                for (int c = 0; c < cols; ++c)
                    values[c] = (generation & 1 ? 4.0f : 1.0f) * std::sin((c + m * 47 + generation * 71) * 0.017f);
                qt->from_float(values.data(), input.data() + m * qs, cols);
                ASSERT_TRUE(densecore::hwy_kernels::FusedSwiGLUQ4KQ8KRows_Hwy(
                    gate.data(), up.data(), input.data() + m * qs, cols, rows, ws, reference.data() + m * rows));
            }
            if (generation == 2) {
                ParityGate::ResetForTest();
                EXPECT_EQ(ParityGate::Check(key), ParityGate::Verdict::kProbe);
            }
            if (generation == 3) ParityGate::Report(key, 1.0f, 1.0f, 0.0f);
            std::fill(guarded.begin() + 8, guarded.end() - 8, std::numeric_limits<float>::quiet_NaN());
            bool used = false;
            ASSERT_TRUE(densecore::runtime::Qwen36GateUpRowPair(gate.data(), up.data(), input.data(),
                cols, rows, tokens, ws, qs, guarded.data() + 8, &used));
            EXPECT_EQ(0, std::memcmp(reference.data(), guarded.data() + 8, reference.size() * sizeof(float)));
            EXPECT_EQ(used, generation < 2 && densecore::kernels::KQuantVecDotRowPairSupported());
            for (int i = 0; i < 8; ++i) {
                EXPECT_EQ(guarded[i], 12345.0f);
                EXPECT_EQ(guarded[guarded.size() - 1 - i], 12345.0f);
            }
        }
    }
    ParityGate::ResetForTest();
