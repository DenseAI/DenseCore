...
/** Authoritative terminal state, independent of visible token chunks. */
typedef enum DenseCoreGenerationFinishReason {
    DENSECORE_GENERATION_STOP = 1,
    DENSECORE_GENERATION_LENGTH = 2,
    DENSECORE_GENERATION_ERROR = 3
} DenseCoreGenerationFinishReason;

typedef void (*GenerationCompletionCallback)(int generated_tokens, DenseCoreGenerationFinishReason reason,
                                              void* user_data);

/**
 * Set an optional terminal metadata callback before submitting requests.
 * Invoked immediately before each terminal token callback, with the same user_data.
 * generated_tokens counts native samples, including hidden/control tokens.
 * Existing token callback signatures and behavior are unchanged. Pass NULL to disable.
 */
DENSECORE_API int DenseCoreSetGenerationCompletionCallback(DenseCoreHandle handle,
                                                          GenerationCompletionCallback callback);
 ...
 * @param size Dimension of the embedding vector on success, or a negative
 *        DenseCoreStatus terminal error code when embedding is NULL
 * @param user_data User-provided pointer passed back to the callback
 *
 * @note Thread-safety: callbacks are invoked from a background thread.
 * @note Ownership: The embedding pointer is only valid during the callback execution.
 *       Copy the data if you need to retain it.
 * @note Exactly one terminal callback is delivered for every accepted request.
 *       Success is reported as embedding != NULL and size > 0. Cancellation,
 *       shutdown, or execution failure is reported as embedding == NULL and
 *       size < 0; an empty vector is never used to represent failure.
 */
...
typedef enum {
    ...
    DENSECORE_KV_INT8 = 2,  ///< INT8 - approximately 50% storage vs FP16; validate quality per model
...
/**
 * Begin graceful engine shutdown without releasing the handle.
 *
 * Stops new admission and delivers terminal callbacks for accepted requests.
 * The handle remains valid only for a later FreeEngine() call.
 */
DENSECORE_API void BeginEngineShutdown(DenseCoreHandle handle);
 ...
 * @param threads Reserved for ABI compatibility. Compute threads are configured
