...
struct BlockManager {
    ...
    // Find the longest reusable full-block prefix for a prompt. Each block is
    // keyed by the cumulative prefix ending at that block, because its KV and
    // recurrent state are left-context dependent.
    ...
    PrefixCacheMatch
    FindLongestCachedPrefixWithVerification(const int* tokens, int n_tokens, bool require_hybrid_ssm_snapshot,
    ...
    // Register a block with local-block token storage for collision
    // verification. The supplied hash must identify the cumulative prefix
