...
public:
    ...
    // Independent mutable backend ownership for an engine. Instance() remains
    // the compatibility registry for standalone callers.
    static std::unique_ptr<BackendRegistry> CreateForEngine();
    ~BackendRegistry();
...
private:
    ...
    // C plugin/device APIs are process scoped. An engine borrows only missing
    // non-CPU devices from that registry; CPU configuration remains engine owned.
    BackendRegistry* non_cpu_fallback_ = nullptr;
