...
public:
    ...
    // Engine-owned execution resources. Existing external ops may keep their
    // original implementations; builtin CPU ops override these overloads.
    virtual void MatMul(const Tensor& A, const Tensor& B, Tensor* C, ThreadPool&) { MatMul(A, B, C); }
    virtual void MatMulTransB(const Tensor& A, const Tensor& B, Tensor* C, ThreadPool&) { MatMulTransB(A, B, C); }
};
