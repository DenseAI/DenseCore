...
public:
     ...
     */
    static std::vector<int> Tokenize(const TransformerModel* model, const std::string& text, bool add_bos = true,
                                     bool add_eos = false);
    ...
    /**
     * Detokenize one token for the structured server callback. Reasoning and
     * tool boundary tokens are preserved so the API layer can parse them;
     * unrelated control tokens remain hidden.
     */
    static std::string DetokenizeStructured(const TransformerModel* model, int token_id);
