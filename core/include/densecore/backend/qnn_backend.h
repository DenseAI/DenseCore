/**
 * @file densecore/backend/qnn_backend.h
 * @brief Qualcomm QNN Backend Implementation
 *
 * Experimental scaffold for Qualcomm Hexagon NPU integration via the QNN SDK.
 * The current implementation is opt-in and still delegates most compute to
 * maintained CPU kernels.
 *
...
public:
    ...
    static bool ExperimentalRegistrationEnabled();
     ...
     *
     * Note: this experimental scaffold still uses CPU fallback kernels for many
    ...
    void FlashAttention(const Tensor& Q, const Tensor& K, const Tensor& V, Tensor* output, float scale, bool causal,
                        int n_head_kv, int sliding_window = -1, float logit_softcap = 0.0f, uint32_t semantic_flags = 0,
                        int q_start_offset = 0, int kv_start_offset = 0) override;
