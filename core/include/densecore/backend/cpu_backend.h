...
#include "densecore/hal/tensor_view.h"
...
public:
    ...
    DENSECORE_API bool MatMulGgmlQuantizedTransB(const Tensor& A, const GgmlQuantizedMatrixView& W, Tensor* C,
                                                 int numa_node_id = 0);
    GgmlQuantizedMatMulPath MatMulGgmlQuantizedTransBWithPath(const CpuExecutionOptions& options,
                                                            const Tensor& A, const GgmlQuantizedMatrixView& W,
    ...
    // Configure this backend only; does not mutate inference configuration.
    void ConfigureThreads(int n_threads);
    ...
    struct ExpertWeights {
        ...
        int numa_node_hint = -1;            ///< Loader-qualified dominant NUMA residency
    ...
    using MovePagesTestHook = std::function<long(size_t count, void** pages, const int* nodes, int* status)>;
    using QueryNumaNodeRangeTestHook = std::function<int(const void* ptr, size_t size)>;

    void SetNumaRebalanceTestHooks(MovePagesTestHook move_pages_hook, QueryNumaNodeRangeTestHook query_range_hook);
     ...
     */
    // Internal numerical execution entrypoints take caller-owned options.
    void DispatchExpertFFN(const CpuExecutionOptions& options, const TransformerLayer* layer_key, int expert_id, const Tensor& input,
                                   const ExpertWeights& expert, const Tensor& w1, const Tensor& w2, const Tensor& w3,
                                   Tensor* output);
    void ForwardMoE(const CpuExecutionOptions& options, const TransformerModel* model, const TransformerLayer* layer_key, int layer_idx,
...
#ifdef DENSECORE_TEST_BUILD
    const OpRegistry* GetOpRegistryForTest() const { return op_registry_; }
#endif
