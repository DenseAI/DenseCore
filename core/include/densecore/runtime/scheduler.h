/**
 * @file densecore/runtime/scheduler.h
...
struct SchedulerConfig {
    ...
    // Restrict a decode batch to sequences sharing one context bucket. Off by
    // default: heterogeneous decode batches schedule better and the maintained
    // decode path handles them.
    bool decode_homogeneous_batch_n_past = false;
    ...
    // Fill available slots while every running sequence is still at its first
    // decode step and waiting prompts fit one chunk. Existing streams and
    // chunked prompts retain normal fairness.
    bool fill_slots_before_first_decode = false;
