...
struct InferenceDependencies {
    ...
    densecore::CpuBackend* cpu_backend = nullptr;
...
enum class NativeMoENumaStickyState : int {
    ...
    DisabledByPolicy,          // experimental sticky routing requires explicit opt-in
    DisabledUnqualifiedModel,  // model family has no positive fallback-free A/B qualification
};
...
void RecordQwenTargetGgmlComputeFallback(InferenceWorkContext* ctx, const TransformerModel* model,
                                         ...
                                         ggml_type weight_type, int m, int n, int k);
