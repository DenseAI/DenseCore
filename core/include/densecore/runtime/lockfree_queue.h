/**
 * @file densecore/runtime/lockfree_queue.h
...
// Use standard 64 bytes to avoid ABI instability warnings across compilers
...
static_assert(sizeof(TaggedPtr) == sizeof(uint64_t), "TaggedPtr must be 64 bits for atomic operations");
        ...
        while (true) {
            ...
            if (tail == tail_.load(std::memory_order_acquire)) {
                if (next.Ptr() == nullptr) {
                    ...
                    if (tail_node->next.compare_exchange_weak(next, new_next, std::memory_order_release,
                    ...
                    tail_.compare_exchange_strong(tail, new_tail, std::memory_order_release, std::memory_order_relaxed);
                }
            }
        }
    }
...
/**
 * @brief Sharded priority queue using mutex-protected FIFO queues per tier
 *
...
public:
    // Remove the highest-priority matching item without changing FIFO order of
    // survivors. Used only while activation is blocked to retire cancellations;
    // normal admission keeps the constant-time Pop path.
    template <typename Predicate> T* PopMatching(Predicate matches) {
        std::lock_guard<std::mutex> lock(mutex_);
        for (auto* queue : {&premium_, &standard_, &batch_}) {
            const auto selected = std::find_if(queue->begin(), queue->end(), matches);
            if (selected != queue->end()) {
                T* item = *selected;
                // Erasing pointer elements preserves survivor order without
                // allocating. A throwing predicate leaves the queue untouched.
    ...
    void Push(T* item, const std::string& tier) {
        std::lock_guard<std::mutex> lock(mutex_);
        if (tier == "premium") {
            premium_.push_back(item);
        } else if (tier == "batch") {
            batch_.push_back(item);
        } else {
            standard_.push_back(item);
        }
        size_++;
    }
...
private:
    ...
    std::deque<T*> premium_;    // Tier 0: highest priority
    std::deque<T*> standard_;   // Tier 1: default
    std::deque<T*> batch_;      // Tier 2: lowest priority
    size_t size_{0};
};
