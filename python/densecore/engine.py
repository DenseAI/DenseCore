...
def _terminal_callback_error(token: str, finished: bool) -> Optional[DenseCoreError]:
    """Convert the C callback's terminal error convention into a Python exception."""
    if not finished:
        return None
    message = token.strip()
    if not message.startswith("Error:"):
        return None
    if "out of memory" in message.lower() or "available memory" in message.lower():
        return OOMError(message)
    return DenseCoreRuntimeError(message)
...
class DenseCore:
    ...
    def generate(
        ...
        **kwargs,
        ...
        def handler(token: str, finished: bool) -> None:
            result_queue.put((token, finished, _terminal_callback_error(token, finished)))
        ...
        tokens: List[str] = []
        while True:
            token, finished, terminal_error = result_queue.get()
    ...
    async def generate_async(
        ...
        **kwargs,
        ...
        def handler(token: str, finished: bool) -> None:
            ...
            if finished:
                terminal_error = _terminal_callback_error(token, finished)
                if terminal_error is not None:
                    loop.call_soon_threadsafe(future.set_exception, terminal_error)
                else:
        ...
        **kwargs,
        ...
        def handler(token: str, finished: bool) -> None:
            loop.call_soon_threadsafe(
                ...
                (token, finished, _terminal_callback_error(token, finished)),
            )
        ...
        while True:
            token, finished, terminal_error = await async_queue.get()
            if terminal_error is not None:
                raise terminal_error
            if token:
                yield token
    ...
    def generate_batch(
            ...
            def make_handler(q: queue.Queue) -> Callable[[str, bool], None]:
                ...
                def handler(token: str, finished: bool) -> None:
                    ...
                    if terminal_error is None:
                        tokens.append(token)
                    if finished:
                        q.put(terminal_error if terminal_error is not None else "".join(tokens))
        ...
        for result_queue in result_queues:
            ...
            if isinstance(result, DenseCoreError):
                raise result
    ...
    def transcribe(
        ...
        """
        Reserved Whisper transcription API.

        The v0.1.0 Python package does not implement audio inference and this
        ...
        """
        raise NotImplementedError(
            "Whisper inference is not implemented in the DenseCore v0.1.0 Python package."
        )
    ...
    def detect_language(
        ...
        """
        Reserved Whisper language-detection API.
        ...
        """
        raise NotImplementedError(
            "Whisper language detection is not implemented in DenseCore v0.1.0."
        )
