...
from densecore.engine import (
    CALLBACK_TYPE,
    DenseCore,
    DenseCoreRuntimeError,
    OOMError,
    _terminal_callback_error,
)
...
def test_terminal_callback_error_promotes_graph_build_failure():
    error = _terminal_callback_error(
        "Error: graph build failed: target fallback-free matmul cannot fall back", True
    )

    assert isinstance(error, DenseCoreRuntimeError)
    assert "fallback-free" in str(error)


def test_terminal_callback_error_classifies_memory_admission_failure():
    error = _terminal_callback_error(
        "Error: graph context reservation exceeds safe available memory", True
    )

    assert isinstance(error, OOMError)


def test_nonterminal_error_text_remains_stream_content():
    assert _terminal_callback_error("Error: appears in generated prose", False) is None


def test_chat_raises_on_terminal_error(monkeypatch):
    engine = _make_engine("/tmp/gemma-4-E2B-it-Q4_K_M.gguf")

    def fake_resolve_generation_params(config, kwargs, default_max_tokens=256):
        return {
            "max_tokens": 4,
            "temperature": 0.0,
            "top_p": 1.0,
    ...
    def fake_submit_request(req_id, prompt_arg, params):
        engine._requests[req_id](
            "Error: graph context reservation exceeds safe available memory", True
        )
        return 1

    monkeypatch.setattr(engine, "_resolve_generation_params", fake_resolve_generation_params)
    monkeypatch.setattr(engine, "_submit_request", fake_submit_request)

    with pytest.raises(OOMError, match="graph context reservation exceeds safe available memory"):
        engine.chat(
