...
def test_qwen_thinking_enabled_defaults_are_model_specific():
    ...
    assert qwen_thinking_enabled("/tmp/Qwen3.8-27B-UD-Q4_K_M.gguf", None) is True
    assert qwen_thinking_enabled("/tmp/Qwen3.8-27B-UD-Q4_K_M.gguf", False) is False


def test_qwen38_prompt_matches_official_thinking_system_prefix():
    prompt = format_chat_prompt(
        "/tmp/Qwen3.8-27B-UD-Q4_K_M.gguf",
        [
            {"role": "system", "content": "Answer as a careful systems engineer."},
            {"role": "user", "content": "Name two release gates."},
        ],
...
def test_qwen38_no_thinking_uses_closed_think_without_directive():
    prompt = format_chat_prompt(
        "/tmp/Qwen3.8-27B-UD-Q4_K_M.gguf",
