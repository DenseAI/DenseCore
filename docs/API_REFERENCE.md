...
Install the beta/source-preview package from this checkout:
...
- Protobuf: [`../proto/densecore.proto`](../proto/densecore.proto)
...
The v0.1.0 server loads one immutable model from `--model` or
`MAIN_MODEL_PATH` during startup. Dynamic load/unload and speculative draft
models are not part of the public HTTP contract.
...
Liveness means the process is alive. Startup and readiness remain non-OK until
the configured startup model is ready.
