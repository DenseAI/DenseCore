...
func TestFormatChatPromptQwen38DefaultsThinkingOnAndSupportsNoThinking(t *testing.T) {
	prompt := FormatChatPrompt("/tmp/Qwen3.8-27B-UD-Q4_K_M.gguf", []domain.Message{
		{Role: "user", Content: "hello"},
	}, nil)

	expectedThinking := "<|im_start|>system\n" + qwen38ThinkingSystemPrompt + "<|im_end|>\n" +
		"<|im_start|>user\nhello<|im_end|>\n<|im_start|>assistant\n<think>\n"
	if prompt != expectedThinking {
		t.Fatalf("unexpected qwen3.8 thinking prompt: %q", prompt)
	}
	...
	expected := "<|im_start|>system\nYou are terse.\n\nPrefer bullet points.<|im_end|>\n" +
		"<|im_start|>user\n안녕?<|im_end|>\n<|im_start|>assistant\n<think>\n\n</think>\n\n"
	if prompt != expected {
		t.Fatalf("unexpected qwen3.8 no-thinking prompt: %q", prompt)
	}
}
