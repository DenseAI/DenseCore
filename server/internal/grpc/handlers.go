...
func (h *DenseCoreHandler) ChatCompletion(ctx context.Context, req *ChatCompletionRequest) (*ChatCompletionResponse, error) {
	...
	generation, err := h.chatService.SubmitGeneration(ctx, domainReq, 100)
	if err != nil {
...
// LoadModel is retained in the protobuf ABI but disabled by the fixed-model server.
func (h *DenseCoreHandler) LoadModel(ctx context.Context, req *LoadModelRequest) (*LoadModelResponse, error) {
	return nil, status.Error(codes.Unimplemented, "v0.1.0 server uses one startup model; deploy a new process to change models")
}

// UnloadModel is retained in the protobuf ABI but disabled by the fixed-model server.
func (h *DenseCoreHandler) UnloadModel(ctx context.Context, req *UnloadModelRequest) (*UnloadModelResponse, error) {
	return nil, status.Error(codes.Unimplemented, "v0.1.0 server uses one startup model; unload is not supported")
}
