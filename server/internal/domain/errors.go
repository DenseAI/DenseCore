...
// ContextLimitError reports the bound model's output-token limit.
type ContextLimitError struct{ Limit int }

func (e *ContextLimitError) Error() string {
	return fmt.Sprintf("max_tokens exceeds model context limit (%d)", e.Limit)
}
