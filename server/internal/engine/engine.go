...
// Optional for compatibility with older native libraries.
extern void generationCompletionGateway(int tokens, DenseCoreGenerationFinishReason reason, void* user_data);
static void InstallGenerationCompletionCallback(DenseCoreHandle handle) {
    typedef int (*SetCompletionFn)(DenseCoreHandle, GenerationCompletionCallback);
    SetCompletionFn fn = (SetCompletionFn)DenseCoreResolveExport("DenseCoreSetGenerationCompletionCallback");
    if (fn) fn(handle, (GenerationCompletionCallback)generationCompletionGateway);
}
...
type DenseEngine struct {
	...
	generationPolicyOnce sync.Once
	generationExclusive  bool
...
func registerGeneration(outputChan chan domain.StreamEvent) generationRegistration {
	return registerGenerationContext(context.Background(), outputChan)
}

func registerGenerationContext(ctx context.Context, outputChan chan domain.StreamEvent) generationRegistration {
	callbackID := uintptr(atomic.AddUint64(&requestIDCounter, 1))
	requestChannels.StoreContext(callbackID, outputChan, ctx)
	return generationRegistration{
		callbackID:   callbackID,
		completionCh: completionChannels.Register(callbackID),
	}
}

func (r generationRegistration) abort() {
	Cleanup(r.callbackID)
}

func (e *DenseEngine) submitGeneration(
	ctx context.Context,
	outputChan chan domain.StreamEvent,
	submit func(callbackID uintptr) int,
) (<-chan struct{}, int, uintptr, error) {
	registration := registerGenerationContext(ctx, outputChan)
	engineReqID := submit(registration.callbackID)
	if engineReqID < 0 {
		registration.abort()
		return nil, 0, registration.callbackID, fmt.Errorf("submission failed with error code %d", engineReqID)
	}

	go e.watchContext(ctx, registration.callbackID, engineReqID, registration.completionCh)
	return registration.completionCh, engineReqID, registration.callbackID, nil
}

type cSamplingArguments struct {
	lora          *C.char
	stopSequences **C.char
	stopCleanup   func()
	allowed       []C.int
	disallowed    []C.int
	allowedPtr    *C.int
...
func newCSamplingArguments(
	...
	args := cSamplingArguments{
		lora:          C.CString(loraAdapter),
		allowed:       make([]C.int, len(allowedTokenIDs)),
		disallowed:    make([]C.int, len(disallowedTokenIDs)),
		jsonMode:      boolToCInt(jsonMode),
		allowedStrict: boolToCInt(allowedTokensStrict),
	}
	args.stopSequences, args.stopCleanup = buildStopSequences(stop)
	for i, id := range allowedTokenIDs {
		args.allowed[i] = C.int(id)
	}
	if len(args.allowed) > 0 {
		args.allowedPtr = (*C.int)(unsafe.Pointer(&args.allowed[0]))
	}
	for i, id := range disallowedTokenIDs {
		args.disallowed[i] = C.int(id)
	}
	if len(args.disallowed) > 0 {
		args.disallowedPtr = (*C.int)(unsafe.Pointer(&args.disallowed[0]))
	}
	return args
}

func (args *cSamplingArguments) cleanup() {
	if args == nil {
		return
	}
	C.free(unsafe.Pointer(args.lora))
	if args.stopCleanup != nil {
		args.stopCleanup()
	}
}
...
func NewDenseEngine(mainModelPath, draftModelPath string, threads int) (*DenseEngine, error) {
	...
	C.InstallGenerationCompletionCallback(handle)
	...
	eng := &DenseEngine{handle: handle}
	eng.initializeGenerationPolicy()
	return eng, nil
}
...
func (e *DenseEngine) GenerateStream(ctx context.Context, prompt string, maxTokens int, outputChan chan domain.StreamEvent) error {
	...
	_, engineReqID, callbackID, err := e.submitGeneration(ctx, outputChan, func(callbackID uintptr) int {
		e.mu.Lock()
		defer e.mu.Unlock()
		return int(C.SubmitRequestWrapper(e.handle, cPrompt, C.int(maxTokens), C.uintptr_t(callbackID)))
	})
	if err != nil {
		return err
	}
	if requestLifecycleDebugEnabled() {
		log.Printf("request lifecycle: submit_text callback_id=%d engine_request_id=%d max_tokens=%d", callbackID, engineReqID, maxTokens)
	}
	return nil
}
...
func (e *DenseEngine) GenerateStreamWithFormat(ctx context.Context, prompt string, maxTokens int, jsonMode bool, outputChan chan domain.StreamEvent) error {
	...
	_, engineReqID, callbackID, err := e.submitGeneration(ctx, outputChan, func(callbackID uintptr) int {
		e.mu.Lock()
		defer e.mu.Unlock()
		return int(C.SubmitRequestWithFormatWrapper(
			e.handle, cPrompt, C.int(maxTokens), boolToCInt(jsonMode), C.uintptr_t(callbackID),
		))
	})
	if err != nil {
		return err
	...
	if requestLifecycleDebugEnabled() {
		log.Printf("request lifecycle: submit_format callback_id=%d engine_request_id=%d max_tokens=%d json_mode=%t",
			callbackID, engineReqID, maxTokens, jsonMode)
	}
	return nil
}
...
func (e *DenseEngine) GenerateStreamWithSamplingAwaitable(ctx context.Context, prompt string, maxTokens int,
	...
	cPrompt := C.CString(prompt)
	defer C.free(unsafe.Pointer(cPrompt))
	args := newCSamplingArguments(loraAdapter, jsonMode, stop, allowedTokenIDs, allowedTokensStrict, disallowedTokenIDs)
	defer args.cleanup()

	completionCh, engineReqID, callbackID, err := e.submitGeneration(ctx, outputChan, func(callbackID uintptr) int {
		return int(C.SubmitRequestWithSamplingConstraintsWrapper(
			e.handle,
			cPrompt,
			C.int(maxTokens),
			args.lora,
			...
			args.jsonMode,
			args.allowedPtr,
			C.int(len(args.allowed)),
			args.allowedStrict,
			args.disallowedPtr,
			C.int(len(args.disallowed)),
	...
	if requestLifecycleDebugEnabled() {
		log.Printf("request lifecycle: submit_sampling callback_id=%d engine_request_id=%d max_tokens=%d json_mode=%t allowed=%d disallowed=%d",
			callbackID, engineReqID, maxTokens, jsonMode, len(allowedTokenIDs), len(disallowedTokenIDs))
	}
...
func (e *DenseEngine) GenerateStreamRenderedChatWithSamplingAwaitable(ctx context.Context, renderedPrompt string, maxTokens int,
	...
	completionCh, engineReqID, callbackID, err := e.submitGeneration(ctx, outputChan, func(callbackID uintptr) int {
		return int(C.SubmitRenderedChatWithSamplingConstraintsWrapper(
			e.handle,
			cRenderedPrompt,
			C.int(maxTokens),
			args.lora,
...
func (e *DenseEngine) generateStreamTokensWithSamplingAwaitable(ctx context.Context, renderedPrompt string, inputIDs []int, maxTokens int,
	...
	completionCh, engineReqID, callbackID, err := e.submitGeneration(ctx, outputChan, func(callbackID uintptr) int {
		if cRenderedPrompt != nil {
			return int(C.SubmitRenderedRequestIdsWithSamplingConstraintsWrapper(
				e.handle,
				cRenderedPrompt,
		...
		return int(C.SubmitRequestIdsWithSamplingConstraintsWrapper(
			e.handle,
			(*C.int)(unsafe.Pointer(&cTokens[0])),
			C.int(len(cTokens)),
			C.int(maxTokens),
	...
	if requestLifecycleDebugEnabled() {
		log.Printf("request lifecycle: submit_token_ids callback_id=%d engine_request_id=%d max_tokens=%d input_ids=%d json_mode=%t allowed=%d disallowed=%d",
			callbackID, engineReqID, maxTokens, len(inputIDs), jsonMode, len(allowedTokenIDs), len(disallowedTokenIDs))
	}
	return completionCh, nil
}
...
func (e *DenseEngine) GetEmbeddingsWithOptions(prompt string, poolingType string, normalize *bool) ([]float32, error) {
	...
	outputChan := make(chan embeddingResult, 1)
	...
	case <-time.After(EmbeddingTimeout):
		e.mu.Lock()
		if e.handle != nil {
			C.CancelRequestWrapper(e.handle, C.int(ret))
		}
		e.mu.Unlock()
		<-outputChan
...
// BeginShutdown stops native admission and resolves accepted callbacks while
// retaining the handle for the final Close after request leases are released.
func (e *DenseEngine) BeginShutdown() {
	e.mu.Lock()
	defer e.mu.Unlock()
	if e.handle != nil {
		C.BeginEngineShutdown(e.handle)
	}
}
...
func (e *DenseEngine) RenderChatPrompt(messages []domain.Message, enableThinking *bool,
	...
	e.mu.Lock()
	defer e.mu.Unlock()
	// The C API returns borrowed thread-local strings. Keep both the engine
	// alive and this OS thread reserved until every field has been copied into
	// Go memory; another goroutine may otherwise reuse the native TLS buffers.
