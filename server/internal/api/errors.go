...
const (
	...
	ErrCodeServiceBusy      = "service_unavailable"
...
func sendError(w http.ResponseWriter, message, errType, code string, statusCode int) {
	...
	if statusCode == http.StatusServiceUnavailable && code == ErrCodeServiceBusy {
...
func classifyGenerationError(err error) (message, errType, code string, statusCode int) {
	...
	var limitErr *domain.ContextLimitError
	switch {
	case errors.As(err, &limitErr):
		return limitErr.Error(), "invalid_request_error", ErrCodeInvalidRequest, http.StatusBadRequest
	...
	case errors.Is(err, domain.ErrServiceBusy):
		return err.Error(), "service_unavailable", ErrCodeServiceBusy, http.StatusServiceUnavailable
