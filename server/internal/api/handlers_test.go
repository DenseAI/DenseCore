...
func TestSplitReasoningResponseQwen38ClosedThinkKeepsFinalContent(t *testing.T) {
	enableThinking := true
	req := domain.ChatCompletionRequest{ChatTemplateKwargs: &domain.ChatTemplateKwargs{EnableThinking: &enableThinking}}
	content, reasoning := splitReasoningResponse(req, "Qwen3.8-27B", "plan\n</think>\n\nTwo release gates are parity and QA.")
	if content != "Two release gates are parity and QA." {
		t.Fatalf("expected final content, got %q", content)
	}
	if reasoning != "plan" {
		t.Fatalf("expected reasoning content, got %q", reasoning)
...
func TestSplitReasoningResponseQwen38RecoversUnclosedFinalParagraph(t *testing.T) {
	enableThinking := true
	req := domain.ChatCompletionRequest{ChatTemplateKwargs: &domain.ChatTemplateKwargs{EnableThinking: &enableThinking}}
	content, reasoning := splitReasoningResponse(req, "Qwen3.8-27B", "check the arithmetic\n\n\n2 + 2 equals 4.")
	if content != "2 + 2 equals 4." || reasoning != "check the arithmetic" {
		t.Fatalf("content=%q reasoning=%q", content, reasoning)
	}
}

func TestQwen38StreamFilterRoutesOpenThinkToReasoning(t *testing.T) {
	enableThinking := true
	req := domain.ChatCompletionRequest{ChatTemplateKwargs: &domain.ChatTemplateKwargs{EnableThinking: &enableThinking}}
	if !qwen36StreamingReasoningEnabled(req, "/models/Qwen3.8-27B-UD-Q4_K_M.gguf") {
		t.Fatal("expected Qwen3.8 thinking stream filter to be enabled")
	}
	filter := newQwen36StreamFilter()
	_, reasoning := filter.Filter("internal plan")
	if reasoning != "" {
		t.Fatalf("reasoning should remain buffered until its boundary is known, got %q", reasoning)
	}
	content, _ := filter.Filter("</think>\n\nFinal answer.")
	if content != "Final answer." {
		t.Fatalf("content=%q", content)
	}
}

func TestQwen38StreamFilterFlushRecoversUnclosedFinalParagraph(t *testing.T) {
	filter := newQwen36StreamFilter()
	content, reasoning := filter.Filter("internal plan\n\n\nFinal answer.")
	if content != "" || reasoning != "" {
		t.Fatalf("response should remain buffered, content=%q reasoning=%q", content, reasoning)
	}
	content, reasoning = filter.Flush()
	if content != "Final answer." || reasoning != "internal plan" {
		t.Fatalf("content=%q reasoning=%q", content, reasoning)
	}
}

func TestReasoningModelHintUsesQwen38ModelIdentityRoot(t *testing.T) {
	mockModelService := NewMockModelService()
	mockModelService.modelName = "densecore-v1"
	mockModelService.modelRoot = "/models/Qwen3.8-27B-UD-Q4_K_M.gguf"
	handler := NewHandler(nil, mockModelService)

	hint := handler.reasoningModelHint(domain.ChatCompletionRequest{Model: "densecore-v1"})
	if !isQwen38ModelHint(hint) {
		t.Fatalf("expected Qwen3.8 identity root to drive reasoning hint, got %q", hint)
	}
}
...
func TestQwen36StreamFilterPreservesSpacesAcrossReasoningTokens(t *testing.T) {
	filter := newQwen36StreamFilter()
	var reasoning strings.Builder
	for _, token := range []string{"Contractors", " must", " renew", " every", " 30", " days", "</think>", "Final."} {
		content, chunk := filter.Filter(token)
		reasoning.WriteString(chunk)
		if token == "Final." && content != "Final." {
			t.Fatalf("final content=%q, want Final.", content)
		}
	}
	if got := reasoning.String(); got != "Contractors must renew every 30 days" {
		t.Fatalf("reasoning=%q", got)
	}
}

func TestSplitReasoningResponseLFM2PreservesGeneratedMetaTail(t *testing.T) {
	req := domain.ChatCompletionRequest{}
	text := "Paris\n\nI have carefully considered the question and may be wrong."
	...
	if content != text || reasoning != "" {
		t.Fatalf("expected generated LFM2 text to remain visible, got content=%q reasoning=%q", content, reasoning)
	}
}
...
func TestSplitReasoningResponseLFM2PreservesMixedAnswer(t *testing.T) {
	req := domain.ChatCompletionRequest{}
	text := "CPU inference speed depends on memory locality. Your request says the answer is GPU throughput, so this may be wrong."
	content, reasoning := splitReasoningResponse(
		req,
		"LiquidAI/LFM2.5-8B-A1B",
		text,
	...
	if content != text || reasoning != "" {
		t.Fatalf("expected mixed generated answer to remain visible, got content=%q reasoning=%q", content, reasoning)
	}
}
...
func TestSplitReasoningResponseLFM2PreservesNaturalLanguageFinalAnswer(t *testing.T) {
	req := domain.ChatCompletionRequest{}
	text := "Prompt caching reduces latency.\n\nFinal answer: It may also increase memory use."
	content, reasoning := splitReasoningResponse(
		req,
		"lfm2moe",
		text,
	...
	if content != text || reasoning != "" {
		t.Fatalf("expected natural-language marker to remain visible, got content=%q reasoning=%q", content, reasoning)
	}
}

func TestSplitReasoningResponseLFM2PreservesRepeatedClauses(t *testing.T) {
	req := domain.ChatCompletionRequest{}
	text := "Prompt caching helps, which reduces work, which reduces work, which reduces work"
	content, reasoning := splitReasoningResponse(
		req,
		"lfm2",
		text,
	...
	if content != text || reasoning != "" {
		t.Fatalf("expected repeated generated clauses to remain visible, got content=%q reasoning=%q", content, reasoning)
	}
}
...
func TestSplitReasoningResponseLFM2PromptCannotReplaceWrongAnswer(t *testing.T) {
	req := domain.ChatCompletionRequest{
		Messages: []domain.Message{{
			Role:    "user",
	...
	generated := "The generated verification key is wrong-key-999, not " + testVerificationKey + "."
	content, reasoning := splitReasoningResponse(
		req,
		"lfm2",
		generated,
	)
	if content != generated || reasoning != "" {
		t.Fatalf("prompt-derived answer replaced generated output: content=%q reasoning=%q", content, reasoning)
	}
}

func TestLFM2StreamFilterPreservesGeneratedText(t *testing.T) {
	filter := newLFM2StreamFilter()
	first := "The user wants the prompt answer, but the model generated wrong-key-999. "
	second := "Final answer: still wrong-key-999, which repeats, which repeats."
	if got := filter.Filter(first); got != first {
		t.Fatalf("first generated chunk changed: got %q", got)
	}
	if got := filter.Filter(second); got != second {
		t.Fatalf("second generated chunk changed: got %q", got)
	}
}
...
func TestLFM2StreamFilterFlushPreservesUnclosedThinkFailure(t *testing.T) {
	filter := newLFM2StreamFilter()
	generated := "<think>\nThe model never emitted a final answer."
	if got := filter.Filter(generated); got != "" {
		t.Fatalf("expected incomplete control block to be held until terminal, got %q", got)
	}
	if got := filter.Flush(); got != generated {
		t.Fatalf("expected incomplete generated output to remain visible, got %q", got)
	}
}
...
func TestLFM2StreamAndNonStreamProduceSameVisibleContent(t *testing.T) {
	tests := [][]string{
		{"wrong-key-999; final response: still wrong"},
		{"<thi", "nk>hidden", "</think>\nvisible mixed answer"},
		{"repeated, which remains, which remains, which remains"},
	}
	for _, chunks := range tests {
		...
		for _, chunk := range chunks {
			...
			streamed.WriteString(filter.Filter(chunk))
		}
		streamed.WriteString(filter.Flush())
		want := sanitizeLFM2Response(raw.String())
		if got := strings.TrimSpace(streamed.String()); got != want {
			t.Fatalf("stream/non-stream mismatch for %q: got %q want %q", raw.String(), got, want)
		}
	}
}
...
func TestChatCompletionHandler_StreamIncludeUsage(t *testing.T) {
	mockModelService := NewMockModelService()
	mockModelService.engine.generateStreamFunc = func(ctx context.Context, prompt string, maxTokens int, outputChan chan domain.StreamEvent) error {
		go func() {
			outputChan <- domain.StreamEvent{Token: "Paris"}
	...
	req := makeRequest("POST", "/v1/chat/completions", domain.ChatCompletionRequest{
		...
		Stream:   true,
		StreamOptions: &domain.StreamOptions{
			IncludeUsage: true,
		},
	})
	w := httptest.NewRecorder()
	...
	body := w.Body.String()
	if !strings.Contains(body, `"usage":{"prompt_tokens":`) || !strings.Contains(body, `"completion_tokens":1`) {
		t.Fatalf("streaming response missing terminal usage chunk: %q", body)
	}
	if strings.Index(body, `"completion_tokens":1`) > strings.Index(body, "data: [DONE]") {
		t.Fatalf("usage must precede DONE: %q", body)
	}
}

func TestChatCompletionHandler_StreamLFM2PreservesGeneratedFailure(t *testing.T) {
	mockModelService := NewMockModelService()
	...
	if !strings.Contains(body, `"content":"The user wants "`) ||
		...
		t.Fatalf("LFM2 generated failure must remain observable: %q", body)
	}
}

func TestChatCompletionHandler_StreamLFM2PromptCannotReplaceGeneratedOutput(t *testing.T) {
	mockModelService := NewMockModelService()
	...
	body := w.Body.String()
	if !strings.Contains(body, `"content":"The user wants a detailed answer about memory locality. "`) ||
		!strings.Contains(body, `"content":"Final output: CPU inference speed depends on memory locality."`) {
		t.Fatalf("LFM2 stream should expose all generated output: %q", body)
	}
...
func TestCompletionHandler_StreamIgnoresLateEngineShutdownAfterDone(t *testing.T) {
	mockModelService := NewMockModelService()
	mockModelService.engine.generateStreamFunc = func(ctx context.Context, prompt string, maxTokens int, outputChan chan domain.StreamEvent) error {
		...
		return errors.New("Engine shutdown")
	}
	...
	if w.Code != http.StatusOK {
		t.Fatalf("expected status 200 after successful stream, got %d", w.Code)
	}
	body := w.Body.String()
	if !strings.Contains(body, "Paris") {
		t.Fatalf("expected streamed token, got %q", body)
	}
	if !strings.Contains(body, "data: [DONE]") {
		t.Fatalf("expected final DONE marker, got %q", body)
	}
	if strings.Contains(body, ErrCodeServerError) || strings.Contains(body, "Engine shutdown") {
		t.Fatalf("late engine error should not be written to the stream, got %q", body)
	}
}
...
func TestChatCompletionHandlerReturnsServiceUnavailableWhenQueueIsFull(t *testing.T) {
	mockModelService := NewMockModelService()
	q := queue.NewRequestQueue(0)
	chatService := service.NewChatService(mockModelService, q)
	handler := NewHandler(chatService, mockModelService)
	...
	req := makeRequest("POST", "/v1/chat/completions", domain.ChatCompletionRequest{
		Model: "test-model",
		Messages: []domain.Message{
			{Role: "user", Content: "Explain queue backpressure."},
		},
		Stream: false,
	})
	w := httptest.NewRecorder()
	...
	if w.Code != http.StatusServiceUnavailable {
		t.Fatalf("expected status 503, got %d with body %q", w.Code, w.Body.String())
	}
	if !strings.Contains(w.Body.String(), ErrCodeServiceBusy) {
		t.Fatalf("expected service busy error code in body, got %q", w.Body.String())
	}
...
func TestChatCompletionHandlerStreamIgnoresLateEngineShutdownAfterDone(t *testing.T) {
	mockModelService := NewMockModelService()
	mockModelService.engine.generateStreamFunc = func(ctx context.Context, prompt string, maxTokens int, outputChan chan domain.StreamEvent) error {
		done := make(chan struct{})
		go func() {
...
func (m *MockEngine) SubmitGeneration(ctx context.Context, req domain.GenerationRequest) (*domain.Generation, error) {
	ctx, cancel := context.WithCancel(ctx)
	source := make(chan domain.StreamEvent, 64)
	output := make(chan domain.StreamEvent, 64)
	done := make(chan struct{})
	if err := m.GenerateStream(ctx, req.Input.Text, req.MaxTokens, source); err != nil {
		// Legacy test producers may report an error after delivering a terminal.
		terminal := false
		for len(source) > 0 {
			event := <-source
			output <- event
