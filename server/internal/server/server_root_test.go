...
func TestEffectiveRunnerShutdownTimeoutUsesConfiguredValue(t *testing.T) {
	custom := 45 * time.Second
	if got := effectiveRunnerShutdownTimeout(custom); got != custom {
		t.Fatalf("shutdown timeout = %v, want %v", got, custom)
	}
	if got := effectiveRunnerShutdownTimeout(0); got != defaultRunnerShutdownTimeout {
		t.Fatalf("zero shutdown timeout = %v, want default %v", got, defaultRunnerShutdownTimeout)
	}
}
