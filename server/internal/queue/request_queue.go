...
// GenerationResult is the single queue submission result contract.
...
func (q *RequestQueue) Drain() []*QueuedRequest {
	...
	drained := make([]*QueuedRequest, 0, len(q.heap))
	for len(q.heap) > 0 {
		drained = append(drained, heap.Pop(&q.heap).(*QueuedRequest))
	}
	return drained
}
